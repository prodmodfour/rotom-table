...
export const buildSessionHostChecklist = (input = {}) => {
  ...
  return [
    ...
    `Player-facing base URL shape: ${config.playerBaseUrl.replace(':3000', `:${config.port}`)} (legacy session lobby at /sessions#player-lobby-title; normal profile play uses /maps/<slug>).`,
    config.followUp,
