...
const generated = {
  schemaVersion: 1,
  fixtureSetId: 'pokemon-contest-variant-matrix-v1',
  sources: [
  ...
  scenarios: scenarios.map(scenario => ({ ...scenario, expected: run(scenario) })),
}
...
if (process.argv.includes('--write')) {
  ...
  if (current !== serialized) throw new Error('Contest variant fixtures drifted; run generator with --write after reviewed changes.')
  console.log(`Contest variant fixtures match (${scenarios.length} scenarios).`)
}
