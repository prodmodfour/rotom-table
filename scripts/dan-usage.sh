...
Environment:
PI_DAN_USAGE_COMMAND  Pi-compatible Dan-profile command. Default: pi-dan-rinse.
USAGE
}
...
DAN_COMMAND="${PI_DAN_USAGE_COMMAND:-pi-dan-rinse}"

if ! command -v "$DAN_COMMAND" >/dev/null 2>&1; then
  pp_error "Dan Pi command not found: $DAN_COMMAND"
  pp_hint "Install pi-dan-rinse or set PI_DAN_USAGE_COMMAND for this invocation."
  exit 127
fi
