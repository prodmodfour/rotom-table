...
PI_MONITOR_AGENT_COMMAND
                      ...
                      PI_AGENT_COMMAND, then pi-dan-rinse.
...
MONITOR_AGENT_COMMAND="${PI_MONITOR_AGENT_COMMAND:-${PI_AGENT_COMMAND:-pi-dan-rinse}}"
      ...
      END {
        ...
        for (row = 1; row <= row_count; row += 1) {
          ...
          if (command[row] ~ /scripts\/quality-gate\.sh/) {
            ...
            detail = "npm ci"
          } else if (command[row] ~ /npm (test|run test)([[:space:]]|$)/ || command[row] ~ /vitest/) {
            detail = "Vitest test run"
          } else if (command[row] ~ /npm run typecheck/ || command[row] ~ /nuxt typecheck/) {
            detail = "Nuxt typecheck"
          } else if (command[row] ~ /npm run build/ || command[row] ~ /nuxt build/) {
            detail = "Nuxt build"
