...
describe('Track 1 benchmark map fixture generator', () => {
  it('builds deterministic valid map documents from public example sheets', () => {
    ...
    for (const fixture of fixtures) {
      ...
      for (const placement of fixture.placements) {
        ...
        expect(existsSync(join(process.cwd(), 'data/sheets/examples', fileName))).toBe(true)
      }
  ...
  it('matches the empty, typical, and stress scenario sizes', () => {
    ...
    expect(typical.placements.length).toBeGreaterThanOrEqual(8)
    ...
    expect(stress.placements.length).toBeGreaterThanOrEqual(40)
