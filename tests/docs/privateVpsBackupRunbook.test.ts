...
describe('private VPS backup and restore runbook docs', () => {
  it('documents SQLite-aware backup, verification, restore, and bad-deploy rollback steps', () => {
    ...
    expect(backups).toContain('npm run backup:campaign')
    expect(backups).toContain('--method online-sqlite-backup-api')
    expect(backups).toContain('--method stopped-service-copy')
    expect(backups).toContain('npm run restore:campaign')
    expect(backups).toContain('npm run audit:campaign')
    expect(backups).toContain('Database downgrade is unsupported; rollback means restoring the exact pre-upgrade backup.')
