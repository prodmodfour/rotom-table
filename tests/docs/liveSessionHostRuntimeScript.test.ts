...
describe('live session host runtime scripts documentation', () => {
  const guide = readText('docs/live-session-host-runtime.md')
  ...
  it('exposes the package scripts and links the runtime guide from primary docs', () => {
    ...
    expect(readText('README.md')).toContain('docs/live-session-host-runtime.md')
    expect(readText('README.md')).toContain('npm run dev:session:lan')
    expect(readText('docs/README.md')).toContain('live-session-host-runtime.md')
    ...
    expect(readText('docs/live-session-lan-hosting.md')).toContain('live-session-host-runtime.md')
    expect(readText('docs/live-session-cloudflare-tunnel-hosting.md')).toContain('live-session-host-runtime.md')
  })
})
