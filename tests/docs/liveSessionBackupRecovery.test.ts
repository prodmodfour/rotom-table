...
describe('Live session backup and recovery runbook', () => {
  const runbook = readText('docs/live-session-backup-recovery.md')
  ...
  it('documents snapshot and event-log backup artifacts without changing the persistence model', () => {
    ...
    expect(runbook).toContain('Live session does not add Postgres, Redis, Durable Objects, cloud object storage, automatic replication, or a hosted database')
  })
  ...
  it('is linked from primary Live session docs and hosting runbooks', () => {
    expect(readText('README.md')).toContain('docs/live-session-backup-recovery.md')
    expect(readText('docs/README.md')).toContain('live-session-backup-recovery.md')
    expect(readText('docs/local-development.md')).toContain('live-session-backup-recovery.md')
    expect(readText('docs/live-session-roadmap.md')).toContain('live-session-backup-recovery.md')
    expect(readText('docs/live-session-protocol.md')).toContain('live-session-backup-recovery.md')
    expect(readText('docs/live-session-storage.md')).toContain('live-session-backup-recovery.md')
    expect(readText('docs/live-session-host-runtime.md')).toContain('live-session-backup-recovery.md')
    expect(readText('docs/live-session-lan-hosting.md')).toContain('live-session-backup-recovery.md')
    expect(readText('docs/live-session-cloudflare-tunnel-hosting.md')).toContain('live-session-backup-recovery.md')
  })
})
