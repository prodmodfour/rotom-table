...
describe('Track 2 local-mode no-regression audit', () => {
  const audit = readText('docs/track-2-local-mode-no-regression-audit.md')

  it('records the ticket 093 local-first audit outcome and scope', () => {
    expect(audit).toContain('ticket 093 final audit')
  ...
  it('documents accepted limitations without changing locked architecture', () => {
    ...
    expect(audit).toContain('not the live Track 2 concurrency mechanism')
    ...
    expect(audit).toContain('does not broaden Track 2 into public authentication, SaaS hosting, a generic collaborative document editor, cloud persistence, or Quick Tunnel campaign hosting')
  })

  it('is linked from primary Track 2 and local-development docs', () => {
    expect(readText('README.md')).toContain('docs/track-2-local-mode-no-regression-audit.md')
    expect(readText('docs/README.md')).toContain('track-2-local-mode-no-regression-audit.md')
    expect(readText('docs/local-development.md')).toContain('track-2-local-mode-no-regression-audit.md')
    expect(readText('docs/track-2-roadmap.md')).toContain('track-2-local-mode-no-regression-audit.md')
    expect(readText('docs/track-2-validation-matrix.md')).toContain('track-2-local-mode-no-regression-audit.md')
    expect(readText('docs/track-2-client-integration.md')).toContain('track-2-local-mode-no-regression-audit.md')
    expect(readText('docs/track-2-command-audit.md')).toContain('track-2-local-mode-no-regression-audit.md')
  })
})
