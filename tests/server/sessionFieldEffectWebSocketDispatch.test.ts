...
const SESSION_ID = parseSessionId('session_socketfield001')
const OTHER_SESSION_ID = parseSessionId('session_socketfield002')
const JOIN_CODE = parseJoinCode('ABC267')
const OTHER_JOIN_CODE = parseJoinCode('ABC268')
const GM_KEY = parseGmKey('gmkey_socketfield000000000001x')
const OTHER_GM_KEY = parseGmKey('gmkey_socketfield000000000002x')
const GM_CLIENT_ID = parseClientId('client_socketfieldgm')
const OTHER_GM_CLIENT_ID = parseClientId('client_socketfieldog')
const PLAYER_ID = parsePlayerId('player_socketfield1')
const PLAYER_CLIENT_ID = parseClientId('client_socketfieldpl')
const PLAYER_DISPLAY_NAME = parseSessionDisplayName('Misty')
const OP_ID = parseOpId('op_socketfield01')
...
const setFieldEffectCommandMessage = (): SessionCommandMessage<SetFieldEffectCommand> => ({
  schemaVersion: SESSION_MESSAGE_SCHEMA_VERSION,
  type: 'command',
  direction: 'client',
  sessionId: SESSION_ID,
  command: {
    ...
    metadata: {
      traceId: 'trace-websocket-field-effect',
    },
  },
})

describe('field-effect WebSocket dispatch', () => {
  it('acks the sender and broadcasts a small fieldEffectsUpdated patch to same-session clients only', () => {
    ...
    const gmPeer = makePeer('peer-field-gm')
    const playerPeer = makePeer('peer-field-player')
    const otherPeer = makePeer('peer-field-other')
    const dependencies = {
      env: enabledEnv,
      registry,
      peers,
      store,
      fieldEffectCommandDependencies: {
    ...
    handleSessionSocketMessage(gmPeer, { text: () => JSON.stringify(setFieldEffectCommandMessage()) }, {
      ...dependencies,
      clock: () => MODIFIED_AT,
    })
      ...
      result: {
        ...
        event: {
          ...
          payload: {
            ...
            current: {
              weather: [{ kind: 'rainy', rounds: 4, source: 'Rain Dance' }],
              terrains: [],
              rooms: [],
            },
          },
    ...
    expect(registry.get('peer-field-gm')).toMatchObject({
      status: SESSION_SOCKET_AUTHENTICATED_STATUS,
      currentRevision: parseSessionRevision(1),
    })
    expect(registry.get('peer-field-player')).toMatchObject({
      status: SESSION_SOCKET_AUTHENTICATED_STATUS,
      currentRevision: parseSessionRevision(1),
    })
  })
