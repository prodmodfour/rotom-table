...
import {
  LUSTER_PURGE_REG_016_SCENARIOS,
  MACH_PUNCH_REG_016_SCENARIOS,
  MAGICAL_LEAF_REG_016_SCENARIOS,
  MAGNET_BOMB_REG_016_SCENARIOS,
  MEGA_PUNCH_REG_016_SCENARIOS,
  METAL_SOUND_REG_016_SCENARIOS,
  MIRROR_SHOT_REG_016_SCENARIOS,
  MIST_BALL_REG_016_SCENARIOS,
  REG_016_MOVE_NAMES,
  REG_016_SCENARIOS_BY_MOVE,
  type RegisteredBatch016MoveName,
} from '../fixtures/moveAutomation/registeredBatch016'
...
const NOW = 5_000
const BLAST_AIM_CELL = { x: 5, y: 0, z: 5 } as const

type TargetId = typeof TARGET_A_ID | typeof TARGET_B_ID | typeof TARGET_C_ID
type SelectionKind = 'single-target' | 'burst' | 'ranged-blast'
...
interface ExecutionScenario {
  ...
  readonly moveName: RegisteredBatch016MoveName
...
const targetPosition = (
  ...
  if (selectionKind === 'burst') {
    ...
    if (id === TARGET_B_ID) return { x: 1, y: 0, z: 5 }
    return { x: 2, y: 0, z: 4 }
  }
  return { x: 2, y: 0, z: 3 }
}
...
const fixtureFor = (scenario: ExecutionScenario): MoveFixture => {
  ...
  const map: TabletopMap = {
    schemaVersion: 2,
    slug: `reg-016-${scenario.scenarioId.replace(/[^a-z0-9]+/gi, '-').toLowerCase()}`,
    name: `REG-016 ${scenario.moveName}`,
    revision: 7,
    ...
    placements: [
      placement(ACTOR_ID, 'actor', { x: 1, y: 0, z: selectionKind === 'ranged-blast' ? 5 : 3 }),
      ...targetIds.map(id => placement(id, id, targetPosition(selectionKind, id))),
    ],
    lights: [],
    initiative: { activeId: ACTOR_ID, round: 1 },
    activeScene: { name: 'REG-016 scene', startedAt: 100 },
    encounterState: {
      ...emptyState,
      sides: {
      ...
      history: scenario.actedThisRound
        ? { ...emptyState.history, actedThisRoundPlacementIds: [ACTOR_ID] }
        : emptyState.history,
    },
  ...
  if (selectionKind === 'single-target') {
    ...
    selection = {
      ...
      ...(selectionKind === 'ranged-blast' ? { aimCell: { ...BLAST_AIM_CELL } } : {}),
...
const assertScenarioResolution = (
  ...
  if ((scenario.selectionKind ?? 'single-target') === 'single-target') {
    ...
    if (scenario.selectionKind === 'ranged-blast') {
...
const normalScenarios: readonly ExecutionScenario[] = [
  {
    scenarioId: LUSTER_PURGE_REG_016_SCENARIOS[0].scenarioId,
  ...
  {
    scenarioId: LUSTER_PURGE_REG_016_SCENARIOS[1].scenarioId,
    moveName: 'Luster Purge',
  ...
  {
    scenarioId: LUSTER_PURGE_REG_016_SCENARIOS[2].scenarioId,
    moveName: 'Luster Purge',
    randomValues: [0],
    expectedAttackedTargetIds: [TARGET_A_ID],
  ...
  {
    scenarioId: LUSTER_PURGE_REG_016_SCENARIOS[3].scenarioId,
    moveName: 'Luster Purge',
  ...
  {
    scenarioId: LUSTER_PURGE_REG_016_SCENARIOS[4].scenarioId,
    moveName: 'Luster Purge',
  ...
  {
    scenarioId: LUSTER_PURGE_REG_016_SCENARIOS[5].scenarioId,
    moveName: 'Luster Purge',
  ...
  {
    scenarioId: LUSTER_PURGE_REG_016_SCENARIOS[6].scenarioId,
    moveName: 'Luster Purge',
  ...
  {
    scenarioId: MACH_PUNCH_REG_016_SCENARIOS[0].scenarioId,
    moveName: 'Mach Punch',
    randomValues: [0.45, 0],
    expectedAttackedTargetIds: [TARGET_A_ID],
    expectedHitTargetIds: [TARGET_A_ID],
  ...
  {
    scenarioId: MACH_PUNCH_REG_016_SCENARIOS[1].scenarioId,
    moveName: 'Mach Punch',
    randomValues: [0],
    expectedAttackedTargetIds: [TARGET_A_ID],
    expectedHitTargetIds: [],
  ...
  {
    scenarioId: MACH_PUNCH_REG_016_SCENARIOS[2].scenarioId,
    moveName: 'Mach Punch',
  ...
  {
    scenarioId: MACH_PUNCH_REG_016_SCENARIOS[3].scenarioId,
    moveName: 'Mach Punch',
  ...
  {
    scenarioId: MAGICAL_LEAF_REG_016_SCENARIOS[0].scenarioId,
  ...
  {
    scenarioId: MAGICAL_LEAF_REG_016_SCENARIOS[1].scenarioId,
    moveName: 'Magical Leaf',
  ...
  {
    scenarioId: MAGNET_BOMB_REG_016_SCENARIOS[0].scenarioId,
  ...
  {
    scenarioId: MEGA_PUNCH_REG_016_SCENARIOS[0].scenarioId,
    moveName: 'Mega Punch',
    randomValues: [0.45, 0, 0],
    expectedAttackedTargetIds: [TARGET_A_ID],
    expectedHitTargetIds: [TARGET_A_ID],
  ...
  {
    scenarioId: MEGA_PUNCH_REG_016_SCENARIOS[1].scenarioId,
    moveName: 'Mega Punch',
    randomValues: [0],
    expectedAttackedTargetIds: [TARGET_A_ID],
    expectedHitTargetIds: [],
  ...
  {
    scenarioId: MEGA_PUNCH_REG_016_SCENARIOS[2].scenarioId,
    moveName: 'Mega Punch',
    randomValues: [0.999, 0, 0, 0, 0],
    expectedAttackedTargetIds: [TARGET_A_ID],
    expectedHitTargetIds: [TARGET_A_ID],
  ...
  {
    scenarioId: MEGA_PUNCH_REG_016_SCENARIOS[3].scenarioId,
    moveName: 'Mega Punch',
    targetProfiles: { [TARGET_A_ID]: { types: ['Ghost'] } },
    randomValues: [0.45, 0, 0],
    expectedAttackedTargetIds: [TARGET_A_ID],
  ...
  {
    scenarioId: METAL_SOUND_REG_016_SCENARIOS[0].scenarioId,
  ...
  {
    scenarioId: METAL_SOUND_REG_016_SCENARIOS[1].scenarioId,
    moveName: 'Metal Sound',
    selectionKind: 'burst',
  ...
  {
    scenarioId: METAL_SOUND_REG_016_SCENARIOS[2].scenarioId,
    moveName: 'Metal Sound',
    selectionKind: 'burst',
    initialStages: [{ recipientId: TARGET_A_ID, key: 'sdef', value: -6 }],
    randomValues: [0.45],
  ...
  {
    scenarioId: MIRROR_SHOT_REG_016_SCENARIOS[0].scenarioId,
    moveName: 'Mirror Shot',
    selectionKind: 'ranged-blast',
    targetIds: [TARGET_A_ID, TARGET_B_ID],
    randomValues: [0.75, 0, 0, 0],
    expectedStages: [{ recipientId: TARGET_A_ID, key: 'acc', value: -2 }],
    expectedAttackedTargetIds: [TARGET_A_ID, TARGET_B_ID],
    expectedHitTargetIds: [TARGET_A_ID],
    expectedDamagedTargetIds: [TARGET_A_ID],
  ...
  {
    scenarioId: MIRROR_SHOT_REG_016_SCENARIOS[1].scenarioId,
    moveName: 'Mirror Shot',
    selectionKind: 'ranged-blast',
  ...
  {
    scenarioId: MIRROR_SHOT_REG_016_SCENARIOS[2].scenarioId,
    moveName: 'Mirror Shot',
    selectionKind: 'ranged-blast',
    randomValues: [0.999, 0, 0, 0, 0],
    expectedStages: [{ recipientId: TARGET_A_ID, key: 'acc', value: -2 }],
  ...
  {
    scenarioId: MIRROR_SHOT_REG_016_SCENARIOS[3].scenarioId,
    moveName: 'Mirror Shot',
    selectionKind: 'ranged-blast',
    targetProfiles: { [TARGET_A_ID]: { abilities: ['Shield Dust'] } },
  ...
  {
    scenarioId: MIRROR_SHOT_REG_016_SCENARIOS[4].scenarioId,
    moveName: 'Mirror Shot',
    selectionKind: 'ranged-blast',
  ...
  {
    scenarioId: MIRROR_SHOT_REG_016_SCENARIOS[5].scenarioId,
    moveName: 'Mirror Shot',
    selectionKind: 'ranged-blast',
    initialStages: [{ recipientId: TARGET_A_ID, key: 'acc', value: -5 }],
    randomValues: [0.75, 0, 0],
  ...
  {
    scenarioId: MIST_BALL_REG_016_SCENARIOS[0].scenarioId,
    moveName: 'Mist Ball',
    randomValues: [0.45, 0, 0],
  ...
  {
    scenarioId: MIST_BALL_REG_016_SCENARIOS[1].scenarioId,
    moveName: 'Mist Ball',
    randomValues: [0.5, 0, 0],
    expectedAttackedTargetIds: [TARGET_A_ID],
    expectedHitTargetIds: [TARGET_A_ID],
  ...
  {
    scenarioId: MIST_BALL_REG_016_SCENARIOS[2].scenarioId,
    moveName: 'Mist Ball',
    randomValues: [0],
    expectedAttackedTargetIds: [TARGET_A_ID],
    expectedHitTargetIds: [],
  ...
  {
    scenarioId: MIST_BALL_REG_016_SCENARIOS[3].scenarioId,
    moveName: 'Mist Ball',
    randomValues: [0.999, 0, 0, 0, 0],
    expectedStages: [{ recipientId: TARGET_A_ID, key: 'satk', value: -1 }],
    expectedAttackedTargetIds: [TARGET_A_ID],
  ...
  {
    scenarioId: MIST_BALL_REG_016_SCENARIOS[4].scenarioId,
    moveName: 'Mist Ball',
    targetProfiles: { [TARGET_A_ID]: { types: ['Dark'] } },
    randomValues: [0.45, 0, 0],
    expectedAttackedTargetIds: [TARGET_A_ID],
  ...
  {
    scenarioId: MIST_BALL_REG_016_SCENARIOS[5].scenarioId,
    moveName: 'Mist Ball',
    targetProfiles: { [TARGET_A_ID]: { abilities: ['Shield Dust'] } },
    randomValues: [0.45, 0, 0],
    expectedAttackedTargetIds: [TARGET_A_ID],
  ...
  {
    scenarioId: MIST_BALL_REG_016_SCENARIOS[6].scenarioId,
    moveName: 'Mist Ball',
...
const recoveryScenarioFor = (moveName: RegisteredBatch016MoveName): ExecutionScenario => {
...
describe('REG-016 registered move conformance', () => {
  it('certifies exactly Luster Purge through Mist Ball with linked evidence', () => {
    expect(Object.keys(REG_016_SCENARIOS_BY_MOVE)).toEqual([...REG_016_MOVE_NAMES])

    for (const [canonicalId, scenarios] of Object.entries(REG_016_SCENARIOS_BY_MOVE)) {
  ...
  it('retains every reviewed canonical mechanic without unresolved rule instructions', () => {
    const expected = {
      'Luster Purge': { ac: 2, damageBase: 7, damageClass: 'Special', range: '12, 1 Target' },
      'Mach Punch': { ac: 2, damageBase: 4, damageClass: 'Physical', range: 'Melee, 1 Target, Priority' },
      'Magical Leaf': { ac: null, damageBase: 6, damageClass: 'Special', range: '8, 1 Target', requiresAccuracy: false },
      'Magnet Bomb': { ac: null, damageBase: 6, damageClass: 'Physical', range: '8, 1 target', requiresAccuracy: false },
      'Mega Punch': { ac: 4, damageBase: 8, damageClass: 'Physical', range: 'Melee, 1 Target' },
      'Metal Sound': { ac: 4, damageBase: 0, damageClass: 'Status', range: 'Burst 2, Friendly, Sonic' },
      'Mirror Shot': { ac: 5, damageBase: 7, damageClass: 'Special', range: '6, Ranged Blast 2' },
      'Mist Ball': { ac: 2, damageBase: 7, damageClass: 'Special', range: '12, 1 Target' },
    } as const
    ...
    expect(explicitScriptForMove('Luster Purge')?.stageSuggestions).toEqual([{
      recipient: 'target',
      key: 'sdef',
      delta: -1,
      label: 'Luster Purge lowers Special Defense on even roll: -1 Special Defense CS',
      threshold: 'even roll',
      optional: true,
    }])
    expect(explicitScriptForMove('Mach Punch')?.keywords).toContain('Priority')
    expect(explicitScriptForMove('Magical Leaf')).toMatchObject({
      requiresAccuracy: false,
      conditionSuggestions: [],
      stageSuggestions: [],
    })
    expect(explicitScriptForMove('Magnet Bomb')).toMatchObject({
      requiresAccuracy: false,
      special: 'Grants Magnetic',
      conditionSuggestions: [],
      stageSuggestions: [],
    })
    expect(resolveMoveGrantedCapabilities([{ name: 'Magnet Bomb' }]).other).toEqual(['Magnetic'])
    expect(explicitScriptForMove('Metal Sound')).toMatchObject({
      keywords: expect.arrayContaining(['Friendly', 'Sonic']),
      areaTemplates: [{ kind: 'burst', size: 2, label: 'Burst 2' }],
      stageSuggestions: [{ recipient: 'target', key: 'sdef', delta: -2 }],
    })
    expect(explicitScriptForMove('Mirror Shot')).toMatchObject({
      areaTemplates: [{ kind: 'ranged-blast', size: 2, range: 6, label: 'Ranged 6 Blast 2' }],
      stageSuggestions: [{
        recipient: 'target',
        key: 'acc',
        delta: -2,
    ...
    expect(explicitScriptForMove('Mist Ball')?.stageSuggestions).toEqual([{
      recipient: 'target',
      key: 'satk',
      delta: -1,
      label: 'Mist Ball lowers Special Attack on even roll: -1 Special Attack CS',
      threshold: 'even roll',
      optional: true,
    }])
  })
  ...
  it('rejects Mach Punch Priority after the actor has acted without partial mutation', async () => {
    const scenario: ExecutionScenario = {
      ...recoveryScenarioFor('Mach Punch'),
      scenarioId: MACH_PUNCH_REG_016_SCENARIOS[4].scenarioId,
      actedThisRound: true,
    }
