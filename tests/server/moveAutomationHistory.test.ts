...
describe('bounded encounter move history', () => {
  it('indexes authoritative move, damage, action, switch, KO, chain, and ancestry facts', () => {
    ...
    expect(queries.consecutiveUseCount('actor-token')).toBe(0)
    expect(queries.consecutiveUseCount('actor-token', 'Fury Cutter')).toBe(0)
    ...
    expect(queries.query('actor-token', 'consecutive-use-count')).toBe(0)
