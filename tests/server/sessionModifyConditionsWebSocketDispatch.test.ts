...
const SESSION_ID = parseSessionId('session_socketmcn001')
const OTHER_SESSION_ID = parseSessionId('session_socketmcn002')
const JOIN_CODE = parseJoinCode('MCN234')
const OTHER_JOIN_CODE = parseJoinCode('MCN235')
const GM_KEY = parseGmKey('gmkey_socketmodifyconditions001x')
const OTHER_GM_KEY = parseGmKey('gmkey_socketmodifyconditions002x')
const GM_CLIENT_ID = parseClientId('client_socketmcngm')
const OTHER_GM_CLIENT_ID = parseClientId('client_socketmcnog')
const PLAYER_ID = parsePlayerId('player_socketmcn')
const PLAYER_CLIENT_ID = parseClientId('client_socketmcnpl')
const PLAYER_DISPLAY_NAME = parseSessionDisplayName('Misty')
const OP_ID = parseOpId('op_socketmcn001')
const CREATED_AT = '2026-05-26T19:30:00.000Z'
const MODIFIED_AT = '2026-05-26T19:30:10.000Z'

const requestedConditions = ['Burned', 'Disabled: Thunderbolt'] as const
...
const modifyConditionsCommandMessage = (): SessionCommandMessage<ModifyConditionsCommand> => ({
  ...
  command: {
    ...
    metadata: {
      traceId: 'trace-websocket-modify-conditions',
    },
  },
})

describe('modifyConditions WebSocket dispatch', () => {
  it('acks the sender and broadcasts a small conditionsModified patch to same-session clients only', () => {
    ...
    const gmPeer = makePeer('peer-mcn-gm')
    const playerPeer = makePeer('peer-mcn-player')
    const otherPeer = makePeer('peer-mcn-other')
    const dependencies = {
      env: enabledEnv,
      registry,
      peers,
      store,
      modifyConditionsCommandDependencies: {
    ...
    handleSessionSocketMessage(gmPeer, { text: () => JSON.stringify(modifyConditionsCommandMessage()) }, {
      ...dependencies,
      clock: () => MODIFIED_AT,
    })
    ...
    expect(sheet.combat?.conditions).toEqual([...requestedConditions])
    ...
    expect(registry.get('peer-mcn-gm')).toMatchObject({
      status: SESSION_SOCKET_AUTHENTICATED_STATUS,
      currentRevision: parseSessionRevision(1),
    })
    expect(registry.get('peer-mcn-player')).toMatchObject({
      status: SESSION_SOCKET_AUTHENTICATED_STATUS,
      currentRevision: parseSessionRevision(1),
    })
  })
