...
const SESSION_ID = parseSessionId('session_socketmcs001')
const OTHER_SESSION_ID = parseSessionId('session_socketmcs002')
const JOIN_CODE = parseJoinCode('MCS234')
const OTHER_JOIN_CODE = parseJoinCode('MCS235')
const GM_KEY = parseGmKey('gmkey_socketmodifycs000000001xxx')
const OTHER_GM_KEY = parseGmKey('gmkey_socketmodifycs000000002xxx')
const GM_CLIENT_ID = parseClientId('client_socketmcsgm')
const OTHER_GM_CLIENT_ID = parseClientId('client_socketmcsog')
const PLAYER_ID = parsePlayerId('player_socketmcs')
const PLAYER_CLIENT_ID = parseClientId('client_socketmcspl')
const PLAYER_DISPLAY_NAME = parseSessionDisplayName('Misty')
const OP_ID = parseOpId('op_socketmcs001')
const CREATED_AT = '2026-05-26T18:30:00.000Z'
const MODIFIED_AT = '2026-05-26T18:30:10.000Z'

const requestedStages = { atk: 1, def: -1, satk: 2, sdef: 0, spd: -2, acc: 3 } as const
...
const modifyCombatStagesCommandMessage = (): SessionCommandMessage<ModifyCombatStagesCommand> => ({
  schemaVersion: SESSION_MESSAGE_SCHEMA_VERSION,
  type: 'command',
  direction: 'client',
  sessionId: SESSION_ID,
  command: {
    ...
    metadata: {
      traceId: 'trace-websocket-modify-combat-stages',
    },
  },
})

describe('modifyCombatStages WebSocket dispatch', () => {
  it('acks the sender and broadcasts a small combatStagesModified patch to same-session clients only', () => {
    ...
    const gmPeer = makePeer('peer-mcs-gm')
    const playerPeer = makePeer('peer-mcs-player')
    const otherPeer = makePeer('peer-mcs-other')
    const dependencies = {
      env: enabledEnv,
      registry,
      peers,
      store,
      modifyCombatStagesCommandDependencies: {
    ...
    handleSessionSocketMessage(gmPeer, { text: () => JSON.stringify(modifyCombatStagesCommandMessage()) }, {
      ...dependencies,
      clock: () => MODIFIED_AT,
    })
    ...
    expect(parseSentJson(gmPeer, 0)).toMatchObject({
      ...
      result: {
        ...
        opId: OP_ID,
        event: {
          eventType: 'combatStagesModified',
    ...
    expect(sheet.stats?.def?.stage).toBe(-1)
    expect(sheet.stats?.satk?.stage).toBe(2)
    expect(sheet.stats?.spd?.stage).toBe(-2)
    expect(sheet.combatStages?.acc).toBe(3)
    ...
    expect(registry.get('peer-mcs-gm')).toMatchObject({
      status: SESSION_SOCKET_AUTHENTICATED_STATUS,
      currentRevision: parseSessionRevision(1),
    })
    expect(registry.get('peer-mcs-player')).toMatchObject({
      status: SESSION_SOCKET_AUTHENTICATED_STATUS,
      currentRevision: parseSessionRevision(1),
    })
  })
