...
describe('SQLite storage migrations', () => {
  it('keeps migration versions contiguous through schema version 7', () => {
    expect(LATEST_STORAGE_SCHEMA_VERSION).toBe(7)
    expect(STORAGE_MIGRATIONS.map((migration) => migration.version)).toEqual([1, 2, 3, 4, 5, 6, 7])
  })
  ...
  it('creates realtime event-log, group inventory, and shop tables for a fresh database', () => {
    ...
    expect(result).toMatchObject({ fromVersion: 0, toVersion: 7, appliedVersions: [1, 2, 3, 4, 5, 6, 7] })
    expect(getStorageSchemaVersion(connection)).toBe(7)
  ...
  it('upgrades schema version 4 databases without deleting existing campaign rows', () => {
    ...
    expect(result).toEqual({ fromVersion: 4, toVersion: 7, appliedVersions: [5, 6, 7] })
    expect(getStorageSchemaVersion(connection)).toBe(7)
    expect(tableNames(connection)).toEqual(expectedTableNames)
    ...
    expect(applyStorageMigrations(connection)).toEqual({ fromVersion: 7, toVersion: 7, appliedVersions: [] })
  })
  ...
  it('upgrades schema version 6 databases with the shop table without touching existing rows', () => {
    ...
    expect(result).toEqual({ fromVersion: 6, toVersion: 7, appliedVersions: [7] })
    expect(getStorageSchemaVersion(connection)).toBe(7)
    expect(tableNames(connection)).toEqual(expectedTableNames)
    expect(tableColumns(connection, 'shop_tables')).toEqual(documentStoreTableColumns)
  ...
  it('upgrades schema version 5 databases with group inventory and shop tables', () => {
    ...
    expect(result).toEqual({ fromVersion: 5, toVersion: 7, appliedVersions: [6, 7] })
    expect(getStorageSchemaVersion(connection)).toBe(7)
    expect(tableNames(connection)).toEqual(expectedTableNames)
    expect(tableColumns(connection, 'group_inventories')).toEqual(documentStoreTableColumns)
    expect(tableColumns(connection, 'shop_tables')).toEqual(documentStoreTableColumns)
