...
const SESSION_ID = parseSessionId('session_socketturn01')
const OTHER_SESSION_ID = parseSessionId('session_socketturn02')
const JOIN_CODE = parseJoinCode('TRN234')
const OTHER_JOIN_CODE = parseJoinCode('TRN235')
const GM_KEY = parseGmKey('gmkey_socketturnprimary000001x')
const OTHER_GM_KEY = parseGmKey('gmkey_socketturnsecondary0002x')
const GM_CLIENT_ID = parseClientId('client_sockettgm1')
const OTHER_GM_CLIENT_ID = parseClientId('client_sockettgm2')
const PLAYER_ID = parsePlayerId('player_socketturn')
const PLAYER_CLIENT_ID = parseClientId('client_sockettplay')
const PLAYER_DISPLAY_NAME = parseSessionDisplayName('Misty')
const OP_ID = parseOpId('op_socketturn001')
const CREATED_AT = '2026-05-26T15:00:00.000Z'
const TURNED_AT = '2026-05-26T15:00:10.000Z'
...
const turnTokenCommandMessage = (): SessionCommandMessage<TurnTokenCommand> => ({
  schemaVersion: SESSION_MESSAGE_SCHEMA_VERSION,
  type: 'command',
  direction: 'client',
  sessionId: SESSION_ID,
  command: {
    ...
    metadata: {
      traceId: 'trace-websocket-turn-token',
    },
  },
})

describe('turnToken WebSocket dispatch', () => {
  it('acks the sender and broadcasts a small tokenTurned patch to same-session clients only', () => {
    ...
    const gmPeer = makePeer('peer-turn-gm')
    const playerPeer = makePeer('peer-turn-player')
    const otherPeer = makePeer('peer-turn-other')
    const dependencies = {
      env: enabledEnv,
      registry,
      peers,
    ...
    handleSessionSocketMessage(gmPeer, { text: () => JSON.stringify(turnTokenCommandMessage()) }, {
      ...dependencies,
      clock: () => TURNED_AT,
    })
    ...
    expect(registry.get('peer-turn-gm')).toMatchObject({
      status: SESSION_SOCKET_AUTHENTICATED_STATUS,
      currentRevision: parseSessionRevision(1),
    })
    expect(registry.get('peer-turn-player')).toMatchObject({
      status: SESSION_SOCKET_AUTHENTICATED_STATUS,
      currentRevision: parseSessionRevision(1),
    })
  })
