...
const SESSION_ID = parseSessionId('session_sockethazard001')
const OTHER_SESSION_ID = parseSessionId('session_sockethazard002')
const JOIN_CODE = parseJoinCode('ABC263')
const OTHER_JOIN_CODE = parseJoinCode('ABC264')
const GM_KEY = parseGmKey('gmkey_sockethazard000000001xxx')
const OTHER_GM_KEY = parseGmKey('gmkey_sockethazard000000002xxx')
const GM_CLIENT_ID = parseClientId('client_sockethazgm')
const OTHER_GM_CLIENT_ID = parseClientId('client_sockethazog')
const PLAYER_ID = parsePlayerId('player_sockethaz01')
const PLAYER_CLIENT_ID = parseClientId('client_sockethazpl')
const PLAYER_DISPLAY_NAME = parseSessionDisplayName('Brock')
const OP_ID = parseOpId('op_sockethazard1')
const CREATED_AT = '2026-05-26T19:30:00.000Z'
const MODIFIED_AT = '2026-05-26T19:30:10.000Z'
...
const placeHazardCommandMessage = (): SessionCommandMessage<PlaceHazardCommand> => ({
  schemaVersion: SESSION_MESSAGE_SCHEMA_VERSION,
  type: 'command',
  direction: 'client',
  sessionId: SESSION_ID,
  command: {
    ...
    payload: {
      mapSlug: 'arena-map',
      hazard: { kind: 'fire', x: 2, y: 0, z: 3, owner: 'GM' },
    },
    metadata: {
      traceId: 'trace-websocket-hazard',
    },
  },
})

describe('hazard WebSocket dispatch', () => {
  it('acks the sender and broadcasts a small hazardsUpdated patch to same-session clients only', () => {
    ...
    const gmPeer = makePeer('peer-hazard-gm')
    const playerPeer = makePeer('peer-hazard-player')
    const otherPeer = makePeer('peer-hazard-other')
    const dependencies = {
      env: enabledEnv,
      registry,
      peers,
      store,
      hazardCommandDependencies: {
        operationTracker: false as const,
        writeSnapshot,
      },
    }

    handleSessionSocketOpen(gmPeer, { ...dependencies, clock: () => '2026-05-26T19:30:00.000Z' })
    handleSessionSocketMessage(gmPeer, { text: () => JSON.stringify(gmHello()) }, {
      ...dependencies,
      clock: () => '2026-05-26T19:30:01.000Z',
    })
    handleSessionSocketOpen(playerPeer, { ...dependencies, clock: () => '2026-05-26T19:30:02.000Z' })
    handleSessionSocketMessage(playerPeer, { text: () => JSON.stringify(playerHello()) }, {
      ...dependencies,
      clock: () => '2026-05-26T19:30:03.000Z',
    })
    handleSessionSocketOpen(otherPeer, { ...dependencies, clock: () => '2026-05-26T19:30:04.000Z' })
    handleSessionSocketMessage(otherPeer, {
      text: () => JSON.stringify(gmHello(OTHER_SESSION_ID, OTHER_GM_KEY, OTHER_GM_CLIENT_ID)),
    }, {
      ...dependencies,
      clock: () => '2026-05-26T19:30:05.000Z',
    })
    ...
    handleSessionSocketMessage(gmPeer, { text: () => JSON.stringify(placeHazardCommandMessage()) }, {
      ...dependencies,
      clock: () => MODIFIED_AT,
    })
      ...
      result: {
        ...
        event: {
          ...
          payload: {
            ...
            cell: { x: 2, y: 0, z: 3 },
            previous: [],
            current: [{ kind: 'fire', x: 2, y: 0, z: 3, owner: 'GM' }],
            placed: { kind: 'fire', x: 2, y: 0, z: 3, owner: 'GM' },
            removed: [],
          },
        },
        metadata: {
    ...
    expect(storedMap?.document.hazards).toEqual([{ kind: 'fire', x: 2, y: 0, z: 3, owner: 'GM' }])
    expect(registry.get('peer-hazard-gm')).toMatchObject({
      status: SESSION_SOCKET_AUTHENTICATED_STATUS,
      currentRevision: parseSessionRevision(1),
    })
    expect(registry.get('peer-hazard-player')).toMatchObject({
      status: SESSION_SOCKET_AUTHENTICATED_STATUS,
      currentRevision: parseSessionRevision(1),
    })
  })
