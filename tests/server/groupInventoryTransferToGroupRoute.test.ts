...
const useFreshTestDatabase = (): void => {
  closeRotomDatabase()
  tempDirectory = mkdtempSync(join(tmpdir(), 'rotom-group-transfer-to-group-'))
...
const invokeRoute = async (
  ...
  return handler({
    ...
    path: GROUP_INVENTORY_API_PATHS.transferToGroup,
    node: {
      req: {
        url: GROUP_INVENTORY_API_PATHS.transferToGroup,
        headers,
...
describe('trainer inventory to group inventory transfer API route', () => {
  it('allows GMs to transfer item quantities and returns authoritative documents', async () => {
    ...
    expect(response.groupInventory.inventory.medicalKit).toEqual([
      { id: 'group-potion-row', name: 'Potion', qty: 4 },
    ])
  })
