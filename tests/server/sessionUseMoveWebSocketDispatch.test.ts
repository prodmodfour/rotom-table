...
const SESSION_ID = parseSessionId('session_socketumv001')
const OTHER_SESSION_ID = parseSessionId('session_socketumv002')
const JOIN_CODE = parseJoinCode('UMV234')
const OTHER_JOIN_CODE = parseJoinCode('UMV235')
const GM_KEY = parseGmKey('gmkey_socketusemove000000001xxx')
const OTHER_GM_KEY = parseGmKey('gmkey_socketusemove000000002xxx')
const GM_CLIENT_ID = parseClientId('client_socketumvgm')
const OTHER_GM_CLIENT_ID = parseClientId('client_socketumvog')
const PLAYER_ID = parsePlayerId('player_socketumv')
const PLAYER_CLIENT_ID = parseClientId('client_socketumvpl')
const PLAYER_DISPLAY_NAME = parseSessionDisplayName('Misty')
const OP_ID = parseOpId('op_socketumv001')
const CREATED_AT = '2026-05-26T18:30:00.000Z'
const USED_AT = '2026-05-26T18:30:10.000Z'
...
const createPokemonSheet = (): Record<string, unknown> => ({
  slug: 'pikachu',
  nickname: 'Pikachu',
  species: '',
  level: 20,
  player: true,
  movelist: [{ name: 'Thunderbolt' }, { name: 'Hyper Beam' }],
})
...
const useMoveCommandMessage = (): SessionCommandMessage<UseMoveCommand> => ({
  schemaVersion: SESSION_MESSAGE_SCHEMA_VERSION,
  type: 'command',
  direction: 'client',
  sessionId: SESSION_ID,
  command: {
    ...
    metadata: {
      traceId: 'trace-websocket-use-move',
    },
  },
})

describe('useMove WebSocket dispatch', () => {
  it('acks the sender and broadcasts a small moveUsed patch to same-session clients only', () => {
    ...
    const sheet = createPokemonSheet()

    const gmPeer = makePeer('peer-umv-gm')
    const playerPeer = makePeer('peer-umv-player')
    const otherPeer = makePeer('peer-umv-other')
    const dependencies = {
      env: enabledEnv,
      registry,
      peers,
      store,
      useMoveCommandDependencies: {
        operationTracker: false as const,
        writeSnapshot,
        readSheet: () => ({
          path: '/tmp/pikachu.json',
          sheet: JSON.parse(JSON.stringify(sheet)) as Record<string, unknown>,
        }),
      },
    }
    ...
    handleSessionSocketMessage(gmPeer, { text: () => JSON.stringify(useMoveCommandMessage()) }, {
      ...dependencies,
      clock: () => USED_AT,
    })
    ...
    expect(JSON.stringify(playerPatch)).not.toContain('movelist')
    ...
    expect(registry.get('peer-umv-gm')).toMatchObject({
      status: SESSION_SOCKET_AUTHENTICATED_STATUS,
      currentRevision: parseSessionRevision(1),
    })
    expect(registry.get('peer-umv-player')).toMatchObject({
      status: SESSION_SOCKET_AUTHENTICATED_STATUS,
      currentRevision: parseSessionRevision(1),
    })
  })
