...
import {
  REG_024_MOVE_NAMES,
  REG_024_SCENARIOS_BY_MOVE,
  SCRATCH_REG_024_SCENARIOS,
  SCREECH_REG_024_SCENARIOS,
  SEARING_SHOT_REG_024_SCENARIOS,
  SEED_BOMB_REG_024_SCENARIOS,
  SEED_FLARE_REG_024_SCENARIOS,
  SHADOW_BALL_REG_024_SCENARIOS,
  SHADOW_BONE_REG_024_SCENARIOS,
  SHADOW_CLAW_REG_024_SCENARIOS,
  type RegisteredBatch024MoveName,
  type RegisteredMoveConformanceScenario,
} from '../fixtures/moveAutomation/registeredBatch024'
import { scratchV2PassHitSemanticScenario } from '../fixtures/moveAutomation/scratchV2'
...
const LEGACY_MOVE_NAMES = Object.freeze([
  ...
  'Shadow Bone',
  'Shadow Claw',
] as const satisfies readonly Exclude<RegisteredBatch024MoveName, 'Scratch'>[])

type TargetId = typeof TARGET_A_ID | typeof TARGET_B_ID | typeof TARGET_C_ID
type SelectionKind = 'single-target' | 'burst' | 'ranged-blast' | 'pass'
...
interface ExecutionScenario {
  ...
  readonly moveName: RegisteredBatch024MoveName
...
const fixtureFor = (scenario: ExecutionScenario): MoveFixture => {
  ...
  const map: TabletopMap = {
    schemaVersion: 2,
    slug: `reg-024-${scenario.scenarioId.replace(/[^a-z0-9]+/gi, '-').toLowerCase()}`,
    name: `REG-024 ${scenario.moveName}`,
    revision: 7,
    ...
    activeScene: { name: 'REG-024 scene', startedAt: 100 },
    encounterState: {
      ...emptyState,
      sides: {
...
const accuracyNaturalResults = (
  ...
  .filter(entry => (
    entry.formula.kind === 'dice'
    && entry.formula.count === 1
    && entry.formula.sides === 20
    && entry.reason.toLowerCase().includes('accuracy')
  ))
...
const assertScenarioResolution = (
  ...
  if (selectionKind === 'single-target') {
    ...
    if (selectionKind === 'ranged-blast') expect(resolution.area?.aimCell).toEqual(BLAST_AIM_CELL)
    if (selectionKind === 'pass') expect(resolution.area?.direction).toBe('east')
  }
...
const thresholdStageScenarios = (options: {
  readonly moveName: 'Shadow Ball' | 'Shadow Bone'
  readonly stageKey: 'def' | 'sdef'
...
}): readonly ExecutionScenario[] => [
  {
    ...
    expectedStages: [{ recipientId: TARGET_A_ID, key: options.stageKey, value: -1 }],
  ...
  },
  {
    ...
    moveName: options.moveName,
    initialStages: [{ recipientId: TARGET_A_ID, key: options.stageKey, value: -6 }],
    randomValues: [0.8, 0, 0],
    expectedStages: [{ recipientId: TARGET_A_ID, key: options.stageKey, value: -6 }],
    expectedAttackedTargetIds: [TARGET_A_ID],
    expectedHitTargetIds: [TARGET_A_ID],
    expectedDamagedTargetIds: [TARGET_A_ID],
    expectedAccuracyNaturalResults: [17],
...
const normalScenarios: readonly ExecutionScenario[] = [
  {
    scenarioId: SCRATCH_REG_024_SCENARIOS[1].scenarioId,
    moveName: 'Scratch',
  ...
  {
    scenarioId: SCRATCH_REG_024_SCENARIOS[2].scenarioId,
    moveName: 'Scratch',
    runtimeKind: 'movespec-v2',
    selectionKind: 'pass',
  ...
  {
    scenarioId: SCRATCH_REG_024_SCENARIOS[3].scenarioId,
    moveName: 'Scratch',
    runtimeKind: 'movespec-v2',
    selectionKind: 'pass',
    ...
    expectedMovement: PASS_MOVEMENT,
    expectedLogFragments: ['Ghost type'],
  },
  {
    scenarioId: SCRATCH_REG_024_SCENARIOS[4].scenarioId,
    moveName: 'Scratch',
    runtimeKind: 'movespec-v2',
    selectionKind: 'pass',
    targetIds: [],
  ...
  {
    scenarioId: SCREECH_REG_024_SCENARIOS[0].scenarioId,
    moveName: 'Screech',
    selectionKind: 'burst',
    targetIds: [TARGET_A_ID, TARGET_B_ID, TARGET_C_ID],
    excludedTargetIds: [TARGET_C_ID],
    randomValues: [0.45, 0],
    expectedStages: [{ recipientId: TARGET_A_ID, key: 'def', value: -2 }],
    expectedAttackedTargetIds: [TARGET_A_ID, TARGET_B_ID],
    expectedHitTargetIds: [TARGET_A_ID],
    expectedDamagedTargetIds: [],
    expectedAccuracyNaturalResults: [10, 1],
  ...
  {
    scenarioId: SCREECH_REG_024_SCENARIOS[1].scenarioId,
    moveName: 'Screech',
    selectionKind: 'burst',
  ...
  {
    scenarioId: SCREECH_REG_024_SCENARIOS[2].scenarioId,
    moveName: 'Screech',
    selectionKind: 'burst',
  ...
  {
    scenarioId: SEARING_SHOT_REG_024_SCENARIOS[0].scenarioId,
  ...
  {
    scenarioId: SEARING_SHOT_REG_024_SCENARIOS[1].scenarioId,
    moveName: 'Searing Shot',
    selectionKind: 'burst',
  ...
  {
    scenarioId: SEARING_SHOT_REG_024_SCENARIOS[2].scenarioId,
    moveName: 'Searing Shot',
    selectionKind: 'burst',
  ...
  {
    scenarioId: SEARING_SHOT_REG_024_SCENARIOS[3].scenarioId,
    moveName: 'Searing Shot',
    selectionKind: 'burst',
  ...
  {
    scenarioId: SEARING_SHOT_REG_024_SCENARIOS[4].scenarioId,
    moveName: 'Searing Shot',
    selectionKind: 'burst',
    targetProfiles: { [TARGET_A_ID]: { abilities: ['Shield Dust'] } },
    randomValues: [0.7, 0, 0, 0],
  ...
  {
    scenarioId: SEED_BOMB_REG_024_SCENARIOS[0].scenarioId,
    moveName: 'Seed Bomb',
    randomValues: [0.45, 0, 0],
    expectedAttackedTargetIds: [TARGET_A_ID],
    expectedHitTargetIds: [TARGET_A_ID],
  ...
  {
    scenarioId: SEED_BOMB_REG_024_SCENARIOS[1].scenarioId,
    moveName: 'Seed Bomb',
    randomValues: [0],
    expectedAttackedTargetIds: [TARGET_A_ID],
    expectedHitTargetIds: [],
  ...
  {
    scenarioId: SEED_BOMB_REG_024_SCENARIOS[2].scenarioId,
    moveName: 'Seed Bomb',
    randomValues: [0.999, 0, 0, 0, 0],
    expectedAttackedTargetIds: [TARGET_A_ID],
    expectedHitTargetIds: [TARGET_A_ID],
  ...
  {
    scenarioId: SEED_BOMB_REG_024_SCENARIOS[3].scenarioId,
    moveName: 'Seed Bomb',
  ...
  {
    scenarioId: SEED_FLARE_REG_024_SCENARIOS[0].scenarioId,
    moveName: 'Seed Flare',
    selectionKind: 'ranged-blast',
    targetIds: [TARGET_A_ID, TARGET_B_ID],
  ...
  {
    scenarioId: SEED_FLARE_REG_024_SCENARIOS[1].scenarioId,
    moveName: 'Seed Flare',
    selectionKind: 'ranged-blast',
    randomValues: [0.999, 0, 0, 0, 0, 0, 0],
    expectedStages: [{ recipientId: TARGET_A_ID, key: 'sdef', value: -1 }],
  ...
  {
    scenarioId: SEED_FLARE_REG_024_SCENARIOS[2].scenarioId,
    moveName: 'Seed Flare',
    selectionKind: 'ranged-blast',
    targetProfiles: { [TARGET_A_ID]: { abilities: ['Sap Sipper'] } },
    randomValues: [0.45],
  ...
  {
    scenarioId: SEED_FLARE_REG_024_SCENARIOS[3].scenarioId,
    moveName: 'Seed Flare',
    selectionKind: 'ranged-blast',
  ...
  ...thresholdStageScenarios({
    moveName: 'Shadow Ball',
    stageKey: 'sdef',
    evidence: SHADOW_BALL_REG_024_SCENARIOS,
  }),
  ...thresholdStageScenarios({
    moveName: 'Shadow Bone',
    stageKey: 'def',
    evidence: SHADOW_BONE_REG_024_SCENARIOS,
  }),
  {
    scenarioId: SHADOW_CLAW_REG_024_SCENARIOS[0].scenarioId,
  ...
  {
    scenarioId: SHADOW_CLAW_REG_024_SCENARIOS[1].scenarioId,
    moveName: 'Shadow Claw',
    selectionKind: 'pass',
  ...
  {
    scenarioId: SHADOW_CLAW_REG_024_SCENARIOS[2].scenarioId,
    moveName: 'Shadow Claw',
    selectionKind: 'pass',
    targetProfiles: { [TARGET_A_ID]: { types: ['Normal'] } },
    randomValues: [0.45],
  ...
  {
    scenarioId: SHADOW_CLAW_REG_024_SCENARIOS[3].scenarioId,
    moveName: 'Shadow Claw',
    selectionKind: 'pass',
    targetIds: [],
    randomValues: [],
...
const recoveryScenarioFor = (moveName: RegisteredBatch024MoveName): ExecutionScenario => {
  const matching = normalScenarios.find(scenario => (
    scenario.moveName === moveName
    && scenario.expectedHitTargetIds.includes(TARGET_A_ID)
    && (scenario.expectedDamagedTargetIds.includes(TARGET_A_ID) || moveName === 'Screech')
  ))
...
describe('REG-024 registered move conformance', () => {
  it('certifies exactly Scratch through Shadow Claw with linked evidence', () => {
    expect(Object.keys(REG_024_SCENARIOS_BY_MOVE)).toEqual([...REG_024_MOVE_NAMES])
    expect(EXPLICIT_MOVE_AUTOMATION_SCRIPTS).toHaveLength(258)

    for (const [canonicalId, scenarios] of Object.entries(REG_024_SCENARIOS_BY_MOVE)) {
  ...
  it('retains every reviewed canonical mechanic without unresolved rule instructions', () => {
    const expected = {
      Scratch: { ac: 2, damageBase: 4, damageClass: 'Physical', range: 'Melee, Pass' },
      Screech: { ac: 4, damageBase: 0, damageClass: 'Status', range: 'Burst 2, Friendly, Sonic' },
      'Searing Shot': { ac: 2, damageBase: 10, damageClass: 'Special', range: 'Burst 1' },
      'Seed Bomb': { ac: 2, damageBase: 8, damageClass: 'Physical', range: '8, 1 Target' },
      'Seed Flare': { ac: 5, damageBase: 12, damageClass: 'Special', range: '6, Ranged Blast 3' },
      'Shadow Ball': { ac: 2, damageBase: 8, damageClass: 'Special', range: '8, 1 Target' },
      'Shadow Bone': { ac: 2, damageBase: 9, damageClass: 'Physical', range: 'Melee, 1 Target' },
      'Shadow Claw': { ac: 2, damageBase: 7, damageClass: 'Physical', range: 'Melee, Pass', criticalRange: 18 },
    } as const
    ...
    expect(explicitScriptForMove('Scratch')?.areaTemplates).toEqual([
      { kind: 'pass', size: 4, label: 'Pass 4' },
    ])
    expect(explicitScriptForMove('Screech')).toMatchObject({
      keywords: expect.arrayContaining(['Friendly', 'Sonic']),
      areaTemplates: [{ kind: 'burst', size: 2, label: 'Burst 2' }],
      stageSuggestions: [{ recipient: 'target', key: 'def', delta: -2 }],
    })
    expect(explicitScriptForMove('Searing Shot')).toMatchObject({
    ...
    expect(explicitScriptForMove('Seed Bomb')).toMatchObject({
      conditionSuggestions: [],
      stageSuggestions: [],
    })
    expect(explicitScriptForMove('Seed Flare')).toMatchObject({
      areaTemplates: [{ kind: 'ranged-blast', size: 3, range: 6 }],
      stageSuggestions: [{ recipient: 'target', key: 'sdef', delta: -1 }],
    })
    expect(explicitScriptForMove('Shadow Ball')?.stageSuggestions).toEqual([{
      recipient: 'target',
      key: 'sdef',
      delta: -1,
      label: 'Shadow Ball lowers Special Defense on 17+: -1 Special Defense CS',
      threshold: '17+',
      optional: true,
    }])
    expect(explicitScriptForMove('Shadow Bone')?.stageSuggestions).toEqual([{
      recipient: 'target',
      key: 'def',
      delta: -1,
      label: 'Shadow Bone lowers Defense on 17+: -1 Defense CS',
      threshold: '17+',
      optional: true,
    }])
    expect(explicitScriptForMove('Shadow Claw')).toMatchObject({
      criticalRange: 18,
      areaTemplates: [{ kind: 'pass', size: 4, label: 'Pass 4' }],
    })
  })

  it('retains the canonical Scratch semantic canary through interpreter, planner, and command', async () => {
    const result = await runAndAssertMoveAutomationSemanticScenario(
  ...
  it.each(REG_024_MOVE_NAMES)(
    ...
    async (moveName) => {
      ...
      const evidence = REG_024_SCENARIOS_BY_MOVE[moveName]
