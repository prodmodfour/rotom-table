...
import { POWER_TRIP_MOVE_SPEC } from '~~/server/domain/moveAutomation/specs/powerTrip'
...
import {
  POISON_STING_REG_020_SCENARIOS,
  POISON_TAIL_REG_020_SCENARIOS,
  POUND_REG_020_SCENARIOS,
  POWDER_SNOW_REG_020_SCENARIOS,
  POWER_GEM_REG_020_SCENARIOS,
  POWER_TRIP_REG_020_SCENARIOS,
  POWER_WHIP_REG_020_SCENARIOS,
  PRECIPICE_BLADES_REG_020_SCENARIOS,
  REG_020_MOVE_NAMES,
  REG_020_SCENARIOS_BY_MOVE,
  type RegisteredBatch020MoveName,
} from '../fixtures/moveAutomation/registeredBatch020'
import {
...
type SelectionKind = 'single-target' | 'line' | 'burst'
...
interface ExecutionScenario {
  ...
  readonly moveName: RegisteredBatch020MoveName
...
const pokemonSheet = (options: {
  ...
  species: options.slug === 'actor' ? 'Skorupi' : 'Clefairy',
  level: 20,
  revision: 3,
...
const targetPosition = (
  ...
  if (selectionKind === 'line') {
    ...
    if (id === TARGET_B_ID) return { x: 7, y: 0, z: 5 }
    return { x: 8, y: 0, z: 5 }
  }
  if (id === TARGET_A_ID) return { x: 6, y: 0, z: 5 }
...
const fixtureFor = (scenario: ExecutionScenario): MoveFixture => {
  ...
  const map: TabletopMap = {
    schemaVersion: 2,
    slug: `reg-020-${scenario.scenarioId.replace(/[^a-z0-9]+/gi, '-').toLowerCase()}`,
    name: `REG-020 ${scenario.moveName}`,
    revision: 7,
    ...
    activeScene: { name: 'REG-020 scene', startedAt: 100 },
    encounterState: {
      ...emptyState,
      sides: {
  ...
  if (selectionKind === 'single-target') {
    ...
    selection = {
      ...
      ...(selectionKind === 'line' ? { direction: 'east' as const } : {}),
    }
  }
...
const assertScenarioResolution = (
  ...
  expect(resolution.auditTrace.program).toMatchObject({
    ...
    runtimeKind: runtimeKindFor(scenario.moveName),
...
const executeCommand = async (
  ...
  idFactory: (() => {
    let sequence = 0
    return () => `reg-020-id-${++sequence}`
  })(),
  relativePath: path => path,
})

const poisoned = { [TARGET_A_ID]: ['Poisoned'] } as const
const frozen = { [TARGET_A_ID]: ['Frozen'] } as const

const normalScenarios: readonly ExecutionScenario[] = [
  {
    scenarioId: POISON_STING_REG_020_SCENARIOS[0].scenarioId,
  ...
  {
    scenarioId: POISON_STING_REG_020_SCENARIOS[1].scenarioId,
    moveName: 'Poison Sting',
    randomValues: [0.75, 0],
    expectedAttackedTargetIds: [TARGET_A_ID],
    expectedHitTargetIds: [TARGET_A_ID],
  ...
  {
    scenarioId: POISON_STING_REG_020_SCENARIOS[2].scenarioId,
    moveName: 'Poison Sting',
    randomValues: [0],
    expectedAttackedTargetIds: [TARGET_A_ID],
  ...
  {
    scenarioId: POISON_STING_REG_020_SCENARIOS[3].scenarioId,
    moveName: 'Poison Sting',
  ...
  {
    scenarioId: POISON_STING_REG_020_SCENARIOS[4].scenarioId,
    moveName: 'Poison Sting',
  ...
  {
    scenarioId: POISON_STING_REG_020_SCENARIOS[5].scenarioId,
    moveName: 'Poison Sting',
  ...
  {
    scenarioId: POISON_STING_REG_020_SCENARIOS[6].scenarioId,
    moveName: 'Poison Sting',
  ...
  {
    scenarioId: POISON_TAIL_REG_020_SCENARIOS[0].scenarioId,
  ...
  {
    scenarioId: POISON_TAIL_REG_020_SCENARIOS[1].scenarioId,
    moveName: 'Poison Tail',
    randomValues: [0.8, 0],
    expectedAttackedTargetIds: [TARGET_A_ID],
    expectedHitTargetIds: [TARGET_A_ID],
  ...
  {
    scenarioId: POISON_TAIL_REG_020_SCENARIOS[2].scenarioId,
    moveName: 'Poison Tail',
    randomValues: [0],
    expectedAttackedTargetIds: [TARGET_A_ID],
    expectedHitTargetIds: [],
  ...
  {
    scenarioId: POISON_TAIL_REG_020_SCENARIOS[3].scenarioId,
    moveName: 'Poison Tail',
  ...
  {
    scenarioId: POISON_TAIL_REG_020_SCENARIOS[4].scenarioId,
    moveName: 'Poison Tail',
    targetProfiles: { [TARGET_A_ID]: { types: ['Steel'] } },
    randomValues: [0.9],
    expectedAttackedTargetIds: [TARGET_A_ID],
  ...
  {
    scenarioId: POISON_TAIL_REG_020_SCENARIOS[5].scenarioId,
    moveName: 'Poison Tail',
    targetProfiles: { [TARGET_A_ID]: { types: ['Poison'] } },
    randomValues: [0.9, 0, 0],
    expectedAttackedTargetIds: [TARGET_A_ID],
  ...
  {
    scenarioId: POISON_TAIL_REG_020_SCENARIOS[6].scenarioId,
    moveName: 'Poison Tail',
    targetProfiles: { [TARGET_A_ID]: { abilities: ['Shield Dust'] } },
    randomValues: [0.9, 0, 0],
    expectedAttackedTargetIds: [TARGET_A_ID],
  ...
  {
    scenarioId: POUND_REG_020_SCENARIOS[0].scenarioId,
    moveName: 'Pound',
    randomValues: [0.45, 0],
    expectedAttackedTargetIds: [TARGET_A_ID],
    expectedHitTargetIds: [TARGET_A_ID],
  ...
  {
    scenarioId: POUND_REG_020_SCENARIOS[1].scenarioId,
    moveName: 'Pound',
    randomValues: [0],
    expectedAttackedTargetIds: [TARGET_A_ID],
    expectedHitTargetIds: [],
  ...
  {
    scenarioId: POUND_REG_020_SCENARIOS[2].scenarioId,
    moveName: 'Pound',
    randomValues: [0.999, 0, 0],
    expectedAttackedTargetIds: [TARGET_A_ID],
    expectedHitTargetIds: [TARGET_A_ID],
  ...
  {
    scenarioId: POUND_REG_020_SCENARIOS[3].scenarioId,
    moveName: 'Pound',
  ...
  {
    scenarioId: POWDER_SNOW_REG_020_SCENARIOS[0].scenarioId,
  ...
  {
    scenarioId: POWDER_SNOW_REG_020_SCENARIOS[1].scenarioId,
    moveName: 'Powder Snow',
    selectionKind: 'line',
    randomValues: [0.85, 0],
    expectedAttackedTargetIds: [TARGET_A_ID],
  ...
  {
    scenarioId: POWDER_SNOW_REG_020_SCENARIOS[2].scenarioId,
    moveName: 'Powder Snow',
    selectionKind: 'line',
    randomValues: [0.999, 0, 0],
    expectedConditions: frozen,
  ...
  {
    scenarioId: POWDER_SNOW_REG_020_SCENARIOS[3].scenarioId,
    moveName: 'Powder Snow',
    selectionKind: 'line',
    targetProfiles: { [TARGET_A_ID]: { types: ['Ice'] } },
    randomValues: [0.9, 0],
    expectedAttackedTargetIds: [TARGET_A_ID],
    expectedHitTargetIds: [TARGET_A_ID],
    expectedDamagedTargetIds: [TARGET_A_ID],
  ...
  {
    scenarioId: POWDER_SNOW_REG_020_SCENARIOS[4].scenarioId,
    moveName: 'Powder Snow',
    selectionKind: 'line',
    targetProfiles: { [TARGET_A_ID]: { abilities: ['Shield Dust'] } },
    randomValues: [0.9, 0],
  ...
  {
    scenarioId: POWER_GEM_REG_020_SCENARIOS[0].scenarioId,
    moveName: 'Power Gem',
  ...
  {
    scenarioId: POWER_GEM_REG_020_SCENARIOS[1].scenarioId,
    moveName: 'Power Gem',
    randomValues: [0],
    expectedAttackedTargetIds: [TARGET_A_ID],
    expectedHitTargetIds: [],
  ...
  {
    scenarioId: POWER_GEM_REG_020_SCENARIOS[2].scenarioId,
    moveName: 'Power Gem',
  ...
  {
    scenarioId: POWER_TRIP_REG_020_SCENARIOS[5].scenarioId,
    moveName: 'Power Trip',
    randomValues: [0.45, 0],
    expectedAttackedTargetIds: [TARGET_A_ID],
    expectedHitTargetIds: [TARGET_A_ID],
  ...
  {
    scenarioId: POWER_TRIP_REG_020_SCENARIOS[3].scenarioId,
    moveName: 'Power Trip',
    randomValues: [0],
    expectedAttackedTargetIds: [TARGET_A_ID],
    expectedHitTargetIds: [],
  ...
  {
    scenarioId: POWER_TRIP_REG_020_SCENARIOS[4].scenarioId,
    moveName: 'Power Trip',
    randomValues: [0.999, 0, 0],
    expectedAttackedTargetIds: [TARGET_A_ID],
    expectedHitTargetIds: [TARGET_A_ID],
  ...
  {
    scenarioId: POWER_WHIP_REG_020_SCENARIOS[0].scenarioId,
    moveName: 'Power Whip',
  ...
  {
    scenarioId: POWER_WHIP_REG_020_SCENARIOS[1].scenarioId,
    moveName: 'Power Whip',
  ...
  {
    scenarioId: POWER_WHIP_REG_020_SCENARIOS[2].scenarioId,
    moveName: 'Power Whip',
  ...
  {
    scenarioId: POWER_WHIP_REG_020_SCENARIOS[3].scenarioId,
    moveName: 'Power Whip',
  ...
  {
    scenarioId: PRECIPICE_BLADES_REG_020_SCENARIOS[0].scenarioId,
  ...
  {
    scenarioId: PRECIPICE_BLADES_REG_020_SCENARIOS[1].scenarioId,
    moveName: 'Precipice Blades',
    selectionKind: 'burst',
    randomValues: [0.999, 0, 0, 0, 0, 0, 0],
    expectedAttackedTargetIds: [TARGET_A_ID],
...
const recoveryScenarioFor = (moveName: RegisteredBatch020MoveName): ExecutionScenario => {
...
describe('REG-020 registered move conformance', () => {
  it('certifies exactly Poison Sting through Precipice Blades with linked evidence', () => {
    expect(Object.keys(REG_020_SCENARIOS_BY_MOVE)).toEqual([...REG_020_MOVE_NAMES])

    for (const [canonicalId, scenarios] of Object.entries(REG_020_SCENARIOS_BY_MOVE)) {
  ...
  it('retains every reviewed canonical mechanic without unresolved rule instructions', () => {
    const expected = {
      'Poison Sting': { version: 2, ac: 2, damageBase: 2, damageClass: 'Physical', range: '6, 1 Target' },
      'Poison Tail': { version: 1, ac: 2, damageBase: 5, damageClass: 'Physical', range: 'Melee, 1 Target', criticalRange: 18 },
      Pound: { version: 1, ac: 2, damageBase: 4, damageClass: 'Physical', range: 'Melee, 1 Target' },
      'Powder Snow': { version: 2, ac: 2, damageBase: 4, damageClass: 'Special', range: 'Line 4' },
      'Power Gem': { version: 1, ac: 2, damageBase: 8, damageClass: 'Special', range: '6, 1 Target' },
      'Power Trip': { version: 1, ac: 2, damageBase: 2, damageClass: 'Physical', range: 'Melee, 1 Target' },
      'Power Whip': { version: 1, ac: 5, damageBase: 12, damageClass: 'Physical', range: '8, 1 Target, Smite' },
      'Precipice Blades': { version: 1, ac: 5, damageBase: 12, damageClass: 'Physical', range: 'Burst 1, Smite' },
    } as const
    ...
    expect(explicitScriptForMove('Poison Sting')?.conditionSuggestions).toEqual([{
      recipient: 'target',
      condition: 'Poisoned',
      action: 'add',
      label: 'Poisoned on 17+',
      threshold: '17+',
      optional: true,
    }])
    expect(explicitScriptForMove('Poison Tail')).toMatchObject({
      criticalRange: 18,
      conditionSuggestions: [{
        recipient: 'target',
        condition: 'Poisoned',
        action: 'add',
        label: 'Poisoned on 19+',
        threshold: '19+',
        optional: true,
      }],
    })
    expect(explicitScriptForMove('Powder Snow')).toMatchObject({
      special: 'Grants Freezer',
    ...
    expect(POWER_TRIP_MOVE_SPEC.phases[1]?.operations[0]).toMatchObject({
    ...
    expect(explicitScriptForMove('Power Whip')).toMatchObject({
      special: 'Grants Threaded',
      keywords: expect.arrayContaining(['Smite']),
    })
    expect(explicitScriptForMove('Precipice Blades')).toMatchObject({
      keywords: expect.arrayContaining(['Smite']),
  ...
  it.each(allPowerTripV2SemanticScenarios())(
    'retains dynamic-damage evidence for $scenarioId at all authority layers',
    async (scenario) => {
      expect(POWER_TRIP_V2_SEMANTIC_SCENARIOS.map(entry => entry.scenarioId))
      ...
      expect(result.plan.value?.resolution.auditTrace.program).toMatchObject({
  ...
  it.each(normalScenarios)(
    ...
    async (scenario) => {
      ...
      expect(response.move?.trace).toMatchObject({
        program: { canonicalId: scenario.moveName, runtimeKind: runtimeKindFor(scenario.moveName) },
      })
  ...
  it.each(REG_020_MOVE_NAMES)(
    ...
    async (moveName) => {
      ...
      const evidence = REG_020_SCENARIOS_BY_MOVE[moveName]
