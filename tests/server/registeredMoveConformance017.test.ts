...
describe('REG-017 registered move conformance', () => {
  ...
  it('applies Mud Sport resistance and consumes the coat once after an Electric hit', async () => {
    ...
    expect(response.move?.transaction.logLines.join('\n')).toContain('× ½')
    expect(response.move?.transaction.logLines.join('\n'))
      .toContain('Electric-Resistant Coat removed after a damaging Electric-Type move hit.')
