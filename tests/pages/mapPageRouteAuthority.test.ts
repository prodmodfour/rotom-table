...
describe('map page route authority', () => {
  it('uses the saved map route regardless of the session query parameter', () => {
    ...
    expect(mapPage).toContain('const runtimeSheetReloadContext = () => (')
    expect(mapPage).toContain('const reconciliationSheetReloadContext = () => ({\n  ...runtimeSheetReloadContext(),\n  throwOnError: true,\n})')
    expect(mapPage).toContain('const reconcileLivePlayState = async () => {\n  await Promise.all([\n    reloadAuthoritativeMap(),\n    reloadRuntimeSheets(reconciliationSheetReloadContext()),\n  ])\n}')
