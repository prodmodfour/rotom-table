...
describe('map page route authority', () => {
  it('uses the saved map route regardless of the session query parameter', () => {
    ...
    expect(mapPage).toContain('const setupEditModeActive = computed(() => isGm.value && adminPanelOpen.value)')
    expect(mapPage).toContain('documentTokenActions.buildTerrainVoxel({ voxel })')
    expect(mapPage).toContain('documentTokenActions.removeTerrainVoxel({ cell })')
