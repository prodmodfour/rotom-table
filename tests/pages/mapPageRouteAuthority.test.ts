...
describe('map page route authority', () => {
  it('wires clicked live-play movement through local prediction feedback', () => {
    ...
    expect(mapPage).toContain('const transientLivePlayCorrectionMotionTokenIds = ref<string[]>([])')
    expect(mapPage).toContain('const transientLivePlaySnapCorrectionTokenIds = ref<string[]>([])')
    expect(mapPage).toContain('const transientLivePlayRemoteAcceptedMotionTokenIds = ref<string[]>([])')
    ...
    expect(mapPage).toContain("if (transition.reason === 'stale-revision') {\n    markLivePlaySnapCorrectionTokens([predictedToken.placementId])")
  ...
  it('wires token-level live-play prediction affordances into the map renderer', () => {
    ...
    expect(mapPage).toContain(':live-play-correction-motion-token-ids="livePlayCorrectionMotionTokenIds"')
    expect(mapPage).toContain(':live-play-snap-correction-token-ids="transientLivePlaySnapCorrectionTokenIds"')
    expect(mapPage).toContain(':live-play-remote-accepted-token-ids="livePlayRemoteAcceptedMotionTokenIds"')
    expect(mapPage).toContain(':map-data-revision="mapDataRevision"')
    ...
    expect(scenePanel).toContain(':live-play-correction-motion-token-ids="livePlayCorrectionMotionTokenIds ?? []"')
    expect(scenePanel).toContain(':live-play-snap-correction-token-ids="livePlaySnapCorrectionTokenIds ?? []"')
    expect(scenePanel).toContain(':live-play-remote-accepted-token-ids="livePlayRemoteAcceptedTokenIds ?? []"')
    expect(scenePanel).toContain(':map-data-revision="mapDataRevision ?? 0"')
    ...
    expect(sceneRenderer).toContain(':live-play-correction-motion-token-ids="livePlayCorrectionMotionTokenIds ?? []"')
    expect(sceneRenderer).toContain(':live-play-snap-correction-token-ids="livePlaySnapCorrectionTokenIds ?? []"')
    expect(sceneRenderer).toContain(':live-play-remote-accepted-token-ids="livePlayRemoteAcceptedTokenIds ?? []"')
    expect(sceneRenderer).toContain(':map-data-revision="mapDataRevision ?? 0"')
    expect(isometricGrid).toContain('const livePlayPendingTokenIdSet = computed(() => new Set(props.livePlayPendingTokenIds ?? []))')
    expect(isometricGrid).toContain('const livePlayCorrectionMotionTokenIdSet = computed(() => new Set(props.livePlayCorrectionMotionTokenIds ?? []))')
    expect(isometricGrid).toContain('const livePlaySnapCorrectionTokenIdSet = computed(() => new Set(props.livePlaySnapCorrectionTokenIds ?? []))')
    expect(isometricGrid).toContain('const livePlayRemoteAcceptedTokenIdSet = computed(() => new Set(props.livePlayRemoteAcceptedTokenIds ?? []))')
    expect(isometricGrid).toContain("motionMode === 'snap'\n        ? 'reconciliation'\n        : placementMotionReasonForPokemon(pokemon)")
    expect(isometricGrid).toContain('resolveTokenMotionDurationOptionsForReason(reason, { reducedMotion: true })')
    ...
    expect(isometricGrid).toContain('remoteAccepted: livePlayRemoteAcceptedTokenIdSet.value.has(pokemon.id)')
    expect(mapPage).toContain("if (!adoption.applied || adoption.origin !== 'remote-accepted') return []")
    expect(mapPage).toContain('if (patch.type !== LIVE_PLAY_PATCH_TYPES.TOKEN_POSITION) return []')
    expect(mapPage).toContain('const visiblePlacementIds = visiblePresenceTokenIdSet.value')
  ...
  it('wires GM-only move operation details to ID-only correction commands', () => {
    const mapPage = readSource('src/pages/maps/[slug].vue')
    const scenePanel = readSource('src/components/map/MapScenePanel.vue')
    const combatLog = readSource('src/components/map/MapCombatLog.vue')
    const correctionPanel = readSource('src/components/map/MapMoveCorrectionPanel.vue')
    const correctionClient = readSource('src/composables/map-editor/useGmMoveCorrections.ts')

    expect(mapPage).toContain('const gmMoveCorrections = useGmMoveCorrections({')
    expect(mapPage).toContain('enabled: isGm')
    expect(mapPage).toContain(':can-manage-move-corrections="isGm"')
    ...
    expect(correctionClient).toContain('payload: {\n        originOperationId: source.originOperationId,\n        operationIds: uniqueOperationIds,')
    expect(correctionClient).not.toContain('expectedCurrent')
    expect(correctionClient).not.toContain('restore:')
  })
})
