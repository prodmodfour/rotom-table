...
describe('map page live-play command recovery integration', () => {
  it('wires context-scoped startup recovery into the new-command gate and displayed readiness', () => {
    ...
    expect(mapPage).toContain('const livePlayActiveBatchPendingLabel = computed(() => buildLivePlayBatchPendingLabel(')
    expect(mapPage).toContain('hazardCount: hazardCount.value')
    expect(mapPage).toContain('fieldEffectCount: fieldEffectCount.value')
  ...
  it('wires accepted realtime command events through safe acknowledgement indirection', () => {
    ...
    expect(mapPage).toContain('movePresentationFromAcceptedRealtimeEvent(event)')
    expect(mapPage).toContain('useAcceptedMovePresentation({')
    expect(mapPage).toContain('onAcceptedMovePresentation: ({ presentation, source }) => {')
    expect(mapPage).toContain("presentAcceptedMove(presentation, source === 'http')")
    expect(mapPage).toContain('presentationHandled: acceptedMoveWasPresented(result.opId)')
  ...
  it('renders the retry panel without adding discard or automatic resend controls', () => {
    ...
    expect(mapPage).toContain("import { buildLivePlayBatchPendingLabel } from '~/utils/livePlayBatchCommandUi'")
    expect(mapPage).toContain('livePlayActiveBatchPendingLabel.value ?? livePlayStateMachine.notice.value')
    expect(mapPage).toContain("livePlayActiveBatchPendingLabel.value ?? 'Sending live-play command to the server.'")
    expect(panel).toContain("import { buildLivePlayBatchCommandSummary } from '~/utils/livePlayBatchCommandUi'")
    expect(panel).toContain('batchRecoveryLabel(entry)')
    expect(panel).toContain('is waiting for live-play recovery')
