...
describe('map page Poké Ball capture result modal sync', () => {
  it('uses transient map-action pokeball-result events to drive the remote result modal', () => {
    ...
    expect(mapPage).toContain('v-if="displayedPokeballCaptureResult && legacyMapFirstUi"')
    expect(mapPage).toContain("const legacyMapFirstUi = computed(() => import.meta.dev && route.query.legacyMapFirst === '1')")
