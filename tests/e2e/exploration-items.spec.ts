...
test('exploration items converge through Trainer activity, route generation, and inert Dowsing UI', async ({ page }, testInfo) => {
  ...
  const routeWard = page.getByRole('region', { name: 'Route ward' })
  await expect(routeWard).toBeVisible()
  await expect(routeWard).toContainText('Route ward')
  await expect(routeWard).toContainText('Repel')
  await expect(routeWard).toContainText('Level 15 or lower')
  await expect(routeWard).not.toContainText('canonicalDefinitionSha256')
  await expect(routeWard).not.toContainText('sourceInstanceId')
  expect((await routeWard.boundingBox())?.width).toBeLessThanOrEqual(await page.evaluate(() => window.innerWidth))
  expect((await new AxeBuilder({ page }).include('.route-repel-context').analyze()).violations).toEqual([])
