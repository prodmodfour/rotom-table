...
describe('live play move-resolution intent parsing', () => {
  ...
  it('rejects roll, script, runtime, transaction, resolved area and target-count shaped client authority fields', () => {
    ...
    expect(expectInvalidCodes(baseIntent({
      ...
      hazardCells: [{ x: 1, y: 0, z: 0 }],
      cellOptions: [{ id: 'client-cell', cell: { x: 1, y: 0, z: 0 } }],
      selectedOptionIds: ['client-cell'],
      constraints: { range: 99, connectedness: 'client-decides' },
      origin: { x: 0, y: 0, z: 0 },
      adjacency: 'client-decides',
      connectedness: 'client-decides',
      occupancy: 'client-decides',
      geometry: { kind: 'client-cells' },
