...
describe('useSessionLobby', () => {
  ...
  it('surfaces map-token helper precondition failures before sending assignment requests', async () => {
    ...
    await expect(lobby.assignSessionMapTokenToPlayer({
      playerId: PLAYER_ID,
      tokenId: ' ',
    })).rejects.toThrow('Choose a token on an available live session map before assigning player token control.')

    expect(api.calls).toEqual([])
    expect(lobby.lastUpdatedPlayerAssignment.value).toBeNull()
    expect(lobby.lastNotice.value).toBeNull()
    expect(lobby.lastError.value).toBe('Choose a token on an available live session map before assigning player token control.')
  })
  ...
  it('requires a remembered GM live session before GM-only session token actions', async () => {
    const storage = makeStorage()
    const api = makeApiClient({})
    const lobby = useSessionLobby({
      apiClient: api.apiClient,
