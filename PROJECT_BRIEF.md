...
Rotom Table — Hover/floating sprite centering.
...
Implement the refreshed work described by `BUILD_TICKETS.md` (`001` through `015`), generated from the supplied ticket planning file before that handoff file was removed.

Implement the work described by `BUILD_TICKETS.md` (`001` through `015`).
...
- `BUILD_TICKETS.md` is the authoritative local autonomous queue for Hover/floating sprite centering.
...
- Do not update ticket statuses beyond the selected ticket. The only exception is the final ticket `015`, which may set `AUTOMATION_STATUS: DONE` after all 15 refreshed tickets are complete and the final quality gate passes.
