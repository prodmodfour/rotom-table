...
## Next phase update: sheet storage path/folder split

- Split sheet filesystem responsibilities into focused server utilities:
  - `server/utils/sheetPaths.ts` now owns sheet-kind config, roots, root lists, and path-derived folder calculation.
  - `server/utils/sheetFolderStorage.ts` now owns sheet folder list/create/move/delete filesystem operations.
- Reduced `server/utils/sheetStorage.ts` to sheet-file CRUD/default-sheet concerns and updated sheet-folder use cases to depend on the focused folder-storage module.
- Preserved existing sheet roots, default slug bases, folder sanitization behavior, folder listing/mutation response shapes, derived-folder semantics, and sheet-file CRUD behavior.
- Added `tests/server/sheetPaths.test.ts` covering stable sheet roots/config, root lists, and path-derived folder labels.
...
- Quality gates after this phase:
  - `npm test -- tests/server/sheetPaths.test.ts tests/server/sheetStorage.test.ts tests/server/sheetFolders.test.ts` — passes: 3 test files / 11 tests.
  - `npm run typecheck` — passes.
...
## Next phase update: isometric block texture pattern helper split

- Extracted generated block-texture pattern selection, Canvas pixel-paint routines, side-depth overlays, and generated face borders from `utils/isometric/blockTextures.ts` into `utils/isometric/blockTexturePatterns.ts`.
- Kept `blockTextures.ts` focused on Three.js texture loading/caching, clear-water texture-pack handling, canvas creation, texture configuration, and material application.
- Preserved generated terrain visuals by delegating the same material-tag/role painting branches and overlay rules through the focused pattern helper.
- Added `tests/utils/isometric/blockTexturePatterns.test.ts` covering material-tag-to-pattern resolution, special hazard/tech/custom precedence, and side-depth overlay scale rules.
- Next remaining phase: continue one bounded cleanup pass, with candidates including another focused renderer/helper split, storage split, map-editor extraction, or small UI duplication cleanup; do not mark the full refactor complete yet.
- Quality gates after this phase:
...
## Next phase update: Pokédex capability search helper split

- Extracted movement and other-capability Pokédex search alias construction from `utils/pokedex/searchFieldValues.ts` into `utils/pokedex/searchCapabilityValues.ts`.
  - The new helper owns canonical movement capability field ordering, movement-capability base/minimum aliases, parenthetical stripping for other capabilities, and labelled other-capability minimum aliases.
- Updated `searchFieldValues.ts` to re-export `buildCapabilitySearchValues` for compatibility while keeping general field-value helpers focused on identity, ability, breeding, skill, stat, and size values.
- Preserved existing Pokédex filtering behavior, field-specific capability search text output, capability minimum threshold aliases, compact aliases, and public Pokédex route behavior.
- Added `tests/utils/pokedex/searchCapabilityValues.test.ts` covering canonical field order, absent/empty capability handling, movement aliases, and labelled other-capability aliases.
- Next remaining phase: continue one bounded cleanup pass, with candidates including another focused Pokédex/search helper split, renderer/helper split, storage split, map-editor extraction, remaining client/helper cleanup, or small UI duplication cleanup; do not mark the full refactor complete yet.
- Quality gates after this phase:
  - `npm test -- tests/utils/pokedex/searchCapabilityValues.test.ts tests/utils/pokedex/searchFieldValues.test.ts tests/utils/pokedex/searchText.test.ts` — passes: 3 test files / 11 tests.
  - `npm run typecheck` — passes.
  - `npm test` — passes: 174 test files / 669 tests.
  - `npm run build` — passes; existing large chunk warnings remain.
  - `npm run check:move-automation` — still fails with baseline `Explicit move automation coverage: 0/769` missing-script report.

## Next phase update: Pokédex search expression helper split

- Extracted Pokédex boolean search expression types, query normalization, tokenization, parsing, criterion construction, and expression matching from `utils/pokedex/searchQuery.ts` into `utils/pokedex/searchExpressions.ts`.
- Reduced `searchQuery.ts` to active field-filter combination while re-exporting the previous parser/matcher API for existing callers and tests.
- Preserved advanced Pokédex search behavior, including dash exclusions, implicit AND, OR/NOT grouping, compact aliases, field-filter operator combination, and route/import compatibility.
- Added `tests/utils/pokedex/searchExpressions.test.ts` covering normalization, tokenization, compact criteria, parsed expression shape, and expression matching.
- Next remaining phase: continue one small bounded cleanup pass, with candidates including another focused Pokédex/search helper split, renderer/helper split, storage split, map-editor extraction, remaining client/helper cleanup, or small UI duplication cleanup; do not mark the full refactor complete yet.
- Quality gates after this phase:
  - `npm test -- tests/utils/pokedex/searchExpressions.test.ts tests/utils/pokedex/searchQuery.test.ts` — passes: 2 test files / 9 tests.
  - `npm run typecheck` — passes.
...
## Next phase update: move automation map-effect helper split

- Extracted move-automation hazard and field-effect transaction payload/log construction from `utils/moveAutomationTransaction.ts` into `utils/moveAutomationMapEffects.ts`.
  - The new helper owns enabled hazard placement payloads, Toxic Spikes layer defaulting, legacy zero-square all-cell fallback, field-effect apply payloads, and compatible log-line construction.
- Reduced transaction assembly so it coordinates damage, HP/status accumulators, map-effect helper results, notes, and final transaction shape without owning map-effect payload construction inline.
- Preserved transaction response shapes, hazard owner/layer behavior, field-effect source values, suggestion enablement behavior, and move-automation dialog/apply compatibility.
- Added `tests/utils/moveAutomationMapEffects.test.ts` covering hazard payloads, zero-square fallback, disabled/empty hazards, and field-effect logs.
- Next remaining phase: continue one bounded cleanup pass, with candidates including another focused move-automation helper split, renderer/helper split, storage split, map-editor extraction, remaining client/helper cleanup, or small UI duplication cleanup; do not mark the full refactor complete yet.
- Quality gates after this phase:
...
## Next phase update: move automation condition suggestion helper split

- Extracted manual move-automation condition suggestion parsing from `utils/moveAutomationManual.ts` into `utils/moveAutomationConditionSuggestions.ts`.
  - The new helper owns status-clear detection, canonical condition normalization, threshold labelling, recipient/action inference, and optional/contextual condition flags.
- Reduced `moveAutomationManual.ts` so it focuses on manual fallback orchestration plus target/field/hazard/manual-note parsing while delegating condition rules to the focused helper.
- Preserved manual fallback script shapes, condition suggestion labels/actions/thresholds/options, and existing public `utils/moveAutomation` compatibility exports.
- Added `tests/utils/moveAutomationConditionSuggestions.test.ts` covering thresholded target conditions, Rest self-sleep routing, target/user clear-status suggestions, contextual optional flags, and removal windows.
- Next remaining phase: continue one bounded cleanup pass, with candidates including another focused move-automation helper split, renderer/helper split, storage split, map-editor extraction, remaining client/helper cleanup, or small UI duplication cleanup; do not mark the full refactor complete yet.
- Quality gates after this phase:
...
## Next phase update: encounter pokegen batch helper split

- Extracted sequential encounter `pokegen.sh` batch execution from `server/useCases/generateEncounters.ts` into `server/utils/pokegenBatch.ts`.
  - The new helper owns before-count capture, per-encounter file-diffing, preview content reads, failure/no-output reporting, and generated-file result shaping behind injected filesystem and runner boundaries.
- Reduced `generateEncountersUseCase` so it focuses on request normalization, table loading, rolling, output-directory selection, slug-prefix construction, result composition, and preview cleanup while delegating batch execution details.
- Preserved encounter generation response shape, sequential pokegen invocation order, persisted vs preview content behavior, beforeCount semantics, and failure messages for failed or no-output pokegen runs.
- Added `tests/server/pokegenBatch.test.ts` covering persisted batches, preview content inclusion, before-count behavior, failed runs, no-output runs, and continued execution after failures.
...
## Next phase update: encounter generation runtime dependency helper split

- Extracted encounter-generation runtime dependency resolution from `server/useCases/generateEncounters.ts` into `server/utils/generateEncountersRuntime.ts`.
  - The new helper owns project/encounter root defaults, Node filesystem adapter defaults, temp-directory creation/cleanup adapters, injected seam preservation, and default `pokegen.sh` runner construction.
- Reduced `generateEncountersUseCase` so it focuses on request normalization, table loading, rolling, output planning, pokegen batch execution, result composition, error normalization, and preview cleanup without importing Node filesystem/path/process helpers directly.
- Preserved encounter generation behavior: request/response shapes, preview cleanup, output directory defaults, injected test seams, default pokegen invocation, table loading, rolling, and failure mapping remain compatible.
- Added `tests/server/generateEncountersRuntime.test.ts` covering injected seam preservation, explicit encounter-root overrides, default filesystem adapters, temp cleanup, and process-root defaults.
- Next remaining phase: continue one bounded cleanup pass, with candidates including another focused server/use-case helper split, move-automation helper split, renderer/helper split, storage split, map-editor extraction, remaining client/helper cleanup, or small UI duplication cleanup; do not mark the full refactor complete yet.
- Quality gates after this phase:
...
## Next phase update: condition tag art helper split

- Extracted condition-tag definitions, fallback label construction, inline SVG icon drawing, size scaling, and SVG markup construction from `utils/statusConditions.ts` into `utils/conditionTagArt.ts`.
- Reduced `statusConditions.ts` to PTU condition loading, ordering, alias normalization, text extraction, grouping, and thin tag/title integration while `ConditionTag`/`ConditionPicker` import the shared tag-size type from the focused art helper.
- Preserved condition normalization, tag badge SVG shape/classes, aliases, condition titles, and condition picker/tag behavior.
- Added `tests/utils/conditionTagArt.test.ts` covering canonical tag definitions, fallback labels, SVG escaping/size scaling, and status-condition integration.
- Next remaining phase: continue one bounded cleanup pass, with candidates including another focused server/use-case helper split, move-automation helper split, renderer/helper split, storage split, map-editor extraction, remaining client/helper cleanup, or small UI duplication cleanup; do not mark the full refactor complete yet.
- Quality gates after this phase:
...
## Next phase update: capability art icon helper split

- Extracted capability-art SVG icon motif markup from `utils/capabilityArt.ts` into `utils/capabilityArtIcons.ts`.
  - `capabilityArt.ts` now focuses on capability art registry/fallback selection and final SVG composition while the focused icon helper owns motif markup for known icons plus the fallback icon.
- Preserved capability art titles, sizes, fallback behavior, known-icon SVG output, escaped accent/color handling at the composition boundary, and public `capabilityArtSvg` behavior.
- Added `tests/utils/capabilityArtIcons.test.ts` covering known icon motif output, fallback icon output, and public SVG integration through the extracted helper.
- Next remaining phase: continue one bounded cleanup pass, with candidates including another focused server/use-case helper split, move-automation helper split, renderer/helper split, storage split, map-editor extraction, remaining client/helper cleanup, or small UI duplication cleanup; do not mark the full refactor complete yet.
- Quality gates after this phase:
  - `npm test -- tests/utils/capabilityArtIcons.test.ts tests/utils/capabilityArtCore.test.ts` — passes: 2 test files / 7 tests.
  - `npm run typecheck` — passes.
  - `npm test` — passes: 197 test files / 752 tests.
  - `npm run build` — passes; existing large chunk warnings remain.
  - `npm run check:move-automation` — still fails with baseline `Explicit move automation coverage: 0/769` missing-script report.


## Next phase update: map field-effect definition split

- Extracted map field-effect kinds, definition tables, typed definition DTO, and weather/terrain/room kind guards from `utils/mapFieldEffects.ts` into `utils/mapFieldEffectDefinitions.ts`.
- Reduced `utils/mapFieldEffects.ts` to field-effect creation, normalization, source/round coercion, deduplication, and active-count logic while renderer/editor UI modules import definitions from the focused definition module.
- Preserved field-effect palettes, default round counts, Weather/Terrain/Room labels/rules/colors, map field-effect normalization, renderer terrain/room overlay colors, and map editor behavior.
- Added `tests/utils/mapFieldEffects.test.ts` covering canonical definition alignment, kind guards, default effect factories, normalization/deduplication/source trimming, and effect counts.
- Next remaining phase: continue one bounded cleanup pass, with candidates including another focused server/use-case helper split, move-automation helper split, renderer/helper split, storage split, map-editor extraction, remaining client/helper cleanup, or small UI duplication cleanup; do not mark the full refactor complete yet.
- Quality gates after this phase:
  - `npm test -- tests/utils/mapFieldEffects.test.ts` — passes: 1 test file / 5 tests.
  - `npm run typecheck` — passes.
  - `npm test` — passes: 198 test files / 757 tests.
  - `npm run build` — passes; existing large chunk warnings remain.
  - `npm run check:move-automation` — still fails with baseline `Explicit move automation coverage: 0/769` missing-script report.

## Next phase update: map hazard definition split

- Extracted canonical map hazard kind lists, definition DTOs, definition table, kind guard, and kind fallback normalization from `utils/mapHazards.ts` into `utils/mapHazardDefinitions.ts`.
- Reduced `utils/mapHazards.ts` to hazard document normalization, layer coercion, hazard keys, and bounds filtering while map-editor UI and hazard renderer modules import hazard definitions from the focused definition module.
- Preserved hazard palettes, labels, colors, Toxic Spikes layer behavior, owner trimming, renderer decal labels/colors, and map hazard normalization behavior.
- Added `tests/utils/mapHazards.test.ts` covering canonical definition alignment, kind guards/fallbacks, layer coercion, hazard normalization/keying, invalid records, and bounds filtering.
- Next remaining phase: continue one bounded cleanup pass, with candidates including another focused server/use-case helper split, move-automation helper split, renderer/helper split, storage split, map-editor extraction, remaining client/helper cleanup, or small UI duplication cleanup; do not mark the full refactor complete yet.
- Quality gates after this phase:
  - `npm test -- tests/utils/mapHazards.test.ts` — passes: 1 test file / 5 tests.
  - `npm run typecheck` — passes.
  - `npm test` — passes: 199 test files / 762 tests.
  - `npm run build` — passes; existing large chunk warnings remain.
  - `npm run check:move-automation` — still fails with baseline `Explicit move automation coverage: 0/769` missing-script report.

## Next phase update: voxel helper module split

- Split the broad voxel helper into focused modules:
  - `utils/voxelMaterials.ts` for terrain material palette entries, material lookup DTOs, and material-id normalization aliases.
  - `utils/voxelColors.ts` for hex parsing/formatting, face palettes, voxel base-color resolution, deterministic water builder colors, and render grouping keys.
  - `utils/voxelOccupancy.ts` for voxel cell keys, movement/all-voxel occupancy sets, token-footprint overlap checks, and bounds filtering.
- Updated map editor, isometric renderer, map occupancy, and dimension-reconciliation imports to depend on the focused voxel modules, and removed the legacy aggregate `utils/voxels.ts` to avoid duplicate Nuxt auto-import surfaces.
- Preserved terrain builder behavior, renderer voxel grouping, water default builder colors, map-dimension voxel trimming, movement/pathfinding occupancy, and all public map/sheet routes.
- Added tests:
  - `tests/utils/voxelColors.test.ts`
  - `tests/utils/voxelOccupancy.test.ts`
- Next remaining phase: continue one bounded cleanup pass, with candidates including another focused server/use-case helper split, move-automation helper split, renderer/helper split, storage split, map-editor extraction, remaining client/helper cleanup, or small UI duplication cleanup; do not mark the full refactor complete yet.
- Quality gates after this phase:
...
## Next phase update: trainer combat vitals strip component

- Extracted the trainer sheet combat-vitals strip from `components/sheets/TrainerCombatOverviewPanel.vue` into `components/sheets/TrainerCombatVitalsStrip.vue`.
  - The new component owns current/max HP, tick/fractional HP, damage reduction, level, combat-stage attack totals, and speed summary presentation plus focused update emits.
- Reduced `TrainerCombatOverviewPanel` so it focuses on composing combat overview sections while preserving Action Point, evasion, condition, digestion, and capability editing behavior.
- Preserved trainer sheet autosave/edit behavior, HP clamping through the existing parent setter, damage reduction/level editable-cell updates, combat-vitals copy/tooltips, and visual styling by moving the strip styles with the extracted component.
...
## Next phase update: trainer action points panel component

- Extracted the trainer sheet Action Points table from `components/sheets/TrainerCombatOverviewPanel.vue` into `components/sheets/TrainerActionPointsPanel.vue`.
  - The new component owns AP left/spent/bound/drained editing, max-AP display, table markup, and table-specific styles behind narrow `ap`/`maxAp` props.
...
## Next phase update: trainer capabilities panel component

- Extracted the trainer sheet Capabilities block from `components/sheets/TrainerCombatOverviewPanel.vue` into `components/sheets/TrainerCapabilitiesPanel.vue`.
  - The new component owns movement/utility capability editable cells, free-form other-capabilities editing, and capability-specific grid/help styling behind focused `capabilities` and `otherCapsCsv` bindings.
- Reduced `TrainerCombatOverviewPanel` so it now composes combat vitals, Action Points, Evasion/Conditions, and Capabilities as focused child panels while preserving existing editable-cell bindings and autosave behavior.
- Preserved trainer capability editing semantics, Other capabilities CSV persistence, visual styling, and sheet route behavior; no data or route semantics changed.
- Next remaining phase: continue one bounded cleanup pass, with candidates including another focused sheet-panel extraction, server/use-case helper split, move-automation helper split, renderer/helper split, storage split, map-editor extraction, remaining client/helper cleanup, or small UI duplication cleanup; do not mark the full refactor complete yet.
- Quality gates after this phase:
  - `npm run typecheck` — passes.
  - `npm test` — passes: 202 test files / 776 tests.
...
## Next phase update: Pokémon combat vitals strip component

- Extracted the Pokémon sheet combat-vitals grid from `components/sheets/PokemonCombatPanel.vue` into `components/sheets/PokemonCombatVitalsStrip.vue`.
  - The new component owns current/max HP, injuries, tick/fractional HP, DR, and Training Exp presentation/editing behind a narrow combat/vitals prop contract plus a focused current-HP emit.
- Reduced `PokemonCombatPanel` so it now focuses on composing the combat vitals strip, Pokémon evasion, conditions, vitamins, and notes while preserving existing editable-cell bindings and autosave behavior.
- Preserved Pokémon HP clamping through the existing parent setter, injury/DR/training-exp edits, HP threshold copy/tooltips, responsive grid styling, and sheet route behavior; no data or route semantics changed.
- Next remaining phase: continue one bounded cleanup pass, with candidates including another focused sheet-panel extraction (for example Pokémon evasion/conditions), server/use-case helper split, move-automation helper split, renderer/helper split, storage split, map-editor extraction, remaining client/helper cleanup, or small UI duplication cleanup; do not mark the full refactor complete yet.
- Quality gates after this phase:
  - `npm run typecheck` — passes.
...
## Next phase update: trainer combat action subpanel extraction

- Extracted the trainer sheet Combat Actions tab sections from `components/sheets/TrainerCombatActionsPanel.vue` into focused child components:
  - `components/sheets/TrainerMovesPanel.vue`
  - `components/sheets/TrainerAbilitiesPanel.vue`
  - `components/sheets/TrainerManeuversPanel.vue`
  - `components/sheets/TrainerOrdersPanel.vue`
- Added `components/sheets/trainerCombatActionPanel.css` for shared combat-action block/table/action styling so the extracted panels preserve the existing visual presentation without duplicating route-level chrome.
- Reduced `TrainerCombatActionsPanel` to a composition shell that forwards explicit add/remove/tag events while preserving move lookup editing, ability lookup editing, maneuver editing, order tag CSV persistence, and sheet autosave behavior.
- Next remaining phase: continue one bounded cleanup pass, with candidates including another focused sheet-panel extraction, server/use-case helper split, move-automation helper split, renderer/helper split, storage split, map-editor extraction, remaining client/helper cleanup, or small UI duplication cleanup; do not mark the full refactor complete yet.
- Quality gates after this phase:
  - `npm run typecheck` — passes.
  - `npm test` — passes: 202 test files / 776 tests.
...
## Next phase update: sheet library page composable extraction

- Extracted `/sheets` library route orchestration from `pages/sheets/index.vue` into `composables/library/useSheetLibraryPage.ts`.
  - The composable now owns sheet-library auth/drag gating, folder navigation, grid filtering, sheet/folder drag-drop actions, creation flows, context-menu submit wiring, and Escape-key cleanup while keeping route-specific API calls injected through the existing API client boundary.
- Reduced `pages/sheets/index.vue` to a route shell for metadata and composition of the shared library layout, intro panel, breadcrumbs, grid, and context menu.
- Preserved sheet-library behavior: player/GM visibility rules, dev-only drag/create gating, hard navigation after new sheet creation, folder breadcrumbs, sheet/folder drag/drop, context-menu move/rename/delete flows, and existing API request shapes.
- Next remaining phase: continue one bounded cleanup pass, with candidates including map library page composable extraction, another focused sheet/map panel extraction, server/use-case helper split, move-automation helper split, renderer/helper split, storage split, map-editor extraction, or remaining client/helper cleanup; do not mark the full refactor complete yet.
- Quality gates after this phase:
  - `npm test -- tests/composables/library/useSheetLibraryData.test.ts tests/composables/library/useSheetLibraryActions.test.ts tests/composables/library/useSheetLibraryCreation.test.ts tests/composables/library/useLibraryGridView.test.ts` — passes: 4 test files / 14 tests.
  - `npm run typecheck` — passes.
  - `npm test` — passes: 203 test files / 779 tests.
  - `npm run build` — passes; existing large chunk warnings remain.
  - `npm run check:move-automation` — still fails with baseline `Explicit move automation coverage: 0/769` missing-script report.

## Next phase update: map library page composable extraction

- Extracted `/maps` library route orchestration from `pages/maps/index.vue` into `composables/library/useMapLibraryPage.ts`.
  - The composable now owns map-library auth visibility filtering, folder navigation, grid filtering, map/folder drag-drop actions, creation flows, context-menu submit wiring, realtime-backed data loading, and Escape-key cleanup while keeping API requests behind the existing API client boundary.
- Reduced `pages/maps/index.vue` to a route shell for metadata and composition of the shared library layout, intro panel, breadcrumbs, grid, and context menu.
- Preserved map-library behavior: player-visible filtering, GM-only create/drag/context actions, `clientId` echo suppression payloads, folder breadcrumbs, map/folder drag/drop, context-menu move/rename/delete flows, realtime updates, and existing API request shapes.
- Next remaining phase: continue one bounded cleanup pass, with candidates including another focused sheet/map panel extraction, server/use-case helper split, move-automation helper split, renderer/helper split, storage split, map-editor extraction, or remaining client/helper cleanup; do not mark the full refactor complete yet.
- Quality gates after this phase:
  - `npm test -- tests/composables/library/useMapLibraryData.test.ts tests/composables/library/useMapLibraryActions.test.ts tests/composables/library/useMapLibraryCreation.test.ts tests/composables/library/useLibraryGridView.test.ts` — passes: 4 test files / 13 tests.
  - `npm run typecheck` — passes.
  - `npm test` — passes: 203 test files / 779 tests.
  - `npm run build` — passes; existing large chunk warnings remain.
  - `npm run check:move-automation` — still fails with baseline `Explicit move automation coverage: 0/769` missing-script report.
...
## Next phase update: Pokédex filter panel extraction

- Extracted the Pokédex sidebar filter-mode toggle, advanced search field, field-filter/operator controls, helper copy, and focused filter styling into `components/pokedex/PokedexFilterPanel.vue`.
- Reduced `components/pokedex/PokedexSidebar.vue` so it focuses on sidebar chrome, scroll preservation, and entry-list rendering while delegating filter presentation through explicit `filterMode`, `filterOperators`, and `searchFilters` bindings.
- Preserved Pokédex field/advanced filtering behavior, search model mutation, operator toggles, helper copy, sidebar scroll restoration, entry routes, type badges, and responsive layout styling.
- Next remaining phase: continue one bounded cleanup pass, with candidates including another focused Pokédex/sidebar component extraction (for example entry-list extraction), sheet/map panel extraction, server/use-case helper split, move-automation helper split, renderer/helper split, storage split, map-editor extraction, or remaining client/helper cleanup; do not mark the full refactor complete yet.
- Quality gates after this phase:
  - `npm run typecheck` — passes.
...
## Next phase update: Pokédex entry list item component extraction

- Extracted the individual Pokédex sidebar result-card/link markup from `components/pokedex/PokedexEntryList.vue` into `components/pokedex/PokedexEntryListItem.vue`.
- Reduced `PokedexEntryList` so it now owns only list/empty-state composition and scroll-ref forwarding, while the new item component owns entry routes, selected-page ARIA state, National Dex/type/source metadata, and card styling.
- Preserved Pokédex entry routes, selected-entry highlighting, type badge rendering, empty-state copy, sidebar scroll restoration, and responsive list behavior.
- Next remaining phase: continue one bounded cleanup pass, with candidates including another focused Pokédex/detail or filter-panel extraction, sheet/map panel extraction, server/use-case helper split, move-automation helper split, renderer/helper split, storage split, map-editor extraction, or remaining client/helper cleanup; do not mark the full refactor complete yet.
- Quality gates after this phase:
...
## Next phase update: editable cell display component extraction

- Extracted editable-cell display/fallback markup and scoped empty/display text styles into `components/EditableCellDisplay.vue`.
- Added `resolveEditableCellDisplay` in `utils/editableCell.ts` so empty-state labels, formatted display values, and fallback rendering decisions are pure and test-covered.
- Reduced `components/EditableCell.vue` so it now coordinates edit-session state and composes focused display/editor children while preserving custom `#display` slot props, placeholder/empty-text fallback behavior, readonly rendering, and autosave-triggering model updates.
- Next remaining phase: continue one bounded cleanup pass, with candidates including another focused editable-cell/UI extraction, Pokédex/detail or sidebar extraction, sheet/map panel extraction, server/use-case helper split, move-automation helper split, renderer/helper split, storage split, map-editor extraction, or remaining client/helper cleanup; do not mark the full refactor complete yet.
- Quality gates after this phase:
...
## Next phase update: Pokédex search filter primitive extraction

- Extracted the Pokédex search input control from `components/pokedex/PokedexSearchFilterFields.vue` into `components/pokedex/PokedexSearchFilterInput.vue`.
- Extracted the field-filter operator selector into `components/pokedex/PokedexFilterOperatorSelect.vue`.
- Reduced `PokedexSearchFilterFields` to field/advanced filter composition while preserving trimmed search models, field-operator bindings, ARIA labels, focus styling, and advanced-vs-field filter behavior.
- Next remaining phase: continue one bounded cleanup pass, with candidates including another focused Pokédex/detail or sidebar extraction, sheet/map panel extraction, server/use-case helper split, move-automation helper split, renderer/helper split, storage split, map-editor extraction, or remaining client/helper cleanup; do not mark the full refactor complete yet.
- Quality gates after this phase:
  - `npm run typecheck` — passes.
  - `npm test -- tests/composables/pokedex/usePokedexFilters.test.ts tests/composables/pokedex/usePokedexBrowser.test.ts tests/utils/pokedex/searchQuery.test.ts tests/utils/pokedex/searchText.test.ts` — passes: 4 test files / 14 tests.
...
## Next phase update: encounter result header and file list extraction

- Extracted encounter-generation result heading, failure badge, file-count pills, and generated-folder hint into `components/encounters/EncounterGenerateResultHeader.vue`.
- Extracted generated/preview file rows, expandable preview content, and per-file error rendering into `components/encounters/EncounterGenerateResultFiles.vue`.
- Reduced `EncounterGenerateResultCard.vue` to a result-card shell that wires result data and open-file toggles while preserving preview expansion, generated-folder copy, failure/file badges, error rows, and visual styling.
...
## Next phase update: encounter result file-row component extraction

- Extracted individual generated/preview encounter result file rows from `components/encounters/EncounterGenerateResultFiles.vue` into `components/encounters/EncounterGenerateResultFileRow.vue`.
- Reduced the result file list so it owns only list iteration and open-file lookup while the focused row component owns expandable preview toggles, file-name/error display, preview content rendering, and row-specific styling.
- Preserved encounter generation result behavior: preview-file expansion, error row styling, generated-file display, toggle payloads, and result-card composition remain unchanged.
- Next remaining phase: continue one bounded cleanup pass, with candidates including another focused encounter component extraction, Pokédex/detail or sidebar extraction, sheet/map panel extraction, server/use-case helper split, move-automation helper split, renderer/helper split, storage split, map-editor extraction, or remaining client/helper cleanup; do not mark the full refactor complete yet.
- Quality gates after this phase:
  - `npm run typecheck` — passes.
  - `npm test` — passes: 204 test files / 785 tests.
...
## Next phase update: encounter tables sidebar header extraction

- Extracted the encounter-tables sidebar heading, count badge, explanatory Generate link copy, and search input into `components/encounters/EncounterTablesSidebarHeader.vue`.
- Reduced `EncounterTablesSidebar.vue` so it now owns only navigation/sidebar shell and region-list composition while the focused header component owns sidebar intro/search presentation and the shared Generate route helper import.
- Preserved encounter-table search binding, count badge copy, Generate-page link target, sidebar styling, region-list selection behavior, and public `/encounter-tables` route behavior.
- Next remaining phase: continue one bounded cleanup pass, with candidates including another focused encounter component extraction (for example region-list item extraction), Pokédex/detail or sidebar extraction, sheet/map panel extraction, server/use-case helper split, move-automation helper split, renderer/helper split, storage split, map-editor extraction, or remaining client/helper cleanup; do not mark the full refactor complete yet.
- Quality gates after this phase:
  - `npm run typecheck` — passes.
  - `npm test -- tests/composables/encounters/useEncounterTableBrowser.test.ts tests/utils/encounterTables.test.ts tests/utils/encounterRoutes.test.ts` — passes: 3 test files / 10 tests.
  - `npm test` — passes: 204 test files / 785 tests.
...
## Next phase update: encounter tables region-list item extraction

- Extracted individual encounter-table sidebar result rows from `components/encounters/EncounterTablesRegionList.vue` into `components/encounters/EncounterTablesRegionListItem.vue`.
- Reduced the region-list component so it owns only region grouping, region headings, empty-state copy, and select-entry forwarding while the focused item component owns table-row button presentation, active styling, level/count metadata, and click emits.
- Preserved encounter-table search/selection behavior, active row styling, table metadata copy, sidebar layout, and public `/encounter-tables` route behavior.
- Next remaining phase: continue one bounded cleanup pass, with candidates including another focused encounter component extraction, Pokédex/detail or sidebar extraction, sheet/map panel extraction, server/use-case helper split, move-automation helper split, renderer/helper split, storage split, map-editor extraction, or remaining client/helper cleanup; do not mark the full refactor complete yet.
- Quality gates after this phase:
  - `npm test -- tests/composables/encounters/useEncounterTableBrowser.test.ts tests/utils/encounterTables.test.ts tests/utils/encounterRoutes.test.ts` — passes: 3 test files / 10 tests.
  - `npm run typecheck` — passes.
