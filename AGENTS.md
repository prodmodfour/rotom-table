...
- Completion snapshot: Plans 1–13 are `DONE` and archived. Plan 11 ([Deferred Mechanics Closure](implementation-plans/done/DEFERRED_MECHANICS_CLOSURE_PLAN.md)) completed all 92 tickets and 29/29 closure rows; Plan 12 ([GM Campaign Toolkit](implementation-plans/done/GM_CAMPAIGN_TOOLKIT_PLAN.md)) completed all 96 tickets and 40/40 footprint rows; Plan 13 ([1.0 Release Readiness](implementation-plans/done/RELEASE_READINESS_PLAN.md)) completed all 86 tickets. Immutable annotated, unpublished `v1.0.0` remains failed closed. Owner-authorized local `v1.0.1` reproduced all 12,663 tagged output checksums exactly at manifest `82fb0dfd…`; remote publication, upload, and deployment remain unauthorized. No post-1.0 scope is active.
- Read `implementation-plans/plan-order.md` before implementation work and follow its dependency order.
- The authoritative archived ledgers for Plans 11–13 are `implementation-plans/done/DEFERRED_MECHANICS_CLOSURE_PLAN.md`, `implementation-plans/done/GM_CAMPAIGN_TOOLKIT_PLAN.md`, and `implementation-plans/done/RELEASE_READINESS_PLAN.md`; each registered numbered plan's `PLAN_STATUS` and `CURRENT_TICKET` remain authoritative.
- The recorded 1.0 finish line in `implementation-plans/plan-order.md` is complete. Plan 13 is the archived numbered ledger for that finish line. Registered drafts and prospective post-1.0 rows remain scope intent only until converted into reviewed numbered ledgers, registered in the authoritative plan table, and explicitly started by the owner.
