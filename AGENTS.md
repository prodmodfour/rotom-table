...
- Active execution snapshot: Plans 1–12 are `DONE` and archived. Plan 11 ([Deferred Mechanics Closure](implementation-plans/done/DEFERRED_MECHANICS_CLOSURE_PLAN.md)) completed all 92 tickets and 29/29 closure rows; Plan 12 ([GM Campaign Toolkit](implementation-plans/done/GM_CAMPAIGN_TOOLKIT_PLAN.md)) completed all 96 tickets and 40/40 footprint rows. Plan 13 ([1.0 Release Readiness](implementation-plans/RELEASE_READINESS_PLAN.md)) is active (`PLAN_STATUS: IN_PROGRESS`, 84/86 tickets complete). P13-085 failed closed when immutable annotated, unpublished `v1.0.0` produced divergent checksums across identical supported-shape builds. A deterministic repair is prepared and has reproduced all 12,663 output files twice, but the consumed P13-084 authorization does not permit the required `1.0.1` patch transaction. Execution remains at `CURRENT_TICKET: P13-085` with `BLOCKED_BY: OWNER_PATCH_RELEASE_AUTHORIZATION_P13_085`; remote publication remains unauthorized.
...
- The recorded finish line is the 1.0 release definition in `implementation-plans/plan-order.md`; Plan 13 is the numbered ledger for that finish line and its own `PLAN_STATUS`, owner start gate, and decision log are authoritative. Registered drafts and prospective rows remain scope intent only until converted into reviewed numbered ledgers registered in the authoritative plan table and explicitly started by the owner.
