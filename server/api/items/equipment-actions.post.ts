...
import { executeEquipmentActionUseCase } from '../../useCases/executeEquipmentAction'
...
export default defineEventHandler(async (event) => {
  ...
  if (Object.keys(body).some(key => !allowedFields.has(key)) || !Object.hasOwn(body, 'command')) {
    badRequest('Equipment action request must contain only command, profileId, and clientId.')
  }
  ...
  try {
    return {
      result: executeEquipmentActionUseCase({
        role,
