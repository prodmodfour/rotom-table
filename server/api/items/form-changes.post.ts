...
import { executeItemFormChangeUseCase } from '../../useCases/executeItemFormChange'
...
export default defineEventHandler(async (event) => {
  ...
  if (Object.keys(body).some(key => !allowedFields.has(key)) || !Object.hasOwn(body, 'command')) {
    badRequest('Item form-change request must contain only command, profileId, and clientId.')
  }
  ...
  try {
    return {
      result: executeItemFormChangeUseCase({
        role,
