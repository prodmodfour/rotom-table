...
import { declareGroupInventoryItemActionUseCase } from '../../../useCases/declareGroupInventoryItemAction'
...
export default defineEventHandler(async (event) => {
  ...
  if (!Object.hasOwn(body, 'intent') || Object.keys(body).some(key => !allowed.has(key))) {
    badRequest('Shared item declaration must contain only intent and profileId.')
  }
  if (body.profileId !== undefined && typeof body.profileId !== 'string') {
    badRequest('profileId must be a string when provided.')
  }
  try {
    return declareGroupInventoryItemActionUseCase({
      role,
