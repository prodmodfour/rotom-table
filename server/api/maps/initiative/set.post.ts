...
type SetInitiativeBody = Record<string, unknown>

const bodyField = (body: SetInitiativeBody, key: string): unknown => body[key]
...
export default defineEventHandler(async (event) => {
  ...
  const body = await readObjectBody<SetInitiativeBody>(event)
  ...
  try {
    const response = await executeLivePlayInitiativeCommandUseCase({
      ...
      expectedType: LIVE_PLAY_COMMAND_TYPES.SET_INITIATIVE,
    })
