...
type RemoveTerrainVoxelBody = Record<string, unknown>

const bodyField = (body: RemoveTerrainVoxelBody, key: string): unknown => body[key]
...
export default defineEventHandler(async (event) => {
  ...
  const body = await readObjectBody<RemoveTerrainVoxelBody>(event)
  ...
  try {
    const response = await executeLivePlayTerrainCommandUseCase({
      ...
      expectedType: LIVE_PLAY_COMMAND_TYPES.REMOVE_TERRAIN_VOXEL,
    })
