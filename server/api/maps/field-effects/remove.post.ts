...
type RemoveFieldEffectBody = Record<string, unknown>

const bodyField = (body: RemoveFieldEffectBody, key: string): unknown => body[key]
...
export default defineEventHandler(async (event) => {
  ...
  const body = await readObjectBody<RemoveFieldEffectBody>(event)
  ...
  try {
    const response = await executeLivePlayMapEffectsCommandUseCase({
      ...
      expectedType: LIVE_PLAY_COMMAND_TYPES.REMOVE_FIELD_EFFECT,
    })
