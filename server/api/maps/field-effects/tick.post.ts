...
type TickFieldEffectDurationsBody = Record<string, unknown>

const bodyField = (body: TickFieldEffectDurationsBody, key: string): unknown => body[key]
...
export default defineEventHandler(async (event) => {
  ...
  const body = await readObjectBody<TickFieldEffectDurationsBody>(event)
  ...
  try {
    const response = await executeLivePlayMapEffectsCommandUseCase({
      ...
      expectedType: LIVE_PLAY_COMMAND_TYPES.TICK_FIELD_EFFECT_DURATIONS,
    })
