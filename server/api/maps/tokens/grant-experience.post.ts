...
type GrantExperienceBody = Record<string, unknown>

const bodyField = (body: GrantExperienceBody, key: string): unknown => body[key]
...
export default defineEventHandler(async (event) => {
  ...
  const body = await readObjectBody<GrantExperienceBody>(event)
  ...
  try {
    ...
    const response = await executeLivePlaySheetCommandUseCase({
      ...
      playerProfile,
      expectedType: LIVE_PLAY_COMMAND_TYPES.GRANT_EXPERIENCE,
    })
