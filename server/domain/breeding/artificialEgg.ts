...
const strictStoredTrainer = (value: unknown): { readonly slug: string, readonly revision: number, readonly document: StrictJsonObject } => {
  ...
  const document = cloneStrictJson(row.document, 'trainerSheet.document', {
  ...
  if (!document || typeof document !== 'object' || Array.isArray(document)
    || document.slug !== row.slug || document.revision !== row.revision) return fail('breeding.artificial-egg.stale-authority', 'Trainer document identity and storage revision must agree exactly.')
  return Object.freeze({ slug: row.slug, revision: row.revision as number, document: document as StrictJsonObject })
}
