...
import {
  equipmentGrantOwnerContext,
  resolveEquipmentGrants,
  type ResolveEquipmentGrantsResult,
...
export interface AuthoritativeMoveEquipmentGrantQueries {
  resolve(placementId: string): ResolveEquipmentGrantsResult | null
}
...
export const createMoveEquipmentGrantQueries = (input: {
...
}): AuthoritativeMoveEquipmentGrantQueries => {
...
/** Presentation-safe map snapshot query with the same Magic Room/Klutz semantics as Move execution. */
export const createEncounterEquipmentGrantQueries = (input: {
  ...
  return createMoveEquipmentGrantQueries({
    placements: input.map.placements,
    sheets: input.sheets,
    itemEffects,
