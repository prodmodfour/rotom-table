...
export const resolveAuthoritativeMoveUserAccuracy = (
  ...
  const encounterAccuracy = applyEncounterNumericModifiers({
    ...
    now: context.time,
    isCapabilityEffective: canonicalId => context.queries.creatureRules.hasCapability(context.actor.placement.id, canonicalId),
    isCapabilityInstanceEffective: (instanceId, canonicalId) => context.queries.creatureRules
      .hasCapabilityInstance(context.actor.placement.id, instanceId, canonicalId),
