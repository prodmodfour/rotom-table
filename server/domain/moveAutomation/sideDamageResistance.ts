...
export type SideDamageResistanceReasonCode =
...
export const createSideDamageResistanceResolver = (input: {
  ...
  const resolve: SideDamageResistanceResolver['resolve'] = (request) => {
    ...
    const available = classEffects.find((effect) => {
...
/** Add bounded activation/retention evidence to the authoritative trace. */
export const appendSideDamageResistanceTrace = (
  ...
  return resolution.evaluations.reduce((current, evaluation, index) => (
    reduceMoveResolutionTrace(current, {
      kind: 'predicate',
      phase,
      predicateId: `side-damage-resistance.${index + 1}`,
