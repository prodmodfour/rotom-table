...
Reducer output enters an immutable typed state-change plan rather than an arbitrary JSON patch. The plan retains one global operation order while grouping changes by map, encounter state, placement, sheet, and separately revisioned external resource. Each physical resource has one expected revision, and exact aggregate before/current values carry either a reviewed restore-previous-value compensation candidate or an explicit reason why compensation is unavailable. Compensation metadata is not permission to overwrite later state: a future correction must still validate current values and revisions. Native v2 core-token reducers now apply server-resolved damage, direct HP, healing, scene-local temporary HP, Injury automation, canonical conditions, and capped combat stages through local accumulators. They independently verify emitted recipients against the resolved selector sets, retain immune/no-op outcomes in the audit trace, and produce one aggregate typed replacement per affected map or sheet without mutating the authoritative snapshot. The legacy planner continues to project its existing writes into the same plan contract; map/usage/log reducers and per-operation v1 adaptation remain separate migration steps.
