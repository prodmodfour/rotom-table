...
Encounter-state effect instances are bounded discriminated data for condition, numeric-modifier, and capability payloads. A temporary-effect operation may request only the context-free typed definition; when lifecycle reduction is enabled, authoritative reducer context—not the spec—must supply source operation/move/placement identity, affected placements/sides/cells, creation turn/round, and suppression state. Duration advancement, stacking/refresh policy, charge consumption, and lifecycle-trigger execution remain fail-closed until their dedicated lifecycle reducers land.
