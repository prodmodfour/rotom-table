...
A forward live-play initiative command now emits `turn-end`, then `round-end`/`round-start` when wrapping, then `turn-start`; beginning an inactive round emits its start before the first turn. The same accepted-result transaction persists effect expiry, currently reducible due direct-HP/heal/condition/stage operations, map state, every changed sheet, the terminal operation result, and authorized realtime rows after validating all calculated-order and mechanics sheet revisions. Unsupported triggered operations fail the command before mutation. Duplicate `opId` delivery cannot tick or apply the batch twice, and the initiative patch carries the resulting encounter/temporary-HP state for connected clients. `previousInitiative` and administrative `setInitiative` do not replay forward lifecycle facts. A complete GM-authored manual order bypasses calculated initiative queries; otherwise active typed initiative modifiers are queried over the calculated score without rewriting placement initiative. Scene-boundary integration and concrete move/field trigger registrations remain deferred to their owning tickets.
