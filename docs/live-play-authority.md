...
Relationship-dependent move rules use the centralized server query seam and only recognize ally/enemy relationships from explicit encounter side IDs. Unknown allegiance fails closed for ally/enemy mechanics: token ownership, player control, GM control, and mere map presence never imply that two placements are allies. Self identity remains available independently of side data. Placement `sideId` values and their map-local side directory are persisted through revision-checked GM setup saves; Prepare Map controls are the only browser mutation surface for creating, renaming, archiving, or assigning them. Cross-placement ally/enemy mechanics remain unconfirmed whenever a placement is intentionally unknown/unaffiliated rather than guessing allegiance.
