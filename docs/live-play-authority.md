...
Authoritative area resolution always completes Burst, Cone, Line, Close/Ranged Blast, Pass, or Cardinally Adjacent geometry first, in map order, and only then evaluates the reviewed MoveSpec relation/state predicate once per affected placement. Friendly intent can narrow that result but cannot add an out-of-area or predicate-illegal recipient. Complete inclusion/exclusion identities and reason codes remain in the server audit trace. Accepted move results expose only predicate-eligible candidates plus explicit Friendly exclusions, public trace summaries omit excluded target events, and legacy diagnostic copy does not name excluded placements; a rule-filtered or hidden token therefore cannot be discovered from an unauthorized terminal result.
