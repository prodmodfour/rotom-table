...
| Term | Meaning |
| --- | --- |
| Track 2 | The workstream that adds GM-hosted multi-device session concurrency to Rotom Table. |
...
| Term | Meaning |
| --- | --- |
| WebSocket session channel | The live bidirectional transport Track 2 uses for session commands, acks/rejections, broadcasts, presence, heartbeat, and reconnect handshakes. |
...
| Cloud database | Hosted persistence such as Postgres, Redis, Durable Objects, or another managed service. Adding one is out of scope for Track 2. |
