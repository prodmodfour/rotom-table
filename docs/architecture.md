...
The repositories keep SQL out of use cases and UI code. Map, sheet, folder, interaction-mode, group inventory, and live-play operation records are stored with explicit revision and timestamp columns while preserving JSON document payloads for setup/edit, imports, exports, and backups. Suspended MoveSpec executions use the separate `pending_move_resolutions` store: canonical validated resolution JSON is indexed by resolution identity and unique originating map/operation identity, updated through repository CAS revisions, and may link to—but never stand in for—a terminal `live_play_ops` row. Interpreter suspension commits that row, its bounded map summary, any explicitly typed pay-phase declaration HP cost, and every reviewed resource cost through the reached phase atomically after full map/sheet read-set validation. Its non-terminal acknowledgement is not saved in `live_play_ops`; exact declaration retries resolve the deterministic command-hash-derived resolution identity instead of replanning or spending again. The map document retains only bounded public pending summaries rather than private windows, rolls, reads, or traces. Private window queries and response-command seams re-evaluate actor, target/placement, selected-profile, explicit-side, and GM ownership against current map/profile token control; their client projection excludes ownership IDs, target identities, operation IDs, reads, rolls, and traces. Durable movement windows are the typed exception for authorized targeting presentation: they expose only server-issued destination/direction selections beside opaque option IDs, and resume regenerates and revalidates those options through the movement oracle before planning a shift. The map repository provides normalized map reads, revision-checked setup saves, create/move/rename/delete, logical folders, sheet-reference retargeting, operation-history barriers, and revision-checked live-play updates. The sheet repository provides sheet reads by kind/slug, revision-checked setup saves, create/move/rename/delete, logical folders, atomic map retargeting on sheet rename/delete, and revision-checked live-play sheet updates. The shop table repository stores reusable campaign shop documents with explicit revisions and timestamps in `shop_tables`; current read routes let GMs list/load all shops while players can list/load only shops that are both player-visible and open. Shop checkout operation history is stored separately in `shop_checkout_ops`, keyed by checkout `opId` and `shop_slug`, so shop-page checkout retries do not depend on a map slug. Accepted shop checkout commits append durable realtime rows for the changed shop document/list summary and any changed group inventory or trainer sheet documents, then publish those sequenced events after commit for echo-aware convergence. See [Shops and live-play checkout](shops.md) for the dedicated shop state, command-boundary, map-interface, idempotency, realtime, and export rules. Group inventory saves and trainer transfer routes append durable realtime events for the shared inventory and affected trainer sheet documents so other open clients can converge after the committing transaction. Immediate move plans may also carry typed group-inventory state changes: `resolveMove` validates their reviewed read revisions, commits repository CAS writes with map/sheet effects and the accepted operation result, and appends `group-inventory-access` realtime rows before the same SQLite commit. See [Group inventory workflow](group-inventory.md) for both boundaries. Live-play commands that change map state, sheet state, group inventory state, or any combination use SQLite repository updates and operation-result storage; multi-document commands use one SQLite transaction for every write and the accepted operation result. Operation history also supports terminal `abandoned` rejections: abandonment stores a tombstone under the same map write queue and command hash without changing map/sheet documents, so later exact command retries return the stored rejection instead of applying effects.
...
## Authoritative MoveSpec engine

Move automation is planned entirely from immutable server snapshots. The semantic manifest selects one versioned runtime and definition hash for a canonical move. All canonical production selections and registrations use MoveSpec v2; legacy definitions are excluded from the production runtime registry and remain available only to explicit migration/parity readers during the backup-compatibility window. Declarative MoveSpec v2 data owns targeting, costs, phases, typed operations, and presentation tags; registered handlers are reserved for bounded calculations that require authoritative context. Handlers receive pure snapshots and query interfaces and return validated operations—they cannot access repositories, clocks, networking, or persistence.

Planning records every consulted map, sheet, group-inventory, item, history, zone, and encounter-resource revision. Reducers produce one ordered typed state-change plan grouped by physical resource. The live-play use case validates the complete read set and requested conflict scopes, then atomically commits map/sheet/external-resource CAS writes, terminal operation status, lifecycle events, and durable realtime rows. A stale resource rejects the entire plan. Setup/execute state, actions, reactions, movement, once-per-turn flags, effects, sides, zones, counters, history, and public pending summaries live in the map's versioned encounter state; private response options and traces live only in `pending_move_resolutions`.

Snapshots and accepted patches carry the exact encounter state through strict parsers. Replay gaps replace local state from an authoritative snapshot; in-place map refresh preserves the Vue root while replacing changed encounter lanes. During the compatibility window mechanics may read a legacy map lane plus its canonical encounter projection, but accepted writes emit the canonical state and a presentation-only projection, and conflicting duplicate field representations fail closed. See [Data model](data-model.md), [Live play authority](live-play-authority.md), and [Move automation contributor guide](move-automation.md).
