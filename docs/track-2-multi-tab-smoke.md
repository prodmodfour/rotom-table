...
- It does not replace the [Track 2 LAN hosting runbook](track-2-lan-hosting.md), [Track 2 named Cloudflare Tunnel runbook](track-2-cloudflare-tunnel-hosting.md), or [Track 2 Quick Tunnel caveat](track-2-quick-tunnel-caveat.md). LAN setup is documented separately for same-network play; named tunnels are documented separately for stable-hostname remote play; Quick Tunnel remains development smoke-test only and legacy SSE remains a non-session/local path.
