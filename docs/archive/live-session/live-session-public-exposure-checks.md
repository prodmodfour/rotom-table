...
For the broader trust-boundary review, join-code limits, tunnel exposure risks, incident response, and non-hardened areas, see the [Live session security boundaries](live-session-security-boundaries.md). For the auth/session/cookie/permission boundary and public exposure warning review, see the [Live session security and secret-hygiene readiness](live-session-security-secret-hygiene-readiness.md).
