...
Ticket 094's [final session security audit](track-2-final-session-security-audit.md) rechecked the implemented auth/session/cookie/permission boundaries, public exposure warnings, and remaining non-goals after the final command, LAN smoke, named-tunnel documentation, and local-mode no-regression passes.

## Security outcome

The current Track 2 boundary is acceptable for the locked product shape when all of these are true:
...
If any of those assumptions are false, do not treat Track 2 as secure enough for public exposure. Use local mode only, or add a separate hardened authentication/deployment design outside Track 2.
...
| Existing `/login` role picker | Local/trusted campaign use on the GM's normal app origin. | Anyone who can reach the exposed origin. | Track 2 session routes add the runtime flag plus session-local GM key/join code/player identity checks; docs and banners repeat that `/login` is not public auth. | Non-session local-first routes still reflect trust-based assumptions. Do not expose the app as a general public website. |
...
Track 2 join codes are session-local capabilities for joining a trusted table, not account passwords.

- The default join code generator uses cryptographic randomness over the Track 2 join-code alphabet and currently creates 8-character codes; validators accept 6-12 characters and normalize case/separators for user entry.
...
The following are intentionally not solved by Track 2 and must not be implied by docs or UI:
...
Before marking a Track 2 hosting setup or docs change as acceptable:
