# Track 2 client integration guide
...
Open the plain map route for local editing. Open the explicit `?session=1` route when the table is using Track 2 session mode. The query flag is intentional so existing Track 1 map workflows keep their behaviour unless the user opts into the session view.
...
The existing `/login` GM/player role picker remains a local trust switch for the app shell. It is not public authentication. Track 2 session authority comes from the runtime flag, session-local GM key or player identity, WebSocket hello/auth validation, and server-side permission checks.
...
- local-mode whole-map saves are acceptable because they are outside the live Track 2 session concurrency path.
...
The final local-mode regression review is recorded in [Track 2 local-mode no-regression audit](track-2-local-mode-no-regression-audit.md), which cross-checks the plain map/sheet workflows, legacy SSE, local autosave, and explicit `?session=1` opt-in boundary after Track 2 landed.
