...
Move from unconditional full-frame work toward explicit dirty reasons and active-animation sources. The first compatibility integration routes the isometric scene through the scheduler while deliberately keeping active animation enabled, so visual behaviour remains the same as the old continuous loop while debug metrics can report scheduler frame reasons. Resize handling and OrbitControls change events now invalidate the scheduler only when their output-relevant state changes. The scheduled scene loop now separates its one-shot render step from an explicit animation-continuation source list; token center interpolation and selection lift now expose a `token-motion` continuation source, visible loaded animated token sprites expose a `sprite-animation` source, visible sprite textures that are still loading expose a `sprite-texture-loading` source, visible movement-preview ghosts with loaded sprite animation or pending texture work expose a `movement-preview-animation` source, and visible weather/field-effect animators expose a `field-effect-animation` source. Token and movement-preview sprite texture loads also request a `token-texture` one-shot render when the current async load completes, so static sprites repaint after decode even when the scene has otherwise settled. The compatibility continuous-loop source has been removed: settled scenes now stop producing duplicate idle RAF frames, while pointer interactions and coarse scene-state changes wake one-shot renders until later tickets refine every watcher and style invalidation path with more specific reasons. Document visibility changes now pause scheduled animation RAF work while the tab is hidden, preserve queued dirty state, and request a full `hidden-tab-resume` render when the tab becomes visible again.
