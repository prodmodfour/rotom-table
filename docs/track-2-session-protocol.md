...
- Live Track 2 sessions must not use `/api/events` for commands, command acknowledgements/rejections, presence, heartbeat, or reconnect. Session clients use `WebSocket /api/sessions/socket` after the session-host flag and hello/auth handshake, and table mutations route through server-authoritative command handlers.

This boundary lets the app preserve existing Track 1/local workflows while Track 2 session mode remains explicit and additive. Separate follow-up work may replace additional local-mode behaviours with session-aware alternatives when a session is active, but it should not remove the legacy SSE route until a dedicated migration explicitly does so.
...
Track 2 implementation keeps these checks at the protocol boundary:
...
- [Track 2 final implementation review](track-2-final-implementation-review.md)
