...
| RT-10 | Durable AoO interruption | Move a token out of adjacency, then choose an Attack of Opportunity or pass from the authorized reaction card. | The solid mover advances only to the authoritative checkpoint while its remaining intent route stays outlined. The eligible responder gets the prominent reaction card; other viewers get only the neutral waiting state. Pass resumes forward movement. Any splash is transient and never drives the durable mechanics. |
