...
| RT-15 | Duplicate event prevention | Reconnect or reload a same-map tab during/after an action, or repeat a debug publish with the same event id in development. | Each unique event id is replayed at most once per receiving tab. Existing visible cues are not replaced by a duplicate payload. |
