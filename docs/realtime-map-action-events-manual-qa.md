...
| RT-10 | Local AoO splash | Trigger the current assisted attack-of-opportunity indicator and choose its attack when available. | Same-map viewers see one reaction splash. Durable Spite, Cute Charm, Poison Point, Moxie, and Celebrate responses use the authoritative response panel instead and do not depend on transient splashes for mechanics. |
