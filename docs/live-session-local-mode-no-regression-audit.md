...
| Local map persistence | Local map edits still debounce and save whole map JSON through `MAP_API_PATHS.save`, then sync over the non-session realtime channel. This last-writer-wins local model remains acceptable outside live Live sessions. | `src/composables/useEditableMap.ts`, `tests/server/loadMap.test.ts`, `tests/server/createMap.test.ts`, `tests/server/renameMap.test.ts`, `tests/server/deleteMap.test.ts`, `tests/utils/maps/persistence.test.ts`, and `tests/utils/autosave.test.ts`. |
...
- Local mode still uses whole-map and whole-sheet JSON saves with last-writer-wins semantics. That is the intended local-first behaviour for one trusted editing browser and is not the live Live session concurrency mechanism.
