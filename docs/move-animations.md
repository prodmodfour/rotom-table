...
Accepted move VFX are driven by a bounded presentation summary in the durable `resolveMove` operation result and its accepted-command realtime patch. The summary contains the operation ID, actor, move identity, attacked/hit target IDs, outcome kind, and confirmed area/pass geometry—never client-authored mechanics. HTTP, SSE/replay, duplicate, and operation-status delivery all converge on that same summary, and each map client remembers the operation ID so it presents the accepted move at most once.

Planned move-animation batches may still be sent as transient map action events as an optional low-latency hint. Missing the transient hint no longer suppresses remote move VFX: a second client can plan generic VFX directly from the durable accepted result. Move roll, hit/miss, crit, effectiveness, and damage feedback snapshots remain transient visual events. Poké Ball capture attempts also broadcast their throw splash, a generic arc VFX from trainer token to target token, capture-roll feedback, and the final result modal or miss/error display. Each receiving browser still applies its own **Move VFX** toggle and reduced-motion preference, and remote VFX/feedback never apply gameplay mechanics, consume inventory, update trainer rosters, delete captured tokens, write logs, or save animation events.
...
Move animation events are runtime-only. The accepted-result rendering chain is bounded accepted summary -> `useAcceptedMovePresentation()` -> `useMoveAnimationQueue()` -> the map renderer; the richer local planner remains available for non-authoritative preview paths. When realtime is connected, the map page may also copy a planned batch through the bounded `/api/maps/action-event` `move-animations` payload so authorized viewers on the same map can enqueue the same transient VFX as an optional hint. The VFX events are not part of `SavedMap`, spawned Pokémon, trainer sheets, move automation transactions, live-session state, or durable storage. The durable operation result stores only mechanics-free accepted presentation facts and never stores renderer event IDs, timings, palettes, or queue state. Optional VFX source metadata such as `sourceKind` and `sourceLabel` follows the transient-only rule.
