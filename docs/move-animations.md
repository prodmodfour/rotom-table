...
When realtime is connected, planned move-animation batches are also sent as transient map action events so other authorized viewers on the same map can enqueue the same VFX. Move roll, hit/miss, crit, effectiveness, and damage feedback snapshots are likewise broadcast as transient visual events so other map viewers see the same overlay phases. Poké Ball capture attempts also broadcast their throw splash, a generic arc VFX from trainer token to target token, capture-roll feedback, and the final result modal or miss/error display. Each receiving browser still applies its own **Move VFX** toggle and reduced-motion preference, and remote VFX/feedback never apply gameplay mechanics, consume inventory, update trainer rosters, delete captured tokens, write logs, or save animation data.
