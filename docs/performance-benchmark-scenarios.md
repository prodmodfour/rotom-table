...
For repeatable public map data, generate the local fixtures in [Performance benchmark fixtures](performance-benchmark-fixtures.md) or follow its manual checklist before recording these scenarios. Use the [Performance benchmark runbook](performance-benchmark-runbook.md) for the step-by-step before/after workflow, overlay delta formulas, and metric interpretation guidance.
