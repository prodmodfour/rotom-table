...
- [ ] Standard checks passed recently, preferably through the project quality gate: `npm run typecheck`, `npm test`, and `npm run build`.
...
- [ ] Quick Tunnel is not being used for a campaign-session smoke. If a temporary `trycloudflare.com` URL is used for development only, use the [Quick Tunnel caveat](track-2-quick-tunnel-caveat.md) instead of this deployment checklist.
...
- LAN remains the primary supported Track 2 path; a named Cloudflare Tunnel with a stable hostname is the supported remote path.
...
- Track 2 session-local identity uses GM keys, join codes, player IDs, client IDs, display names, and GM-managed assignments; it is not a full account system.
