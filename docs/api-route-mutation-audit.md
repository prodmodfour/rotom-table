...
| `/api/maps/initiative/next` | GM-only map command write | Covered: production requires `ROTOM_ENABLE_HOSTED_WRITES=1`. | Advances the active initiative token through a canonical `nextInitiative` command, persists the authoritative map revision through SQLite, stores the `opId` result, and publishes map and accepted-command events. Player requests are rejected. |
| `/api/maps/initiative/previous` | GM-only map command write | Covered: production requires `ROTOM_ENABLE_HOSTED_WRITES=1`. | Moves the active initiative token backward through a canonical `previousInitiative` command, persists the authoritative map revision through SQLite, stores the `opId` result, and publishes map and accepted-command events. Player requests are rejected. |
