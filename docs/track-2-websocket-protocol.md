...
- the shared `moveToken`, `turnToken`, `spawnToken`, `deleteToken`, `sendOutPokemon`, `modifyHp`, `modifyCombatStages`, `modifyConditions`, `useMove`, `setInitiative`, `nextInitiative`, and `previousInitiative` command payload contracts, validators, server-side application use cases, sender acknowledgement/rejection, and same-session `tokenMoved`/`tokenTurned`/`tokenSpawned`/`tokenDeleted`/`pokemonSentOut`/`hpModified`/`combatStagesModified`/`conditionsModified`/`moveUsed`/`initiativeUpdated` patch broadcasts now apply through `/api/sessions/socket`; move/turn also have explicit map-view client dispatch and client-local optimistic visual reconciliation.
