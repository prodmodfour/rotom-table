...
The Breeding Workshop, Project wizard/guidance, same-owner rank-choice/confirmation flow, current Project/Egg activity cards, and the role-projected hatch decision/special-review/accepted-reveal flow are available at `/breeding`, and the durable server runtime through BR-069 remains operational. BR-073 confirmation creates a revision-zero Project through the existing initial-progress transaction; BR-074 cards read current campaign progress, history, recovery, and transfer handoffs without creating a second mechanics path; BR-075 delegates hatch mutations to the established offer, special, and atomic-completion authorities; BR-076 adds current team/Box choice, capacity explanations, accepted linkage, and sheet navigation. Complete cross-owner consent UX and final interaction acceptance remain incremental BR-077 and BR-078 work. Do not describe the Workshop as the complete workflow until the implementation ledger is complete. Do not represent edits to `eggMoves`, `inheritedMoves`, Gender, Nature, Abilities, `babyTemplate`, inventory, or map metadata as a completed breeding or hatch operation.
