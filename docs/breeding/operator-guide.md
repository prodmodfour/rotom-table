...
The Breeding Workshop, Project wizard/guidance, same-owner rank-choice/confirmation flow, current Project/Egg activity cards, and the role-projected hatch decision/special-review/accepted-reveal flow are available at `/breeding`, and the durable server runtime through BR-069 remains operational. BR-073 confirmation creates a revision-zero Project through the existing initial-progress transaction; BR-074 cards read current campaign progress, history, recovery, and transfer handoffs without creating a second mechanics path; BR-075 delegates hatch mutations to the established offer, special, and atomic-completion authorities. Child destination/navigation, complete cross-owner consent UX, and final interaction acceptance remain incremental BR-076 through BR-078 work. Do not describe the Workshop as the complete workflow until the implementation ledger is complete. Do not represent edits to `eggMoves`, `inheritedMoves`, Gender, Nature, Abilities, `babyTemplate`, inventory, or map metadata as a completed breeding or hatch operation.
...
A currently eligible Egg card opens the BR-075 hatch dialog. Every step calls `POST /api/breeding/hatch` with current selectors and one closed intent. Mutation buttons require explicit confirmation; the server currently selects the authoritative Box offer until BR-076 adds destinations. A normal persisted special roll advances to completion. A triggered roll shows owners only **Waiting for GM**; an authenticated GM receives three bounded nonmechanical classifications and must select one before completion. The final reveal is accepted only after the atomic hatch transaction and is not a preview or editable child draft.
