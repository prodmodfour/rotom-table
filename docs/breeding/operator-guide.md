...
The Breeding Workshop, Project wizard/guidance, and same-owner rank-choice/confirmation flow are available at `/breeding`, and the durable server runtime through BR-069 remains operational. BR-073 confirmation creates a revision-zero Project through the existing initial-progress transaction; Project/Egg cards, cross-owner consent UX, and later lifecycle controls remain incremental BR-074 through BR-078 work. Do not describe the Workshop as the complete workflow until the implementation ledger is complete. Do not represent edits to `eggMoves`, `inheritedMoves`, Gender, Nature, Abilities, `babyTemplate`, inventory, or map metadata as a completed breeding or hatch operation.
