...
The page uses the existing Rotom design-system tokens required by `DESIGN.md`. BR-078's executable matrix covers 320, 390, 768, and 1440 CSS-pixel widths; 200% and 400% zoom/reflow; keyboard and screen-reader semantics; 44-pixel touch targets; reduced motion; and shared-display table distance. Cards and controls collapse to one column without two-dimensional essential-content scrolling, long human labels wrap, and dialogs remain internally scrollable inside the dynamic viewport.

Opening the Project wizard focuses its heading, each step change focuses the new visible step heading, and closing restores the opening control. The transfer and hatch dialogs trap Tab/Shift+Tab, use visible heading names, support safe Escape close, and restore their opening controls. Loading and accepted changes are polite statuses, failures are alerts, native progress has complete campaign-minute labels, and semantic state always includes text rather than colour or motion alone. Reduced motion removes hatch reveal and incidental transitions while preserving accepted-state copy.

The complete component and browser acceptance plus operational caveats are recorded in `docs/breeding/accessibility-responsive-and-table-distance.md`. Player and GM procedures are in `docs/breeding/gm-and-player-guide.md`; exact HTTP schemas are in `docs/breeding/api-reference.md`. BR-079's Nuxt and production-Nitro Playwright suites consume the BR-078 matrix without weakening it. Desktop Chromium and Pixel 7 baselines cover the Workshop and hatch dialog; axe, selector-only requests, simultaneous GM/player privacy, explicit reconnect, Profile-snapshot authority, reduced motion, touch sizing, and narrow-width overflow are executable release checks.
...
The reviewed BR-070 shell contract is `data/breeding-automation/workshop-presentation-contract.json`, the BR-071 wizard contract is `data/breeding-automation/project-wizard-presentation-contract.json`, the BR-072 guidance contract is `data/breeding-automation/project-guidance-presentation-contract.json`, the BR-073 choice/creation contract is `data/breeding-automation/project-choices-presentation-contract.json`, the BR-074 card contract is `data/breeding-automation/workshop-activity-presentation-contract.json`, the BR-075 hatch-flow contract is `data/breeding-automation/hatch-workflow-presentation-contract.json`, the BR-076 destination contract is `data/breeding-automation/hatch-destination-presentation-contract.json`, the BR-077 consent contract is `data/breeding-automation/consent-workflow-presentation-contract.json`, the BR-078 interaction matrix is `data/breeding-automation/workshop-interaction-acceptance.json`, and the BR-079 production-browser evidence contract is `data/breeding-automation/workshop-browser-acceptance.json`. Focused evidence lives in the shared-contract, server-projection, current-reference, API-route, composable, component/Nuxt accessibility, Playwright, navigation, and Profile-route-guard tests named by those contracts.
