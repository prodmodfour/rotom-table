...
BR-070 establishes the Workshop shell, ownership context, navigation, and safe loading/empty/error states. BR-071 adds the transient Project wizard for destination, Breeder, parents, consent status, and campaign timeline. BR-072 adds closed compatibility and unavailable explanations, current source contributions, and bounded GM diagnostics. BR-073 adds rank-authority presentation, campaign settings, server-issued setup choices, explicit confirmation, and same-owner Project creation through the existing durable initial-progress path. BR-074 adds owner/GM Project and Egg cards with exact campaign progress, bounded lifecycle history, system recovery state, and transfer handoffs. BR-075 adds the role-projected hatch decision, private bounded GM special adjudication, accepted completion, and child reveal over the existing durable hatch authorities. Destination selection/navigation, complete private cross-owner consent UX, and final interaction acceptance remain owned by BR-076 through BR-078.
...
BR-075 opens a hatch flow only from a currently authorized Egg card. Its request contains the exact selected Profile or GM context, owner Trainer and Egg selectors, expected Egg revision, one closed intent, at most one opaque special option ID, and explicit confirmation for every mutation. It cannot contain a command, destination fact, roll, trait, child patch, roster count, read set, receipt, provider, or mechanics claim. The server rebuilds current role/Profile control, owner Trainer, Egg, campaign clock/options, app-owned references, hatch offer, complete read sets, and authorization receipts on each step.

The flow delegates rather than duplicating mechanics:

1. **Begin hatch** consumes the existing current hatch offer. BR-075 uses the server-owned Box default until BR-076 adds destination selection. The established transaction persists exactly one d100 before applying it; retries never redraw.
...
The dialog has a labelled modal boundary, native GM radio group, separate confirmation buttons, 44-pixel controls, visible focus, Escape close, status/error/transition announcements, a mobile bottom-sheet layout, and a reveal animation disabled by reduced-motion preference. BR-076 owns team/Box selection, capacity explanation, destination navigation, and Trainer/sheet links.
