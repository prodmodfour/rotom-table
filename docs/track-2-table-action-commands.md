# Track 2 table action command reference

This guide documents the server-authoritative table action commands that landed in Track 2 chunk 06. It is the reviewer-facing inventory for HP, combat-stage, condition, move/action, initiative, hazard, field-effect, and terrain command flows after tickets 060-068.

The architecture remains the locked Track 2 model: a GM-hosted local server, explicit session-host runtime flag, WebSocket command transport, session-local identity, server-owned revisions, local JSON snapshots, and small same-session patches instead of live-client whole-map autosaves.
...
The following commands are GM-only in Track 2. Player actors are rejected even if they can see the map.
...
The table action commands follow the Track 2 conflict model:
...
The chunk 06 command flows are covered by focused shared-contract, server-use-case, and WebSocket dispatch tests, including:
...
For the surrounding transport and storage details, see the [Track 2 session protocol](track-2-session-protocol.md), [Track 2 WebSocket protocol](track-2-websocket-protocol.md), and [Track 2 session storage](track-2-session-storage.md) guides.
