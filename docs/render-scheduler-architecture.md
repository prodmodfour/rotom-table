...
- CSS3D dirty tracking starts dirty for initial paint, marks camera/resize/HUD/targeting/pointer-related invalidations as CSS-visible, and marks token-motion animation frames because token HUD transforms follow moving/lifting tokens. Same-frame HUD and targeting overlay update helpers also report output changes for HP bars, elevation badges, reticles, feedback, and attack-of-opportunity buttons before the CSS dirty state is consumed. WebGL-only animation frames such as weather, field effects, sprite animation, or texture-loading frames skip `CSS3DRenderer.render()` when no CSS-visible state is dirty.
