...
| `move-vfx-animation` | The transient move VFX renderer has active effect instances that still need scheduler-driven frames. Most move VFX are WebGL-only; optional CSS3D badge instances separately report CSS dirtiness through `needsCss3DFrame()` so they do not make every move-VFX frame render CSS3D. |
...
During a scheduled frame, `stepIsometricAnimationFrame()` advances the move VFX renderer only when a renderer is present and reports active work. It receives the same frame timestamp, delta, elapsed clock time, camera, optional token-render-object map, layer visibility, and primitive-level reduced-motion hint as the rest of the isometric animation path, then completion pruning happens before the WebGL render. When `MoveVfxRenderer.needsAnimationFrame()` transitions from true to false, the grid emits a runtime-only settle signal so the map page prunes expired queue entries with the renderer-clock timestamp; this avoids stale VFX input without a timer, interval, independent RAF loop, or persistence. The map page derives that hint from the client-only `prefers-reduced-motion: reduce` media query and updates it on media-query changes without touching SSR, saved map data, or scheduler ownership. Delayed/staggered move VFX count as active work until their `createdAtMs + startOffsetMs + durationMs` lifecycle finishes, but their per-event groups stay hidden until the effective start frame so they do not appear early. The scheduler still settles after the renderer's `needsAnimationFrame()` returns false. The local browser move-animation enable/disable preference clears the runtime queue and passes an empty event list to the renderer when disabled, so disabled move VFX cannot keep `move-vfx-animation` active. Token-object sync also resyncs the move VFX renderer; instances track the render objects that existed when their anchors were created and dispose/complete if those render objects disappear, so token deletion or map reload cleanup cannot leave a stale move-VFX continuation source alive. CSS3D badge primitives do not add a separate continuation source; while they are active, `needsCss3DFrame()` causes the same frame step to mark CSS3D dirty before the dirty tracker is consumed. When the render debug overlay is explicitly enabled, the grid samples the renderer's `debugSnapshot()` so developers can inspect active move VFX count, CSS3D-badge activity, and whether that source is keeping scheduler frames alive; this sampling is skipped on normal production paths.
