...
Reviewed MoveSpecs can turn one requirement-addressed candidate set into a durable item choice. A bounded filter selects allowed item-reference kinds, canonical item IDs, and minimum quantity; the server combines those candidates with closed reviewed destination categories and optional explicit-none/pass branches. Stable option IDs hash logical item and destination identity without exposing it or binding to a mutable revision. The response body returns only the chosen option ID. Inventory changes remain outside this choice primitive: a choice records and revalidates intent, while typed mutation operations own later quantity and destination writes.

Server item reducers can now project equip, unequip, transfer, swap, stack decrement, consume, destroy, consumed-item restoration, and ground-item add/remove operations into typed CAS plans. All operations run against detached authoritative snapshots and produce at most one revision advance per touched map, sheet, or group-inventory document, even when several operations share that resource. Conservative operations must have a zero canonical quantity delta; only explicitly typed decrement, consume, destroy, and restore-consumed operations may reduce or increase quantity, and each non-zero delta is retained in the private operation audit. Immediate `resolveMove` orchestration now commits any typed group-inventory state change with the map, sheets, terminal operation result, and authorized resource realtime rows in one SQLite transaction. Every write must match a reviewed group-inventory read and an explicit `groupInventory:<slug>:inventory` command scope; stale or mismatched resources roll the complete move back, and duplicate operation delivery applies no second write.
