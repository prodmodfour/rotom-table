...
The Knock Off planning foundation composes those seams without persistence: only a positive server-resolved damage outcome may inspect the target's Pokémon Held Items or Trainer Accessory Slot, one legal candidate becomes a typed ground-item plan, and multiple candidates become a private actor-owned option set. Miss, immunity, zero damage, and itemless branches return traced no-item outcomes. The reviewed v2 draft remains unselected until the separate durable persistence and conformance ticket is complete, so live play continues to expose Knock Off as assisted legacy automation in the interim.
