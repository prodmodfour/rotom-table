...
The Knock Off continuation composes those seams at the interpreter boundary: only a positive server-resolved, non-immune damage outcome may inspect the target's Pokémon Held Items or Trainer Accessory Slot. No legal candidate finishes as a traced no-item branch, one candidate becomes a deterministic typed ground-item plan, and multiple candidates suspend behind one actor-owned durable window. The private pending record retains the complete item/sheet read set and server-owned candidate bindings while authorized views expose only stable option IDs and safe item/destination presentation; damage, usage, and item mutations remain deferred. Resume accepts only an option ID, replays the stored roll ledger, and revalidates the selected candidate against fresh authoritative resources before returning one typed terminal move/item plan. This continuation boundary performs no terminal repository write; atomic application and catalog certification remain separate persistence work.
