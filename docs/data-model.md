...
Every normalized map has encounter-state schema version 1. The state contains explicit side membership, typed effects, counters, turn/action resources, battlefield zones, map-ground items, structured move history, and bounded public pending-resolution summaries. Each collection has strict count, identifier, JSON-depth, and numeric limits. Effects and zones retain source operation/move/placement provenance, recipients, duration boundaries, stacking/charge policies, and cleanup rules, so restart and replay do not need to reconstruct mechanics from prose or UI state.

Legacy hazards/global fields, temporary HP, move usage, and prompt metadata remain dual-readable only for the documented compatibility window. Native encounter entries shadow matching compatibility projections exactly once. A shared identity with contradictory duration or activation state is rejected rather than preferred. Accepted mechanics write one canonical encounter-state value; renderer-facing field/hazard lanes are derived compatibility projections, not independent mechanics authority.

The authoritative live-table snapshot includes the whole parsed encounter state. Accepted move patches include exact previous/current encounter values, and snapshot adoption replaces changed lanes in place. `data/maps` JSON export retains active encounter rules state but deliberately terminally abandons resumable prompts: exported maps clear public pending summaries and `data/move-automation-abandoned-pending-resolutions.json` records the IDs/statuses that were abandoned. A byte-for-byte SQLite backup remains the resumable backup because it includes private `pending_move_resolutions` rows. Future encounter-state schema versions fail import/export rather than being silently downgraded.

## Pending move resolutions

Ability invocation declarations use two private SQLite stores. `ability_declaration_offers` keeps the
canonical full offer, request hash, map/actor revision identity, lifetime, and optional consumed
intent hash; clients receive only its hash-bound controller projection. `ability_resolution_ops`
keys terminal results by intent ID and stores the exact intent hash/JSON, redacted accepted result,
result revision, and private audit JSON. Exact retries return the stored result without another map
or sheet revision, while changed reuse conflicts. Both tables are included in normal SQLite backup;
JSON map export never turns their private mechanics into public map metadata. Live-table snapshot
schema v2 carries only a revision-matched controller capability projection and omits capabilities
for placements the principal cannot control.

A MoveSpec execution suspended for an authorized choice or reaction is stored in `pending_move_resolutions`, separately from terminal command results in `live_play_ops`. Each row contains canonical strictly parsed resolution JSON plus indexed status, originating map and operation identity, a repository CAS revision, creation/update timestamps, and an optional link to the eventual terminal operation. Resolution IDs are primary keys and each originating map/operation pair is unique; declaration orchestration derives the ID from the exact command hash, so same-body retries resolve the existing row while changed material under the same `opId` conflicts. The private row, its bounded encounter-state summary, any explicitly typed pay-phase declaration HP cost, and reviewed encounter-resource spends through the reached phase commit atomically after full read-set validation; later-window spends are folded into the durable declaration compensation plan. The non-terminal pending acknowledgement is not a `live_play_ops` terminal. Continuation kinds distinguish suspended MoveSpec execution, reviewed ability follow-ups, and current post-action Attack of Opportunity responses. The private durable record owns resource reads, rolls, audit trace, response ownership, and stable option IDs; map encounter state exposes only its bounded public summary/ID. A movement option may carry a strict server-issued `movement-destination` or `movement-direction` selection (set ID, bounded destination, and reviewed direction where applicable). That typed selection is available only through the authorized response query for battlefield targeting, is never copied into the response command, and is regenerated/revalidated by the movement oracle on resume. A durable item option similarly retains its exact private item reference, owning revision, reviewed requirement, and destination branch in the pending row. Its authorized view exposes only an opaque option ID plus canonical item and destination presentation; row IDs, owner slugs, quantities, sections, and revisions are removed. Resume reloads the reviewed item scope and re-enumerates the selected option from current authoritative documents before continuing.
...
Before resolution, the server loads only item locations named by reviewed move requirements: the controlled actor's equipped state or trainer inventory, explicit targets' equipped state, reviewed shared group-inventory sections, or the current map's ground items. Stored labels resolve through canonical item rules data; unknown labels and unavailable quantities do not become legal candidates. Parameterized Type Plate, Drive, and Memory Disc identities retain their selected elemental type rather than collapsing to a generic item row, and the reviewed Natural Gift Berry table supplies stable Berry identity, type, and Damage Base without parsing rules prose. Every consulted sheet or group-inventory revision enters the private resolution read set, including an empty/invalid candidate source, and is revalidated in the commit transaction. Item candidates remain server-only and inaccessible target sheet updates are omitted from player HTTP responses, so inspecting move legality cannot reveal another participant's private inventory.

Ability automation persists temporary server-owned state inside the map's versioned `encounterState`: `abilityUsage` (Scene spends), `abilityTiming` (round/turn/cooldown windows and receipts), `abilityEffectLifecycle` (effect-duration links), `abilityOwnedState` (marks, counters, token pools, modes, and forms), `abilityEventReceipts` (derived-event replay suppression), `abilityReactionAvailability` (the round-shared Interrupt/Reaction pool), `abilityEntities` (versioned temporary anchors, decoys, objects, and subordinate encounter entities), and `abilityTransformations` (immutable form/copy mechanics plus authorization-separated presentation snapshots). Daily spends remain on each Pokémon/trainer sheet as `abilityUsage`. All envelopes are bounded, strict JSON with source/operation identities and are included by normal map/sheet export.

Ice Face ownership and active Illusion appearance use typed capability effects rather than inferring mechanics from a display form or sprite. Ice Face ownership is reconciled after every authoritative HP path: depletion or an increase not explicitly sourced from Ice Face removes its marker, and unrelated Temporary HP can never establish the form. Illusion marks remain in `abilityOwnedState`; its renderer-only projection can reference a server-snapshotted creature appearance or bounded object cell/material sprite, while authoritative Move context always keeps the user’s original creature-rule token. Substitute likewise uses a canonical capability marker instead of inventing a condition. Normal and post-Interrupt-resumed token movement run the battlefield-zone lifecycle before persistence; resulting map, encounter, and HP sheet writes share the command transaction, and accepted patches retain the pre-resource turn lane so reconnecting clients can apply them without speculative reconciliation.

Map-ground items are persisted in the map's versioned `encounterState.groundItems` collection. Each bounded record stores a stable map-local ID, canonical item ID and name, quantity, integer cell/height, exact source resource revision, source operation ID, and nullable side/previous-placement hints. Those hints are provenance and presentation only: they do not grant control or replace map ownership. Normalization rejects duplicate IDs, malformed or oversized payloads, unknown side hints, and positions outside map bounds. The isometric scene renders a generic selectable marker and exposes only its stable ID through pointer interaction; typed item mutations remain responsible for authoritative state changes.

Reviewed MoveSpecs can turn one requirement-addressed candidate set into a durable item choice. A bounded filter selects allowed item-reference kinds, canonical item IDs, Trainer equipment slots, and minimum quantity; the declaration separately assigns response ownership to the mechanics recipients or actor and chooses a fail-closed reject or canonical no-op policy for an empty legal set. The server combines eligible candidates with closed reviewed destination categories and optional explicit-none/pass branches. Stable option IDs hash logical item and destination identity without exposing it or binding to a mutable revision. The response body returns only the chosen option ID. A later reviewed `item` operation binds that request and destination branch to mechanics; the interpreter never accepts an item reference, quantity, destination, or mutation from the response body.

Knock Off composes those seams end to end: only a positive server-resolved, non-immune damage outcome may inspect the target's Pokémon Held Items or Trainer Accessory Slot. No legal candidate finishes as a traced no-item branch, one candidate becomes a deterministic typed ground-item plan, and multiple candidates suspend behind one actor-owned durable window. The private pending record retains the complete item/sheet read set and server-owned candidate bindings while authorized views expose only stable option IDs and safe item/destination presentation; damage, usage, and item mutations remain deferred while that window is open. Resume accepts only an option ID, replays the stored roll ledger, and revalidates the selected candidate against fresh authoritative resources. The immediate or resumed terminal plan removes exactly that item, adds its provenance-linked ground record, applies damage and usage, stores the operation result, and appends realtime rows in one SQLite transaction. A stale resource or persistence failure rolls the whole terminal outcome back, and duplicate declaration or response delivery returns the durable pending or terminal result without removing or creating the item again.

Server item reducers project equip, unequip, transfer, swap, stack decrement, consume, destroy, consumed-item restoration/effect replay, ground-item add/remove, item suppression, and digestion-buff storage/use into typed CAS plans. The high-level item interpreter maps give, steal, held-item swap (including one empty endpoint), knock, throw, consume, restore, destroy, suppress, store-buff, and digest-buff behavior onto those bounded mutations. Missing optional possession candidates produce an explicit traced no-op, occupied legal destinations produce a traced prevented outcome, and mismatched ownership or equipment identity rejects the plan. Restore operations resolve only a strict private consumed-item record loaded by the server; effect replay conserves physical quantity, while a physical restore consumes that recorded availability and must target an unoccupied reviewed destination. Suppression is a typed encounter effect and item-specific suppression retains only opaque logical bindings, not private owner identities or an unequip mutation. Digestion storage consumes exactly one canonical item while recording source provenance, and trading in a bounded stored buff clears only that selected entry in the same typed sheet plan; a bounded one-based slot preserves occurrence identity when canonical item IDs repeat. The legacy single digestion field remains readable; Gluttony-aware sheets may hold a strict maximum of three names in `digestionFoods`. Honey Paws adds only `honeyPawsFood`, a separate fourth selectable occurrence that must contain canonical Leftovers and does not consume normal capacity. The server writes that slot only when a scene-bounded one-shot preparation matches the same currently effective Honey Paws instance, consumes the preparation atomically with the held Honey, and otherwise preserves ordinary Honey behavior. Each successful trade adds one bounded Scene stack to the opaque sheet-bound digestion marker, allowing effective Gluttony to enforce three uses while ordinary users remain limited to one. Effective Handyman shares the two-item held destination and durable affected-item selection contract with Delivery Bird; both are revalidated at the item mutation boundary from exact manifest-selected effective runtimes. Effective Harvest adds separate sheet-bound encounter markers for the current round/turn trade and a scene-duration tails stop. A digest mutation may retain its exact selected Berry occurrence only when it carries one server-ledger-bound heads result or authoritative Sunny Weather evidence; the reducer revalidates the Berry family, effective runtime, suppression, actor-scoped weather, per-turn gate, and marker state before committing the item/map plan.

The bounded expression language can query physical holding state independently from item contributions, then query a reviewed equipped requirement or stored digestion buff for eligibility, family, Fling category/power/effect, elemental move type, or Natural Gift Damage Base. Scalar contribution queries reject ambiguous candidate sets so a move must use a durable item choice instead of silently selecting one. Static/triggered contributions pass through Magic Room and benefit-suppression overlays; activated/consumable contributions pass through use-suppression overlays. Physical `holding-item`/`holding-nothing` checks bypass those overlays because suppression never unequips an item. These primitives cover Acrobatics, Natural Gift, Fling, Judgment, Techno Blast, Multi-Attack, and Poltergeist without client-provided item mechanics.

All item operations and queries run against detached authoritative snapshots and produce at most one revision advance per touched map, sheet, or group-inventory document, even when several operations share that resource. Conservative operations must have a zero canonical quantity delta; only explicitly typed decrement, consume, destroy, and physical restore-consumed operations may reduce or increase quantity, and each non-zero delta is retained in the private operation audit. Immediate `resolveMove` orchestration commits typed item state with the map, sheets, terminal operation result, and authorized resource realtime rows in one SQLite transaction. Every write must match the reviewed read set and command scopes; stale or mismatched resources roll the complete move back, and duplicate operation delivery applies no second write.
