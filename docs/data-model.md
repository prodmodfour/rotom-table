...
Before resolution, the server loads only item locations named by reviewed move requirements: the controlled actor's equipped state or trainer inventory, explicit targets' equipped state, reviewed shared group-inventory sections, or the current map's ground items. Stored labels resolve through canonical item rules data; unknown labels and unavailable quantities do not become legal candidates. Every consulted sheet or group-inventory revision enters the private resolution read set, including an empty/invalid candidate source, and is revalidated in the commit transaction. Item candidates remain server-only and inaccessible target sheet updates are omitted from player HTTP responses, so inspecting move legality cannot reveal another participant's private inventory.
