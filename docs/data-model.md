...
Reusable shop tables are campaign-level SQLite state in the `shop_tables` table. The current shop APIs let GMs create, save, delete, list, and load authoritative shop documents with revisions and timestamps; player read routes only expose shops that are both open and player-visible. The `/shops` player library renders those open player-visible shopfronts as links to `/shops/<slug>` and omits closed or hidden shops instead of presenting disabled private setup rows. The `/shops/<slug>` shopfront loads the same authoritative document, renders read-only item names, prices, stock, max-per-purchase limits, and player descriptions, redacts GM notes, and keeps buy controls disabled until live-play checkout commands are implemented. Checkout operation history lives in `shop_checkout_ops` with `op_id`, `shop_slug`, command JSON/hash, terminal result JSON, result revision, and creation time; this idempotency storage is shop-scoped and does not require a map slug for shop-page-origin checkout commands. `data/shops/` is an explicit export/interchange hierarchy only, and exported shop documents include `revision` and `updatedAt` so maintenance backups do not drop checkout-critical stock metadata.
