...
This is the WebSocket-focused reference for the Live session transport. It complements the broader [live session protocol](live-session-protocol.md) and the [Live session table action command reference](live-session-table-action-commands.md) by collecting the live socket route, message examples, heartbeat/reconnect behaviour, command-result flow, session isolation rules, and named Cloudflare Tunnel expectations in one place.

This document describes the transport slice that exists after the WebSocket transport implementation:
...
- the shared `moveToken`, `turnToken`, `spawnToken`, `deleteToken`, `sendOutPokemon`, `modifyHp`, `modifyCombatStages`, `modifyConditions`, `useMove`, `useManeuver`, `useAbility`, `useOrder`, `setInitiative`, `nextInitiative`, `previousInitiative`, `placeHazard`, `removeHazard`, `setFieldEffect`, `removeFieldEffect`, `tickFieldEffectDurations`, `buildTerrainVoxel`, and `removeTerrainVoxel` command payload contracts, validators, server-side application use cases, sender acknowledgement/rejection, and same-session `tokenMoved`/`tokenTurned`/`tokenSpawned`/`tokenDeleted`/`pokemonSentOut`/`hpModified`/`combatStagesModified`/`conditionsModified`/`moveUsed`/`maneuverUsed`/`abilityUsed`/`orderUsed`/`initiativeUpdated`/`hazardsUpdated`/`fieldEffectsUpdated`/`terrainVoxelsUpdated` patch broadcasts now apply through `/api/sessions/socket`; move/turn also have explicit map-view client dispatch and client-local optimistic visual reconciliation.

## Route, runtime gate, and URL shape

Session WebSockets use the same explicit host opt-in as the Live session HTTP session endpoints:

```bash
ROTOM_ENABLE_SESSION_HOST=1
```

For local development hosting, use `npm run dev:session:lan` for same-Wi-Fi play or `npm run dev:session:tunnel` before starting a named Cloudflare Tunnel; see [live session host runtime scripts](live-session-host-runtime.md) for the safe bindings and manual equivalents.
...
Unknown frames, invalid JSON, unsupported message types, wrong schema versions, missing session scope, pre-auth heartbeat/command messages, and cross-session messages fail closed with a safe server `error` frame when possible. Valid authenticated `moveToken`, `turnToken`, `spawnToken`, `deleteToken`, `sendOutPokemon`, `modifyHp`, `modifyCombatStages`, `modifyConditions`, `useMove`, `useManeuver`, `useAbility`, `useOrder`, `setInitiative`, `nextInitiative`, `previousInitiative`, `placeHazard`, `removeHazard`, `setFieldEffect`, `removeFieldEffect`, `tickFieldEffectDurations`, `buildTerrainVoxel`, and `removeTerrainVoxel` frames are dispatched to server-authoritative command handlers; authenticated command types outside the implemented Live session set receive `unsupported-message` without closing the socket.
...
The client sends a command envelope inside a client `command` message. The envelope includes the authenticated actor, an `opId`, `baseRevision`, scope lanes, and a JSON payload. The `moveToken` example below matches the shared movement payload validator and is dispatched by the WebSocket route to the server-side application use case; `turnToken` uses the same envelope with `field: "facing"` and payload `{ "tokenId": string, "facing": "south-east" | "north-east" | "north-west" | "south-west" }`. GM-only `spawnToken` uses `field: "spawn"` and a placement payload; GM-only `deleteToken` uses `field: "delete"` and a token ID payload. `sendOutPokemon` includes both a trainer `field: "sendOut"` token scope and a spawned Pokémon `field: "spawn"` token scope, with payload `{ "trainerTokenId": string, "pokemonSlug": string, "tokenId": string, "position": { "x": number, "y": number, "z": number }, "facing"?: direction }`. `modifyHp` uses a token `field: "hp"` scope plus an optional matching sheet `field: "hp"` scope, with payload `{ "tokenId": string, "currentHp": number, "injuries"?: number }`; the server updates the authoritative placed sheet, advances the session/map revision, and broadcasts `hpModified` without sending a whole sheet or whole map. `modifyCombatStages` uses token/sheet `field: "combatStages"` scopes and an absolute stage payload `{ "tokenId": string, "stages": { "atk": number, "def": number, "satk": number, "sdef": number, "spd": number, "acc": number } }`; each stage must be an integer from -6 to +6, and accepted updates broadcast `combatStagesModified`. `modifyConditions` uses token/sheet `field: "conditions"` scopes and payload `{ "tokenId": string, "action": "add" | "remove" | "replace", "conditions": string[] }`; replace may clear to an empty list, add/remove require at least one condition entry, and accepted updates broadcast `conditionsModified`. `useMove` uses token/sheet `field: "moveUsage"` scopes and payload `{ "tokenId": string, "moveName": string }`; the server resolves the move on the placed sheet, spends EOT/Scene usage on authoritative map state, spends Daily usage on the sheet file, and broadcasts `moveUsed` without sending a whole sheet or whole map. `useManeuver`, `useAbility`, and `useOrder` use token/sheet `field: "maneuver"`, `field: "ability"`, or `field: "order"` scopes and payloads with a non-empty action name plus optional `targetTokenId`; accepted updates append authoritative map metadata, sheet ability/automation updates, or active-order effects and broadcast `maneuverUsed`/`abilityUsed`/`orderUsed` without sending a whole sheet or whole map. GM-only initiative commands use an initiative `field: "initiative"` scope: `setInitiative` can set token initiative scores, active token, and/or round, while `nextInitiative` and `previousInitiative` derive the next active token and round from authoritative placement initiative values; accepted updates broadcast `initiativeUpdated`. GM-only hazard commands use a hazard `field: "hazards"` scope: `placeHazard` adds or layers a supported hazard kind at a map cell, while `removeHazard` removes one kind or every hazard at that cell; accepted updates broadcast `hazardsUpdated` without sending a whole map. GM-only field-effect commands use a field-effect `field: "fieldEffects"` scope: `setFieldEffect` sets or updates Weather, Terrain field-effect, or Room entries with optional duration/source data, `removeFieldEffect` removes one kind, one category, or all field effects, and `tickFieldEffectDurations` decrements finite durations and removes expired effects; accepted updates broadcast `fieldEffectsUpdated` without sending a whole map. GM-only terrain voxel commands use a cell-specific terrain scope such as `field: "voxel:1,0,2"`: `buildTerrainVoxel` adds or replaces one map voxel with material/style flags, while `removeTerrainVoxel` removes one voxel cell; accepted updates broadcast `terrainVoxelsUpdated` with Track 1 terrain renderer invalidation reasons and without sending a whole map. The map page uses these command paths only when it is explicitly opened in session mode with a remembered session identity, for example `/maps/viridian-gym?session=1`; otherwise local-first token dragging, turning, and table controls still mutate/autosave through existing local handlers as before. `useSessionMapSceneCommands` routes MapScenePanel-originated session actions for token delete/send-out, HP, combat stages, conditions, move/action use, maneuver/ability/order use, initiative, hazards, field effects, and terrain through the shared session WebSocket command dispatcher.
...
5. Reject stale same-resource move/turn/HP/table-action commands when a newer accepted change for the same token, sheet field, initiative lane, hazard lane, field-effect lane, or terrain voxel cell exists, or when the server lacks enough recent command history to prove the old base revision is safe; the rejection includes the current authoritative token, sheet, initiative, hazard, field-effect, or terrain state. Valid non-conflicting effects then apply to server-owned state after command-specific rules checks such as map bounds, blocking voxels, occupied token/terrain cells, valid facing values, GM-only spawn/delete/initiative/hazard/field-effect/terrain authority, duplicate placement IDs, missing delete targets, sheet identity matches, send-out trainer control, trainer current-team ownership, Poké Ball throw range, sheet HP persistence, move-frequency availability, Daily move-usage sheet persistence, valid initiative active/round transitions, valid hazard placement/removal cells, valid field-effect categories, and Track 1-compatible terrain material/build rules.
...
Implemented session command boundary: `moveToken` and `turnToken` dispatch are live on both the WebSocket route and explicit map-view session mode, with client-local optimistic token-position/facing overrides that confirm from `commandAck` or `tokenMoved`/`tokenTurned` patch frames and roll back or reconcile to returned `currentState.position`/`currentState.facing` on `commandReject`. The optimistic overrides affect rendering/session controls only; they do not mutate the persisted map document or trigger whole-map autosave. `spawnToken`, `deleteToken`, `sendOutPokemon`, `modifyHp`, `modifyCombatStages`, `modifyConditions`, `useMove`, `useManeuver`, `useAbility`, `useOrder`, `setInitiative`, `nextInitiative`, `previousInitiative`, `placeHazard`, `removeHazard`, `setFieldEffect`, `removeFieldEffect`, `tickFieldEffectDurations`, `buildTerrainVoxel`, and `removeTerrainVoxel` are dispatched through the WebSocket route, broadcast the small patch events listed above, and are routed from explicit session-mode MapScenePanel interactions by `useSessionMapSceneCommands` where the current UI exposes those actions. Command types outside the implemented Live session set still receive a server `error` with `code: "unsupported-message"` instead of mutating state.
...
The patch is a small authoritative event, not a whole-map autosave from a browser. Accepted `moveToken`, `turnToken`, `spawnToken`, `deleteToken`, `sendOutPokemon`, `modifyHp`, `modifyCombatStages`, `modifyConditions`, `useMove`, `useManeuver`, `useAbility`, `useOrder`, `setInitiative`, `nextInitiative`, `previousInitiative`, `placeHazard`, `removeHazard`, `setFieldEffect`, `removeFieldEffect`, `tickFieldEffectDurations`, `buildTerrainVoxel`, and `removeTerrainVoxel` commands send `commandAck` to the submitting socket and fan out small token/sheet/initiative/hazard/field-effect/terrain patches to authenticated peers in the same session; the sender can receive both frames. Spawn patches include the authoritative placement that was appended; delete patches include the removed placement and whether the active initiative pointer was cleared; send-out patches include the source trainer token, Pokémon sheet slug, and spawned placement; HP patches include the previous/current HP and Injury counts for the placed sheet; combat-stage patches include previous/current absolute Atk/Def/SAtk/SDef/Spd/Acc stage maps; condition patches include previous/current normalized condition lists; move-used patches include the resolved move name/key, frequency bucket, previous usage, and current usage summary; maneuver/ability/order patches include the resolved action name, optional target token, log lines, ability category or active-order effect metadata; initiative patches include previous/current active ID, round, and map-local token initiative entries; hazard patches include the changed cell plus previous/current, placed, and removed hazard entries for that cell; field-effect patches include previous/current Weather, Terrain, and Room arrays plus the changed category/kind or tick amount; terrain patches include the changed voxel cell, previous/current voxel values, build/remove details, and renderer invalidation reasons so clients keep Track 1 terrain, movement-preview, build-preview, and hazard-preview invalidation intact.
...
Normal command rejections use `reason: "invalid"`, `"unauthorized"`, `"stale"`, or `"conflict"`. For `moveToken`, stale same-token rejections include `baseRevision`, `changedScopes`, and the current authoritative token position/revision so clients can reconcile optimistic state. For `turnToken`, stale same-token facing rejections include the same revision/scope metadata plus the current authoritative facing. For `modifyHp`, stale same-token/same-sheet HP rejections include the current authoritative HP/Injury state. For `modifyCombatStages`, stale same-token/same-sheet combat-stage rejections include the current authoritative stage map. For `modifyConditions`, stale same-token/same-sheet condition rejections include the current authoritative condition list. For `useMove`, stale same-token/same-sheet move-usage rejections include the current usage summary, and conflicts cover unknown sheet moves plus unavailable EOT/Scene/Daily frequencies. For `useManeuver`, `useAbility`, and `useOrder`, stale same-token/same-sheet rejections include current action metadata counts; conflicts cover missing sheets/targets, unknown actions, passive or unautomated abilities, already-active sheet abilities, and non-trainer order users. For initiative commands, stale same-lane rejections include the current authoritative active ID, round, and initiative score entries. For hazard commands, stale same-lane rejections include the changed map cell and current authoritative hazards on that cell. `spawnToken`/`deleteToken` reject player actors as unauthorized and return conflicts for duplicate spawn IDs, missing delete targets, sheet-identity mismatches, and blocked/out-of-bounds/occupied spawn cells. Initiative commands reject player actors as unauthorized and return conflicts for missing maps, missing/duplicate token entries, and no-op active/round/score updates. Hazard commands reject player actors as unauthorized and return conflicts for missing maps, out-of-bounds cells, duplicate non-layered hazards, maximum Toxic Spikes layers, and missing remove targets. `sendOutPokemon` rejects players who do not control the trainer token and returns conflicts for non-trainer source tokens, missing sheets, Pokémon not on the trainer's current team, duplicate spawned token IDs, occupied/out-of-bounds destinations, and destinations outside Poké Ball throw range. They are distinct from transport `error` messages.
...
```json
{
  ...
  "message": "live session WebSocket command dispatch supports moveToken, turnToken, spawnToken, deleteToken, sendOutPokemon, modifyHp, modifyCombatStages, modifyConditions, useMove, useManeuver, useAbility, useOrder, setInitiative, nextInitiative, previousInitiative, placeHazard, removeHazard, setFieldEffect, removeFieldEffect, tickFieldEffectDurations, buildTerrainVoxel, and removeTerrainVoxel commands only.",
  "retryable": false,
  "currentRevision": 42
}
```
...
`GET /api/events` remains the legacy Server-Sent Events channel for local-first, non-session map/sheet/library updates. It is not used for live session commands, acknowledgements, rejections, presence, heartbeat, reconnect, or session patches. Existing local workflows may still carry whole saved map/sheet payloads over SSE; live sessions must use `/api/sessions/socket` and server-authoritative commands instead. The [Live session Quick Tunnel caveat](live-session-quick-tunnel-caveat.md) documents why a temporary `trycloudflare.com` URL does not make legacy SSE a supported public session transport.
...
## Manual WebSocket smoke expectations
...
## Automated token command smoke coverage

`tests/server/sessionTokenCommandTwoClientSmoke.test.ts` is the two-client token-command smoke test. It opens authenticated GM and player WebSocket peers in the same session, assigns the player a token, and verifies that:
