...
The response intentionally excludes the GM key. It may include an ended session's status for GM inspection, but ended sessions remain absent from active join-code lookups. This endpoint is read-only; assignment mutation uses a dedicated GM endpoint. The former session map attachment endpoint has been removed from normal play, so saved maps are not published into sessions through `/api/sessions/maps/attach`.
