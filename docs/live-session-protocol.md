...
| `shared/sessionCommands.ts` | command envelope, `opId`, `baseRevision`, scope lanes, metadata | The common command wrapper used by all command payload modules. |
...
| `shared/sessionMapAttachment.ts` | attach-map request/result contracts | Defines the GM-only request for attaching a persisted map by slug, selected-map behaviour, player-visibility behaviour, validation issues, and no-secret public result shape. |
...
See [Live session public exposure checks](live-session-public-exposure-checks.md) for the operational checklist attached to these startup issues, [Live session security boundaries](live-session-security-boundaries.md) for the broader trust boundaries, join-code limits, tunnel exposure risks, and non-hardened areas, [Live session security and secret-hygiene readiness](live-session-security-secret-hygiene-readiness.md) for the auth/session/cookie/permission boundary maintenance check, [Live session persistence/recovery maintenance](live-session-persistence-recovery-maintenance.md) for the snapshot/event-log and backup hygiene maintenance check, [Live session concurrency benchmark notes](live-session-concurrency-benchmark-notes.md) for latency-sensitive concurrency observations, [Live session implementation maintenance](live-session-implementation-maintenance.md) for implementation evidence, tests, docs, and limitations, and [Live session dependency and runtime maintenance](live-session-dependency-runtime-maintenance.md) for the checked package/runtime boundaries.
...
The existing non-session realtime channel remains available during the WebSocket migration. It is intentionally separate from live session hosting:
...
- Live Live sessions must not use `/api/events` for commands, command acknowledgements/rejections, presence, heartbeat, or reconnect. Session clients use `WebSocket /api/sessions/socket` after the session-host flag and hello/auth handshake, and table mutations route through server-authoritative command handlers.

This boundary lets the app preserve existing local map workflows while live session mode remains explicit and additive. Separate follow-up work may replace additional local-mode behaviours with session-aware alternatives when a session is active, but it should not remove the legacy SSE route until a dedicated migration explicitly does so.

## GM start-session endpoint

`POST /api/sessions/start` creates the first server-side identity and state record for a live session. The route fails closed unless `ROTOM_ENABLE_SESSION_HOST=1` is present, and the server use case repeats that runtime-gate check before allocating anything. The route currently also requires the existing local GM role so the GM can start a session from the trusted local app, but that role picker is not public authentication; the returned session-local GM key is the credential that GM management routes and WebSocket handshakes validate.
...
The GM key and join code are session-local secrets and should be shown or stored only by the lobby/client-identity flows. Starting a session does not add accounts, hosted persistence, or client whole-map authority.

## Player join-session endpoint

`POST /api/sessions/join` lets a player join an active live session with a short join code and a display name. The route fails closed unless `ROTOM_ENABLE_SESSION_HOST=1` is present, but unlike the GM start route it does not require the existing local role-picker cookie: the join code is the session-local capability for creating a player identity. This is still not public account authentication; it creates only a session-local `playerId`, `clientId`, safe display name, and empty assignment record for GM-managed assignment flows.
...
The server normalizes join-code casing/separators, sanitizes the display name into the shared safe display-name shape, rejects unknown or ended sessions, creates a unique player ID and client ID, advances the session revision, writes the updated authoritative snapshot, and returns the identity the WebSocket hello/client-identity flow uses:
...
`POST /api/sessions/manage` returns the GM-facing lobby summary for one live session. The route fails closed unless `ROTOM_ENABLE_SESSION_HOST=1` is present and requires the session-local `gmKey`; it must not rely on the trust-based local role picker as public authentication. The request body contains only the session identity and GM key, not map edits:
...
The response intentionally excludes the GM key. It may include an ended session's status for GM inspection, but ended sessions remain absent from active join-code lookups. This endpoint is read-only; map attachment and assignment mutation use dedicated GM endpoints.

## Attach session map endpoint

`POST /api/sessions/maps/attach` lets the GM attach a persisted map to an active live session. The route fails closed unless `ROTOM_ENABLE_SESSION_HOST=1` is present, requires the session-local `gmKey`, and loads the map document by `mapSlug` on the server. It does not accept a browser-provided whole-map document as live-session authority.
...
The server validates the request shape, rejects untrusted document fields such as `map`, `maps`, `document`, `mapDocument`, or `mapState`, authorizes the GM key, loads and clones the persisted map, upserts it into `AuthoritativeSessionState.maps`, advances the session revision once, and writes a local session snapshot. New attachments start at the initial map revision; reattaching a map preserves that session map revision while refreshing the server-owned map document by slug.

Selection and visibility are explicit:

- `selectedMapBehavior: "select-attached-map"` makes the attached map the selected session map; `"preserve-current-selection"` leaves the current selected map unchanged.
- `visibilityBehavior: "visible-to-all-players"` grants current joined players map visibility and marks the map visible for future joins.
- `"visible-to-joined-players"` grants only players who have already joined.
- `"gm-only"` attaches the map without adding player visibility grants.

A successful response contains only no-secret summaries:
...
```json
{
  ...
  "visibility": {
    ...
    "visiblePlayerIds": ["player_generated_id"]
  },
  "snapshot": {
...
The response excludes the GM key, join code, map document, raw snapshot, persisted file path, and hidden player details. After attachment, player-state summaries expose only visible session maps to each player. Session-map commands use the server-owned attached map state; browser-local seed state is not authority.
...
The helper clears malformed local records or malformed/secret-bearing cookie hints instead of treating them as authority. This keeps Live session as session-local continuity, not full accounts, public auth, or durable cloud identity.
...
This model is the state stored in the in-memory session store and written as local snapshots. It is not a client autosave format: live clients still send commands, the server mutates this authoritative copy, and broadcasts small patches or snapshots from it.
...
If the same `opId` is reused with a materially different command envelope or payload, server command handlers reject it safely rather than treating it as an edit to the original command.
...
live-session implementation keeps these checks at the protocol boundary:
