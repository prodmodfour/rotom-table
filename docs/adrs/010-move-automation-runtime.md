...
Branch operations select only reviewed later operation IDs. The server may choose one resolution-wide or per-recipient path from a bounded predicate, route each recipient through the exhaustive self/ally/enemy/unknown relationship result, or consume the final branch ID of an earlier authoritative check. Human branch options are mutually exclusive stable IDs; an optional effect uses an explicit empty pass path. Until durable resume orchestration commits a response, a resolution-wide or target-specific choice returns a typed non-committing pending result whose public options omit the referenced mechanics. Unselected paths do not roll, resolve recipients, enter the emitted plan, or mutate state.
