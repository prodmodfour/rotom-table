...
HP operations distinguish healing from direct HP change and never use suggestion labels as mechanics. Fixed, real/formula Max HP, current HP, missing HP, and bounded formula calculations use explicit root rounding and final-pool bounds; full healing uses the Injury-adjusted cap, while temporary HP, set, copy, and simultaneous split modes are separate typed branches. Copy sources and all mutated recipients are server-resolved selectors, and formula reads join the authoritative read set. Healing cannot become loss through a restrictive bound. A split calculates every recipient's requested final value before changing any recipient. Direct HP reduction explicitly chooses whether HP-marker Injuries are applied after the operation or ignored, and always declares that Massive Damage is `never`; only a damage operation can apply the normal Massive Damage threshold. Each recipient's calculation, bounds, prior/final pools, and Injury outcome are retained in the structured trace.
