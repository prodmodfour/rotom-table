# Track 2 session backup and recovery
...
Treat the resulting archive as sensitive campaign data. If the GM chooses to copy a backup to private cloud storage, that is an explicit GM backup decision, not Track 2 runtime cloud persistence.
...
Track 2 backup/recovery keeps the following on GM-controlled storage:
