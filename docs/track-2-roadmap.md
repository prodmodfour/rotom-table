...
Cloudflare Quick Tunnel can be used only as a temporary development smoke-test option when docs explicitly call out its limitations. It is not the supported campaign-session deployment path.
...
| 7. Client integration | Session/local state split, map composable, MapScenePanel wiring, rejection UI, presence, reconnect states, navigation | The app can run local mode and session mode side by side without regressing Track 1 map quality. |
...
See [Track 2 glossary](track-2-glossary.md) for the shared vocabulary used by this roadmap and later protocol documents. See the [Track 2 validation matrix](track-2-validation-matrix.md) for the expected tests, smoke checks, docs, and safety reviews attached to later implementation areas.
