...
- Session files stay on the GM's machine or another machine they control. Live session does not add Postgres, Redis, Durable Objects, SaaS storage, or automatic cloud replication.
...
5. Let the server load the latest valid `snapshot.json`; Live session reconnect remains safe through snapshot fallback when event replay is unavailable.
...
- `events.jsonl` is optional and helper-level only in Live session; WebSocket reconnect uses snapshot fallback when replay is unavailable.
