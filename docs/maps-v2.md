...
Each entry may track `rounds`; `null` means sustained until explicit removal. This array is now a compatibility presentation lane: authoritative global-field ownership and lifecycle live in `encounterState.zones`. Forward initiative decrements finite durations at the declared round boundary and scene-end expires scene-duration fields. The `tickFieldEffectDurations` command remains only as an explicit typed GM correction; it is not ordinary rules advancement. Weather plus Electric/Grassy Terrain mechanics are authoritative; Misty/Psychic Terrain and Room calculations remain in their dedicated mechanics tickets.

Electric and Grassy membership is derived from the active zone projection, a placement's complete footprint, and its sheet-derived grounded/airborne state. Battlefield geometry affects every grounded placement; local cell geometry affects only overlapping grounded footprints. Movement comparisons use server-owned origin/destination footprints, so no terrain-membership flag is stored on the placement or accepted from the browser. Electric Terrain blocks Sleep and boosts grounded Electric attacks by +10. Grassy Terrain boosts grounded Grass attacks by +10 and heals a grounded member by one Tick at turn start through the initiative lifecycle. Same-kind overlapping zones do not stack these rules.
