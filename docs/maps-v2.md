...
The editor renders setup/brush hazards as floor decals and persists those compatibility rows in `hazards[]`. Native MoveSpec hazard and pledge operations instead persist typed cell zones in `encounterState.zones`; they never copy a move source or side into the legacy free-form `owner`. Neither representation blocks token placement or pathfinding by itself.
...
Existing editor/brush `hazards` and global `fieldEffects` remain in their compatibility lanes during staged migration. New native move hazards write only typed zones. Server zone queries project valid in-bounds compatibility hazards plus global Weather, Terrain, and Room entries into deterministic query-only zone records. A native migrated record uses the same reserved `legacy.<lane>.<key>` ID and shadows its source lane record, preventing dual application while unrelated legacy entries remain available. Legacy free-form hazard owner labels never become a `sideId`, and a legacy `scope: area` terrain has no cells to adapt, so it fails closed rather than being treated as battlefield-wide. The adapter does not persist on read or add mechanics by itself. `temporaryHitPoints` and `moveUsage` also remain separate during the staged migration.
