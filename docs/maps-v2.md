...
Existing editor/brush `hazards` and global `fieldEffects` remain in their compatibility lanes during staged migration. New native move hazards write only typed zones. Server zone queries project valid in-bounds compatibility hazards plus global Weather, Terrain, and Room entries into deterministic query-only zone records. A native migrated record uses the same reserved `legacy.<lane>.<key>` ID and shadows its source lane record, preventing dual application while unrelated legacy entries remain available. Known compatibility hazards receive the same registered hook/modifier references in this query projection, but enemy-only mechanics fail closed because legacy free-form owner labels never become a `sideId`; neutral Fire Hazard entry remains mechanically queryable. A legacy `scope: area` terrain has no cells to adapt, so it fails closed rather than being treated as battlefield-wide. The adapter never persists on read. `temporaryHitPoints` and `moveUsage` also remain separate during the staged migration.
