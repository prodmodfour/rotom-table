...
For the broader trust-boundary review, join-code limits, tunnel exposure risks, incident response, and non-hardened areas, see the [Live session security review](live-session-security-review.md). For the auth/session/cookie/permission boundary and public exposure warning review, see the [Live session security readiness audit](live-session-security-readiness-audit.md).
