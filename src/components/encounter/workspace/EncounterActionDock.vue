...
const recentOfferIds = ref<readonly string[]>([])
...
const orderedFiltered = computed(() => recentOnly.value
  ? orderEncounterActionsByRecency(filtered.value.filter(offer => recentOfferIds.value.includes(offer.offerId)), recentOfferIds.value)
  : filtered.value)
...
const activate = (offer: EncounterActionOffer): void => {
  recentOfferIds.value = recordRecentEncounterAction(recentOfferIds.value, offer.offerId)
  emit('activate', offer)
}
  ...
  <section class="encounter-action-dock" data-encounter-focus="action-dock" tabindex="-1">
    ...
    <div class="encounter-action-dock__filters" role="search" aria-label="Filter available actions">
      ...
      <label>
        ...
        <select v-model="group">
          ...
          <option v-for="value in availableGroups" :key="value" :value="value">{{ value }}</option>
        </select>
      </label>
      <label>
      ...
      <button type="button" :aria-pressed="recentOnly" @click="recentOnly = !recentOnly">Recent</button>
    </div>
    ...
    <div v-if="grouped.length" class="encounter-action-dock__groups">
      <section v-for="actionGroup in grouped" :key="actionGroup.group" :aria-labelledby="`action-group-${actionGroup.group}`">
        <h3 :id="`action-group-${actionGroup.group}`">{{ actionGroup.group }}</h3>
        <div>
          <EncounterOfferCard
...
.encounter-action-dock__filters { display: flex; align-items: flex-end; gap: 0.45rem; margin: 0.6rem 0; overflow-x: auto; }
