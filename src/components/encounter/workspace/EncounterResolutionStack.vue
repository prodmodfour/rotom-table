...
<template>
  <section class="encounter-resolution-stack" aria-labelledby="encounter-resolution-heading">
    ...
    <ol v-if="ordered.length">
      ...
      >
        <button type="button" class="encounter-resolution-stack__open" @click="emit('open', interaction.interactionId)">
          <span class="encounter-resolution-stack__order rt-numeric">{{ index + 1 }}</span>
          <span>
            <strong>{{ interaction.prompt }}</strong>
            <small>
              {{ interaction.status }} ·
              {{ interaction.projection === 'public' ? `${interaction.outstandingChoiceCount} outstanding` : `${interaction.choices.length} choice groups` }}
            </small>
          </span>
        ...
        <p v-else>Waiting for an authorized responder. Private options are not present in this view.</p>
      </li>
    </ol>
