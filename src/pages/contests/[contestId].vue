...
import { CONTEST_INTRODUCTION_SKILL_IDS, CONTEST_STAT_IDS, emptyContestStatRecord, type ContestStatId } from '#shared/contests/ids'
...
import type { ContestantStateV1, ContestMoveOptionV1, ContestPerformerSnapshotV1 } from '#shared/contests/document'
...
const appeal = reactive({ moveOptionId: '', spent: emptyContestStatRecord(() => 0) })
...
const activePerformer = computed<ContestPerformerSnapshotV1 | null>(() => {
  ...
  const index = projection.value.variantId === 'rotation' ? contestant.rotationOrder[Math.max(0, projection.value.round - 1)] : 0
  return Number.isInteger(index) ? contestant.performers[Number(index)] ?? null : null
})
const availableRotationPerformers = computed(() => activeContestant.value?.performers.filter((_row, index) => !activeContestant.value?.rotationOrder.includes(index)) ?? [])
const activeLastMoveOptionId = computed(() => [...(projection.value?.acceptedAppeals ?? [])].reverse().find(row => row.contestantId === activeContestant.value?.contestantId && row.performerId === activePerformer.value?.performerId)?.moveOptionId ?? null)
...
const appealAssembly = computed(() => {
  ...
  return assembleContestAppeal({ effectId, moveTypeId, contestTypeId, spentDice: spentTotal.value, startingVoltage: activeScore.value?.voltage ?? 0, adjacentVoltages: (position?.adjacentContestantIds ?? []).map(id => projection.value?.scoreboard.find(row => row.contestantId === id)?.voltage ?? 0), repeatedMove: activeLastMoveOptionId.value === move.optionId, baseMoveDiceMultiplier: contestant.pendingEffects.nextRoundBaseMoveDiceMultiplier, alignmentSteps: contestant.pendingEffects.nextAppealAlignmentSteps, sonic: move.tags.includes('sonic'), voiceLessonsActive: performer.providerIds.includes('feature:Voice Lessons'), acceptedInterventionBonusDice: contestant.pendingEffects.nextAppealBonusDice })
})
...
const introductionHasGrace = computed(() => introductionActor.value?.performers.some(row => row.providerIds.includes('edge:Grace')) === true)
const introductionStatOptions = computed(() => introductionHasGrace.value ? CONTEST_STAT_IDS : [introductionMappedStat.value])
const introductionProviders = computed(() => introductionActor.value?.performers[0]?.providerIds ?? [])
...
const availablePokemon = computed(() => sheets.value.pokemonSheets.filter(sheet => !gmProjection.value?.contestants.some(row => row.performers.some(performer => performer.pokemonSheetSlug === sheet.slug))))
...
const offeredInterventions = computed(() => {
  const providers = activePerformer.value?.providerIds ?? []
...
const pendingAppeal = computed(() => projection.value?.acceptedAppeals.find(row => row.appealId === projection.value?.pendingInterventionAppealId) ?? null)
...
const postOfferedInterventions = computed(() => offeredInterventions.value.filter((id) => {
  ...
  return activePerformer.value.providerIds.includes(`${prefix}:${id}:${pendingAppeal.value.moveTypeId}`)
}))
...
const enroll = async (): Promise<void> => {
  if (!enrollment.trainerSlug || !enrollment.pokemonSlugs.length) return
  const result = await runtime.execute({ ...commandBase(), commandKind: 'enroll-contestant', contestantId: contestantId(), trainerSheetSlug: enrollment.trainerSlug, pokemonSheetSlugs: [...enrollment.pokemonSlugs], controller: enrollment.controllerId === 'gm' ? { kind: 'gm' } : { kind: 'profile', profileId: enrollment.controllerId as never }, rotationOrder: projection.value?.rotationOrderPolicy === 'choose-each-round' ? [] : enrollment.pokemonSlugs.map((_, index) => index) })
...
const activePoolRemaining = (statId: ContestStatId): number => (activePerformer.value?.dicePools[statId].remaining ?? 0) + (projection.value?.variantId === 'rotation' ? activeContestant.value?.teamDicePools[statId].remaining ?? 0 : 0)
...
const submitAppeal = async (): Promise<void> => {
  ...
  const accepted = await runtime.execute({ ...commandBase(), commandKind: 'declare-appeal', contestantId: activeContestant.value.contestantId, performerId: activePerformer.value.performerId, moveOptionId: appeal.moveOptionId, spentDice: { ...appeal.spent } })
  if (accepted) { appeal.moveOptionId = ''; Object.assign(appeal.spent, emptyContestStatRecord(() => 0)); await focusPrimaryDecision() }
}
...
<template>
  <main ref="cockpit" class="contest-cockpit rt-design-system" data-rt-context="liveplay">
    <AppNavigation />
    <header class="contest-header">
      ...
      <div v-if="projection" class="contest-heading">
        ...
        <div class="header-meta"><span>{{ pretty(projection.variantId) }}</span><span>{{ pretty(projection.currentRoundContestTypeId ?? projection.contestTypeId) }}</span><span>{{ pretty(projection.stage) }}</span><span v-if="projection.round">Round {{ projection.round }} of {{ projection.scoreboard.filter(row => row.active).length }}</span></div>
      </div>
    ...
    <template v-if="projection">
      <section v-if="projection.scoreboard.length" class="stage-line" aria-label="Contest stage positions">
        <article v-for="row in projection.scoreboard" :key="row.contestantId" class="performer-card" :class="{ 'performer-card--active': row.contestantId === projection.activeContestantId }">
          <img v-if="row.portraitUrl" :src="row.portraitUrl" alt="" />
          <span v-else class="portrait-fallback" aria-hidden="true">{{ row.letter ?? '?' }}</span>
          <div><strong>{{ row.pokemonName }}</strong><small>{{ row.displayName }}</small><em v-if="row.position?.centerOfAttention">Center of attention</em></div>
          <b>{{ row.letter ?? '—' }}</b>
          <dl><div><dt>Appeal</dt><dd>{{ row.appeal }}</dd></div><div><dt>Fumble</dt><dd>{{ row.fumble }}</dd></div><div><dt>Voltage</dt><dd>{{ row.voltage }}</dd></div><div><dt>Score</dt><dd>{{ row.finalScore }}</dd></div></dl>
        </article>
      </section>
      ...
      <div class="cockpit-grid">
        <div class="primary-column">
          <section v-if="projection.stage === 'setup'" class="action-panel" aria-labelledby="setup-title">
            <div class="panel-heading"><p>Contest Workshop</p><h2 id="setup-title" tabindex="-1" data-contest-primary>Enroll contestants</h2></div>
            <template v-if="isGm">
              ...
              <form class="enrollment-form" @submit.prevent="enroll">
                ...
                <label><span>{{ projection.variantId === 'rotation' ? 'Performer sheets (in round order)' : 'Pokémon sheet' }}</span><select v-model="enrollment.pokemonSlugs" :multiple="projection.variantId === 'rotation'" :size="projection.variantId === 'rotation' ? 5 : undefined" required><option v-for="pokemon in availablePokemon" :key="pokemon.slug" :value="pokemon.slug">{{ pokemon.nickname || pokemon.species || pokemon.slug }}</option></select></label>
                <label><span>Controller</span><select v-model="enrollment.controllerId"><option value="gm">GM</option><option v-for="profile in profileOptions" :key="profile.id" :value="profile.id">{{ profile.displayName }}</option></select></label>
                <button class="primary-action" :disabled="runtime.submitting.value">Enroll snapshot</button>
              </form>
              <ul class="enrolled-list"><li v-for="contestant in gmProjection?.contestants" :key="contestant.contestantId"><span><strong>{{ contestant.displayName }}</strong><small>{{ contestant.performers.map(row => row.displayName).join(' · ') }} · {{ contestant.controller.kind === 'gm' ? 'GM controlled' : 'Player controlled' }}</small></span><button type="button" class="icon-action" :aria-label="`Remove ${contestant.displayName}`" :disabled="runtime.submitting.value" @click="removeContestant(contestant.contestantId)"><PhTrash :size="18" /></button></li></ul>
              <button type="button" class="primary-action start-action" :disabled="projection.scoreboard.length < 3 || runtime.submitting.value" @click="stageCommand('start-introduction')">Lock {{ projection.scoreboard.length }} contestants and begin introductions</button>
              <p v-if="projection.scoreboard.length < 3" class="bounded-note"><PhLockKey :size="17" /> Three through five contestants are required.</p>
            </template>
          ...
          <section v-else-if="projection.stage === 'introduction'" class="action-panel" aria-labelledby="intro-title">
            <div class="panel-heading"><p>Introduction stage</p><h2 id="intro-title" tabindex="-1" data-contest-primary>Make an impression</h2></div>
            <form v-if="introductionActor" class="intro-form" @submit.prevent="declareIntroduction">
              <p><strong>{{ introductionActor.displayName }}</strong> chooses one social Skill. Authority will roll <b>{{ introductionPreviewDice }}d6</b><template v-if="introductionBonusLabels.length"> ({{ introductionBonusLabels.join(', ') }})</template>.</p>
              ...
              <label><span>Skill-roll Contest stat</span><select v-model="intro.generatedStatId"><option v-for="stat in introductionStatOptions" :key="stat" :value="stat">{{ pretty(stat) }}</option></select><small>{{ introductionHasGrace ? 'Grace allows any Contest stat.' : `${pretty(intro.skillId)} maps to ${pretty(introductionMappedStat)}.` }} Standard matching Appeal is determined by the selected Skill.</small></label>
              ...
              <button class="primary-action" :disabled="projection.paused || runtime.submitting.value">Roll introduction</button>
            </form>
            <p v-else class="empty-state"><PhCheckCircle :size="20" /> Your introduction is accepted. Waiting for the remaining contestants.</p>
            <div class="accepted-grid"><article v-for="row in projection.scoreboard" :key="row.contestantId"><strong>{{ row.displayName }}</strong><span v-if="allContestants.find(item => item.contestantId === row.contestantId)?.introduction.status === 'accepted'">Accepted · Letter {{ row.letter ?? 'pending tie resolution' }}</span><span v-else>Waiting</span></article></div>
            <div v-if="isGm" class="button-row"><button type="button" class="quiet-action" @click="stageCommand('restart-introduction')">Restart introductions</button><button type="button" class="primary-action" :disabled="!allIntroduced || projection.paused || runtime.submitting.value" @click="stageCommand('start-performance')">Start performance</button></div>
          </section>
          ...
          <section v-else-if="projection.stage === 'performance'" class="action-panel appeal-panel" aria-labelledby="appeal-title">
            ...
            <template v-else-if="canControlActive && activePerformer">
              <p class="active-performer"><strong>{{ activePerformer.displayName }}</strong> · {{ activePerformer.species }} <span v-if="activeScore?.position?.centerOfAttention">· Center of attention</span></p>
...
@media(max-width:760px){.score-head,.score-row{min-width:22rem}.contest-header{grid-template-columns:1fr auto}.contest-heading{grid-column:1/-1;grid-row:2;text-align:left}.header-meta{justify-content:flex-start}.enrollment-form,.intro-form{grid-template-columns:1fr}.intro-form>p{grid-column:auto}.move-options{grid-template-columns:1fr}.dice-pools{grid-template-columns:repeat(2,1fr)}.secondary-column{grid-template-columns:1fr}.gm-tools{grid-column:auto}.score-head,.score-row{grid-template-columns:minmax(7rem,1.6fr) repeat(4,minmax(2.7rem,.5fr))}.score-head span,.score-row>span,.score-row>strong{padding:.5rem .15rem}}
@media(max-width:480px){.stage-line{grid-template-columns:1fr}.performer-card{min-width:0}.performer-card dl{grid-template-columns:repeat(2,minmax(0,1fr))}.performer-card dl div{min-width:0}.dice-pools{grid-template-columns:1fr}.intervention-form{grid-template-columns:1fr}.assembled-roll p{align-items:flex-start;flex-direction:column}.accepted-grid article{align-items:flex-start;flex-direction:column}.result-row summary em{display:none}}
