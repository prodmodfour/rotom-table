...
import { contestPerformerIsPokemon, type ContestantStateV1, type ContestMoveOptionV1, type ContestPokemonPerformerSnapshotV1 } from '#shared/contests/document'
...
const activePerformer = computed<ContestPokemonPerformerSnapshotV1 | null>(() => {
...
const introductionProviders = computed(() => introductionActor.value?.performers.find(contestPerformerIsPokemon)?.providerIds ?? [])
        ...
        <div class="primary-column">
          <section v-if="projection.stage === 'setup'" class="action-panel" aria-labelledby="setup-title">
            ...
            <template v-if="isGm">
              ...
              <button type="button" class="primary-action start-action" :disabled="projection.scoreboard.length < 3 || projection.participantVariantId === 'trainer-participant' && !projection.participantMethodId || runtime.submitting.value" @click="stageCommand('start-introduction')">Lock {{ projection.scoreboard.length }} contestants and begin introductions</button>
