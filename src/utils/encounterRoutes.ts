...
export const ENCOUNTER_BUILDER_PATH = '/encounters/new'
...
export const encounterBuilderPath = (): typeof ENCOUNTER_BUILDER_PATH => ENCOUNTER_BUILDER_PATH
...
export const encounterBuilderTablePath = (region: string, table: string): string => {
  const query = new URLSearchParams({ region, table })
  return `${ENCOUNTER_BUILDER_PATH}?${query.toString()}`
}
