...
export const useSessionMapSceneCommands = (
  ...
  const requireActor = <TCommand extends SessionMapSceneCommand>(): true | Extract<SessionMapSceneCommandDispatchResult<TCommand>, { readonly dispatched: false }> => {
    if (!enabled.value) return fail<TCommand>('not-session-mode', 'Track 2 session scene command dispatch is not enabled for this map view.')
    if (options.session.identity.value === null) {
      return fail<TCommand>('missing-session-identity', 'No remembered Track 2 session identity was found; open the session lobby and start or join a session first.')
    }
    return true
  }
  ...
  const requireMap = <TCommand extends SessionMapSceneCommand>(): TabletopMap | Extract<SessionMapSceneCommandDispatchResult<TCommand>, { readonly dispatched: false }> => {
    ...
    if (!options.map.value) return fail<TCommand>('missing-map', 'Cannot dispatch a Track 2 session command before the map is loaded.')
  ...
  const requirePlacement = <TCommand extends SessionMapSceneCommand>(
    ...
    if (!placement) return fail<TCommand>('missing-placement', 'Cannot dispatch a Track 2 session command because the token is no longer on the map.')
    return placement
  }
