...
| ---: | --- | --- | --- |
| 11 | [Deferred Mechanics Closure — draft](drafts/DEFERRED_MECHANICS_CLOSURE_PLAN.md) | Draft registered for review by P10-100; it is non-authoritative and does not activate execution until converted into a reviewed numbered ledger in the table above. Proposed scope: drive every mechanics row recorded as deferred or visible-with-reason to a final state—the six deferred ranged weapon profiles, seven absent supplemental Move definitions, deferred item actions, Battle Contest and Trainer Participant variants, and a generic liveplay Skill Check surface if the closure audit confirms the gap. Exit: zero known deferred mechanics across all ledgers. | Plan 10 (complete; the combat-blend Contest variants need both engines). |
| 12 | GM Campaign Toolkit | Modernize the legacy JSON encounter-table tooling into liveplay authority and complete Running the Game coverage: wild encounter generation, NPC trainer generation, and session preparation as ordinary campaign work. | Plan 11 (generated NPCs need the closed move, weapon, and item catalogs). |
