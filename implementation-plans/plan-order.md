...
| 13 | [1.0 Release Readiness](RELEASE_READINESS_PLAN.md) | `IN_PROGRESS` | P13-001–P13-084 complete (84/86 tickets complete). P13-085 failed closed because immutable annotated, unpublished `v1.0.0` produced three distinct checksum manifests on identical supported-shape builds. A deterministic source repair is prepared and twice reproduced 12,663/12,663 output files exactly, but version policy requires an authorized `1.0.1` successor; the one P13-084 transaction is consumed. `BLOCKED_BY: OWNER_PATCH_RELEASE_AUTHORIZATION_P13_085`; remote publication remains unauthorized. |
