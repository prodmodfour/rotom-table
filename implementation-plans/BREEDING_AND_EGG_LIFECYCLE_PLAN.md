...
- Durable breeding-project schema: **SQLite v28 strict repository active**
- Durable Egg schema: **SQLite v28 strict dedicated aggregate repository active**
- Atomic child-sheet insert: **strict complete-document revision-0 repository active**
- Historical Species acquisition: **shared immutable capture/hatch/evolution/trade/review history plus absent=one `dexExp` / present=zero reward and release retention active**
...
- Feature/Edge interaction certification: **all 21 reviewed rows certified: 16 active owning runtimes, four upstream tutoring-operation boundaries, one explicit no-consumption-authority failure boundary, and zero facility authority**
...
- [x] **BR-068 — Integrate frozen inheritance candidates with level-up learning, errata prerequisites, move slots, and permanent-move provenance** — `DONE`
  - Evidence: Inheritance-learning contract `068e407457fe1b0ce5ace96a9d594caa402db3c27709ff3b862d1e9d621a8d0d` and closed runtime policy `0ae30cde6f2bcd70b4db6a60c9af6e7cf784a4522a1dd363b733adeb8aa85bb8` bind Levels 20–100 to the immutable origin candidate set, current app-owned Move authority, errata frequency/Damage Base bands, exact command-bound offer options, six natural Move slots, the separate three applied TM/Tutor limit, and typed origin/Egg/checkpoint/operation/candidate provenance. Starting-Level checkpoints now settle before revision-zero hatch insertion; later skipped checkpoints settle as one gap-free batch, illegal candidates leave an auditable empty slot and remain eligible, and exhausted candidates create explicit empty records. `recordPokemonInheritanceLearning.ts` rebuilds current Egg, child, unique owner, hydrated lineage, clock, references, offers, read set, and owner/GM authority before one caller-owned transaction consumes offers, advances the sheet once, inserts records, settles the operation, and appends restricted refreshes. Rollback leaves only the recoverable reservation, exact retries do not republish and survive later child revisions, malformed/accessor-backed Move rows fail closed, and Level-25 Baby Template/Marsupial recovery preserves inheritance state. Focused contract, domain, transaction, hatch, live-play, repository, authorization, checker, lint, and typecheck validation pass.
- [x] **BR-069 — Reuse acquisition history across capture, hatch, evolution, trade, release, and migration** — `DONE`
  - Evidence: Species-acquisition integration contract `c2678af9b6299bc58605832a7e4ae17ad90b66dda774bb549afe299665ec35e2`, closed runtime policy `99098376839dd2a1b4c7d0b28851d5498f48dce3e06ba21f47b371de319a5fdd`, and byte-equivalent storage schema v27 `3c742cbb35c36a0928708319840f9802520b2bed8b490fda0d1956ca835947c3` route successful live-play capture targets/companions, owned setup evolution, unique destination-roster trade additions, and source-hash-bound migration/GM reviews through the shared immutable `(Trainer, Species)` history and exactly-once reward service. External events retain deterministic self-hashed evidence and private terminal settlements without forging breeding commands; hatch remains in its ordinary transaction. Release performs no history write or delete, reacquisition grants zero, and exact retry returns persisted evidence with zero new reward or publication. The caller-owned transaction rolls map/sheet transitions, history, `dexExp`, source settlement, and restricted realtime rows back together on stale authority, duplicate ownership, unknown Species, collision, overflow, or injected failure. Focused integration, capture/route, setup-save, reward, migration, repository, malformed-input, privacy, replay, and rollback validation passes across 98 checks; the expanded 72-file/586-check Breeding surface, checker, targeted lint, and typecheck also pass.
...
- [x] **BR-072 — Implement compatibility explanations, unavailable reasons, source contributions, and GM diagnostics** — `DONE`
...
- [x] **BR-075 — Implement the hatch decision flow, special adjudication, child reveal, and accepted presentation choreography** — `DONE`
  - Evidence: Hatch-workflow presentation contract `373d79489e6265104f237452e6d953fc75581028c91c584d7711223b71379c46` and runtime policy `f3c73f295cd99bdefed7d1df9003d461971cc50a3605f601e06a1550cb373b1f` add strict `POST /api/breeding/hatch` choreography over the existing current hatch-offer, persisted special-roll/adjudication, and atomic completion authorities. Requests carry only role/Profile context, owner Trainer/Egg selectors, expected revision, one closed intent, at most one opaque GM option, and explicit mutation confirmation; every command, destination fact, roll, provider, child fact, read set, receipt, and evidence value remains server-owned. Owners receive only bounded waiting state for a triggered roll, while current GMs receive one persisted total, canonical triggers, and three nonmechanical choices; neither 1 nor 100 implies Shiny or changes Nature. Completion reveals only accepted child navigation and canonical presentation facts after child, Trainer link, acquisition/reward, Egg, lineage, result, and restricted refresh rows commit together. Pending operations become refresh-only recovery, exact retries neither redraw nor republish, and the initial flow intentionally uses the server-owned Box destination pending BR-076. The labelled responsive modal provides native radio semantics, separate confirmations, 44-pixel controls, focus/Escape/status announcements, and reduced-motion reveal handling. Twenty-four focused contract/use-case/route/composable/component checks pass; the 90-file/615-check Breeding surface passes after transitive refresh of five semantic-registry-bound fixtures, with checker, changed-file lint, and clean diff validation green.
- [x] **BR-076 — Integrate child destination, team-limit explanations, box linkage, Trainer pages, and sheet navigation** — `DONE`
  - Evidence: Hatch-destination presentation contract `28c2c1d79227b8f9b9bbf869c16ec908234b50237ca7434216ed8db3120c065a` and runtime policy `7d50f8aad9542690c5ad6a97b79e600027b089df1762807a7574d1e4141b8a5e` extend the strict BR-075 workflow with two separately regenerated current command-bound Box/team options. The browser submits one opaque option ID and cannot claim destination kind, capacity, roster contents, command, or authority; stale, unavailable, ambiguous, or full-team selection rejects before reservation or randomness. Box remains available for a ready Egg, team derives from the authoritative six-slot count, and only remaining slots plus `breeding.hatch-offer.team-full` are projected. The accepted begin command freezes the destination through special review and completion; the existing BR-057 transaction remains the sole writer of exactly one `currentTeam` or `boxedPokemon` link, and exact begin/completion replay adds no child, reward, link, Egg revision, event, or publication. Accepted reveal links use only the storage-owned child slug and current authorized Trainer slug through existing sheet routes. Native destination radios, disabled-state explanation, separate confirmation, 44-pixel controls, responsive navigation, and existing focus/reduced-motion semantics are covered. Twenty-three focused workflow checks and the expanded 90-file/620-check Breeding surface pass; all five semantic-registry-bound fixtures were rehashed transitively, changed-file lint and checker pass, and repository typecheck reports no BR-076 file errors beyond the known unrelated baseline.
- [x] **BR-077 — Implement cross-owner consent, revocation, notifications, private views, and GM override UX** — `DONE`
  - Evidence: Consent-workflow presentation contract `a450b7d354f462ff46ec0feb2ca79e337c87514df67f6191640cafbfc7b0ae81`, runtime presentation policy `8c608a3e2e22d848383f5c4153d18dc6156e45b0de4b8a8f0a24bdc6d8c9639b`, operation contract `391eacdd340d44249755e0db6911b8de4068dee17ce9fa3ff03d3ba736f8739c`, and byte-equivalent SQLite schema v28 `a9ab42f40b0094c2659c091b4b5ba1edde9ce3281e45a689680b65d0a6011802` add strict `POST /api/breeding/consent` private Project and Egg-transfer orchestration. Confirmed cross-owner setup persists only `awaiting-parent-consent`; the final current Profile/Trainer/parent-revision grant atomically starts ordinary initial time, while participant revocation delegates to lifecycle recovery and a new Project revision requires a fresh consent identity. Egg gifting remains a separate linked `source-gift` plus `recipient-acceptance` agreement; GM authority cannot create, revoke, or substitute either approval, and completion delegates to BR-064's atomic owner transition. Dedicated replay-safe `settle-egg-transfer-consent` operations persist campaign-time expiry or participant revocation before application, preserve immutable evidence, and never transfer ownership. Server-projected cards and bounded notifications structurally omit counterpart identity, private mechanics, Profile/evidence/command/roll facts, disable every action during pending recovery, and revalidate current authority on terminal retry. Responsive matte cards and the labelled transfer dialog provide semantic status/alert regions, native controls, 44-pixel targets, visible focus, touch/keyboard support, reduced motion, and recovery/expiry explanations. Fifty-six focused workflow/operation/migration/use-case/route/composable/component checks and the expanded 94-file/639-check Breeding surface pass; checker, changed-file lint, clean diff validation, and filtered typecheck report no BR-077-specific errors, and five semantic-registry-bound authority fixtures were rehashed transitively.
- [x] **BR-078 — Complete responsive, keyboard, screen-reader, touch, zoom, reduced-motion, and table-distance acceptance** — `DONE`
  - Evidence: Workshop interaction acceptance `b68242ba84122c5c0b45b3b9017ef1ea452fdd0396f9855a3a9f0908a4912c85` freezes the DESIGN.md component matrix at 320/390/768/1440 CSS-pixel widths, 200%/400% reflow, 44-pixel targets, complete semantic status/error/progress naming, reduced-motion equivalence, and shared-display hierarchy without changing server authority or structural privacy. The reusable presentation-only focus boundary moves focus into the Project wizard, advances it to each visible step heading, contains Tab/Shift+Tab in transfer and hatch dialogs, supports safe Escape close, and restores the connected opening control; it performs no API, command, aggregate, consent, or mechanics work. Long human labels wrap, narrow grids collapse to one column, modal block sizes remain viewport-bounded, touch controls avoid hover/right-click dependence, and every state retains text beyond colour/motion. The dedicated operator acceptance guide distinguishes this component closure from BR-079's real-browser/axe/multi-context ownership. Thirty-six focused component/acceptance checks and the expanded 95-file/646-check Breeding surface pass; checker, changed-file lint, clean diff validation, and filtered typecheck show no BR-078-specific errors, with five semantic-registry-bound authority fixtures rehashed transitively.
- [x] **BR-079 — Add Nuxt, Playwright, axe, multi-context, reconnect, visual-regression, and privacy browser suites** — `DONE`
  - Evidence: Production-browser acceptance `b17d69450b062d5052481b9351077c555eb131eb8bae792a7a84dcc40dc8de9c` consumes the BR-078 matrix `b68242ba84122c5c0b45b3b9017ef1ea452fdd0396f9855a3a9f0908a4912c85` without redefining it and binds the synthetic server-created projection fixture `dc7460f6c988ee559baa5b13fcffaa678892ddd739f69b9e5fb54abafe73e6c4` plus four reviewed desktop/mobile visual baselines. Nuxt `mountSuspended` coverage verifies role-private hydration, heading/modal semantics, initial focus, native hatch choices, and raw-ID omission. The production Nitro Playwright suite runs Desktop Chromium and Pixel 7 contexts, reports zero serious/critical WCAG 2 A/AA and 2.1 AA axe findings, exercises 320/390/768/1440 reflow, 44-pixel targets, reduced motion, modal focus containment/restoration, selector-only consent/hatch payloads, Profile-snapshot authority, local-persistence privacy, simultaneous structurally different GM/player views, bounded 503 Retry/full reconnect, and one card per aggregate. A cyclic grid-width overflow and undefined on-focus contrast token discovered by the browser suite were corrected fail visibly rather than waived. Seven project/browser checks plus three Nuxt checks pass against the production build; 159 focused checks and the expanded 95-file/646-check Breeding surface pass when combined with the synchronized checker, with changed-file lint and contract/hash validation clean and five semantic-registry-bound authority fixtures rehashed transitively.
...
- [x] **BR-080 — Enforce strict breeding-spec, project, Egg, operation, projection, and interaction manifest closure** — `DONE`
  - Evidence: Semantic-closure manifest `718a6e4507fa3aa930be47ac596a05c44e80a792bd64b1fe87fc815c551ac625` and registry `b93a3b5f6f5de183b72ea0cd16f1fd789a3f09910538c158f65b3e821c250447` close all six declared dimensions without becoming a second runtime authority. The checker recursively requires every `data/breeding-automation/**/*.json` artifact exactly once among 105 registry rows (except the registry self), rejects duplicate IDs/paths and hash drift, and verifies every manifest artifact and implementation path. Runtime equality binds all 407 Families/862 Species, 12 Project statuses, seven Egg statuses/four source kinds, 22 command/outcome kinds, 10 conflict scopes, five base audiences, seven Workshop APIs, and all 21 reviewed interaction IDs to their strict exported enums and owning contract hashes. Unknown/incomplete specs, commands, interactions, files, routes, paths, or client shape/hash drift fail closed; declaration remains distinct from, and is now bound to, BR-082's disposition-aware mechanics certification. Forty-three focused compiler/aggregate/operation/projection/checker closure checks pass, changed-file lint and checker pass, and all five semantic-registry-bound authority fixtures were rehashed transitively.
- [x] **BR-081 — Run whole-species compatibility, family, trait, inheritance, hatch, and ruleset conformance suites** — `DONE`
  - Evidence: Whole-Species conformance `07701bebb2d646a877fe47fe6964afcf4a30fe3e21f31773965f7f35fa7afbb7` certifies the current BR-080 closure and BR-019 pure matrix without supplementing the canonical registry. One exhaustive traversal validates all 407 Family DAGs, 455 evolution edges, 862 compiled Species/861 producible offspring, 1,126 Egg Group memberships, 1,507 Basic Ability options, 3,555 Egg-Move and 25,838 machine-compatible identities, ratio/genderless boundaries, two-parent inheritance deduplication/pathway provenance, exact per-Species hatch duration, and ordinary Level 1. The report also binds all 15 ruleset options, 287 explicit source exclusions, d20/ordered-2d6/d100/50–200 domains, 2,048 malformed fuzz cases, 100 deterministic full-pipeline replays, frozen child construction, seven-state lifecycle, two destinations, and atomic hatch settlement. The 12-file conformance checkpoint passes 95 tests in 18.59 seconds; checker and changed-file lint pass, and five semantic-registry-bound authority fixtures were rehashed transitively.
- [x] **BR-082 — Certify Edge, Feature, item, Ability, capability, move, form, fossil, Baby Template, and campaign-clock interactions** — `DONE`
  - Evidence: Interaction certification `93041fbf371c9b063232d174e16135d8ec5c231496d64c360a4abc6184344998` closes all 21 exact modifier-inventory rows against their app-owned canonical record/mechanic hashes, runtime owners, and focused evidence. Sixteen terminate in active owning runtimes; Egg Tutor, Ancient Heritage, Genetic Memory, and Tutoring remain typed upstream operation boundaries that cannot forge inherited provenance or bypass permanent-Move authority; `This One’s Special, I Know It` remains explicitly unavailable without durable rank-use consumption. The ten-dimension Edge/Feature/item/Ability/capability/Move/form/fossil/Baby Template/campaign-clock matrix preserves Trainer/Poké Edge separation, empty facility authority, exact item custody, synchronous effective providers, compiled form roots, ordinary fossil/artificial Egg pipelines, server-owned derived templates, and campaign-time-only lifecycle semantics. Six recovery cases bind retained rolls, source costs, learning, and bounded clock prefixes to exact resume and silent retry. The complete 25-file certification suite passes 213 tests in 44.10 seconds; semantic closure, whole-Species regression, checker, changed-file lint, and transitive authority-fixture hashes pass.
- [x] **BR-083 — Complete transaction failure injection, concurrency, idempotency, correction, abandonment, and disaster-recovery validation** — `DONE`
  - Evidence: Resilience certification `54c2d9ecbe5a9d23aac82ffac41b06cdd3632513a88a04e2077265047502e0c2` classifies all 22 strict command kinds without promoting declarations: 19 active transactional reducers, one recovery controller, one non-mutating preview, and one fail-closed declared Egg-cancellation boundary. Thirteen surfaces bind durable phase-one command/read-set/receipt/offer/roll evidence to one caller-owned synchronous phase-two transaction and current-authority-only recovery. Forty-five exact cases cover all-participant rollback, two-connection stale races, persisted-randomness reuse, silent terminal retries, audited correction, campaign-time expiry, evidence-preserving abandonment, crash convergence, strict restart reparsing, 100-Egg prefix recovery, replay gaps, atomic backup/restore injection points, and post-commit publisher failure. No accepted partial child, source cost, ownership transfer, learning checkpoint, Project, Egg, archive, or realtime state survives an injected rollback. The 26-file resilience suite passes 218 tests in 51.77 seconds; checker, semantic/report regressions, changed-file lint, diff validation, and transitive fixture rehashing pass. BR-085 retains final migration/export/import/reference/orphan release acceptance.
- [x] **BR-084 — Complete authorization, consent, privacy, information-flow, malformed-input, and abuse testing** — `DONE`
  - Evidence: Security certification `e254f56f6a56340d062e30371a4be2c52d60131c074194356523eaa24bb5bfaa` binds six dimensions to all five closed audiences, seven Workshop APIs, eight authorization surfaces, 11 distinct consent cases, 22 privacy fields, nine information flows, all 13 policy threats, 14 malformed-input classes, all 22 abuse controls, and eight durable audit requirements. Every Breeding POST now uses exact 32 KiB UTF-8 JSON ingress; confirmed Project creation, hatch mutations, and consent mutations use bounded request admission at 30 writes per player Profile and a conservative 120-write authenticated GM process-session bucket, returning only 429 plus `Retry-After`. Repository pagination now defaults to 25 and caps at 100, while one atomic conditional insert prevents a parent from entering two active Projects. The 39-file security evidence suite passes 279 tests; semantic/conformance/interaction/resilience regressions, checker assertions, documentation, route admission checks, and transitive authority-fixture rehashing pass. Request limits remain availability controls only and cannot affect campaign time, mechanics, randomness, authorization, operation evidence, or exact retry.
- [ ] **BR-085 — Complete legacy migration, export/import, backup/restore, reference-version, and orphan-repair acceptance** — `IN_PROGRESS`
