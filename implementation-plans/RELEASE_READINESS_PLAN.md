...
`BLOCKED_BY: OWNER_PATCH_RELEASE_AUTHORIZATION_P13_085`
...
- [x] **P13-074 — Rehearse the full release** — `DONE`
  ...
  - Evidence: immutable local annotated tag `v1.0.0-rc.5` (tag object `61c0821d…`) identifies clean commit `2e68864c17ee241cf624808703adfe4016deaea8`. One exact `npm run release:prepare` invocation passed the complete bounded aggregate, produced a 13,629-file production build, generated checksum manifest `21508460…`, recorded zero artifact-audit findings, and verified all four deterministic evidence inputs plus the self-excluding bundle manifest `0b63be70…` at exact `0640` permissions. A synthetic app-produced v55 database upgraded to v56, passed the 82-table/126-JSON-column integrity audit, restored byte-exact to SHA-256 `501a3fb0…`, then reused the matching backup and reproduced upgraded SHA-256 `33becc19…`. Production desktop 1440×960 and mobile 412×915 Chromium smokes selected the trusted-table GM role and verified Settings identity `1.0.0-rc.5`/v56/`2e68864c17ee`, zero horizontal overflow, and zero console errors. No tag or note was published.
- [x] **P13-075 — Record the release-rehearsal certification artifact** — `DONE`
  - Machine-readable rehearsal record binding command outputs, checksums, and evidence hashes.
  - Registered in the drift gate.
  - Evidence: `release-rehearsal-certification.v1.json` binds the immutable rc.5 commit/tree/annotated-tag object, supported builder, five bounded command rows, 13,629-file/379,045,054-byte output census, checksum and five evidence-file hashes, zero-finding artifact audit, six tagged source bindings, exact v55 upgrade/restore/re-upgrade hashes, production desktop/mobile Settings smokes, all eight Phase 7 rubric rows, privacy posture, and owner-controlled publication boundary. `check-release-rehearsal.mjs` verifies the tag through Git, tagged source bytes, schema/rubric states, drill convergence, browser outcomes, current source hashes, and exact operator-local evidence when present; `scripts/quality-gate.sh` registers the checker independently of mutable package scripts.
- [x] **P13-076 — Release rehearsal acceptance (slice gate)** — `DONE`
  ...
  - Evidence: `check-release-rehearsal.mjs` verifies the exact annotated rc.5 source and five-file operator-local bundle, the byte-exact restore/re-upgrade drill, both production browser smokes, and all eight ordered Phase 7 rubric rows as `Certified` against their allowed states. Changelog, release notes, release-command, artifact-audit, clean-host, rehearsal, and release-evidence checks all pass together; no publication or final `1.0.0` claim occurred. Phase 7 closes at 10/10 and Phase 8 may begin.
...
- [x] **P13-083 — Owner go/no-go review** — `DONE`
  ...
  - Evidence: the owner responded explicit `GO` after reviewing the plain-language effect of the transaction. The decision authorizes exactly one local P13-084 transaction, accepts the dossier's named recommendation-5 residual risk without claiming legal clearance, and does not authorize pushing a tag or publishing notes, artifacts, evidence, or a hosted release.
- [x] **P13-084 — Execute the atomic 1.0 release transaction** — `DONE`
...
- [ ] **P13-085 — Verify the released identity** — `BLOCKED`
  ...
  - External blocker: P13-083 authorized and consumed exactly one local `1.0.0` transaction. Version policy requires the next patch (`1.0.1`) for this post-tag source repair, but no patch release transaction is authorized. Completion requires explicit owner authorization for one local atomic `1.0.1` successor transaction; remote publication remains separately unauthorized.
...
- **2026-08-27 — Third release candidate exposed evidence drift fail-closed.** P13-074 performed `1.0.0-rc.2 → 1.0.0-rc.3`; local annotated tag `v1.0.0-rc.3` (tag object `c6ddd16a…`) immutably records commit `ed5e49a9c484737a78209e5f82656fb5b9d0ee90`. The repaired identity gate passed, as did upgrade and backup certification, before the aggregate stopped on a stale production documentary source-graph proof. Manual continuation identified five missing accepted hash successors, including the versioned package surface. No build, artifact audit, or release evidence bundle ran; rc.3 remains unmoved.
- **2026-08-27 — Fourth release candidate exposed a final permission-contract mismatch.** P13-074 performed `1.0.0-rc.3 → 1.0.0-rc.4`; local annotated tag `v1.0.0-rc.4` (tag object `f132584a…`) immutably records commit `a0ee5b1079079db71c6f9e2dd0cae36e7601f57f`. The complete aggregate passed, including 2,457/2,457 final mechanics rows, 34 golden-journey tests, 115 privacy tests, and 88 performance tests; the production build generated 13,629 files and the artifact audit found zero private, documentary-authority, dependency, or secret findings. The final verifier then rejected its own writers' intended `0640` group-readable evidence mode. Direct verification after predicate repair passed every remaining check, but the original command remained failed and rc.4 remains unmoved.
- **2026-08-27 — Fifth release candidate completed the full rehearsal.** P13-074 performed `1.0.0-rc.4 → 1.0.0-rc.5` after making the verifier require exact `0640` evidence files, adding a deterministic source assertion, and documenting the matching `0750` directory mode. Local annotated tag `v1.0.0-rc.5` (tag object `61c0821d…`) immutably records clean commit `2e68864c17ee241cf624808703adfe4016deaea8`. The exact release command passed end to end with 13,629 checksummed output files, zero artifact-audit findings, and a verified five-file local bundle; the v55→v56→byte-exact-v55→v56 drill converged, and production desktop/mobile identity smokes passed with no console errors or horizontal overflow. All prior tags remain unmoved, nothing was published, and owner control is unchanged.
- **2026-08-27 — Phase 7 release rehearsal accepted.** All eight notes/provenance rubric rows are `Certified`. Immutable `v1.0.0-rc.5` binds the clean command/build/evidence run, exact v55 restore/re-upgrade convergence, and desktop/mobile production identity smokes; the machine rehearsal checker passes with the five-file local bundle and from the immutable hash record. Phase 8 opens at P13-077 without publishing any tag, notes, artifact, or final release claim.
