...
- **2026-08-27 — Fifth release candidate minted for the permission-aligned rehearsal.** P13-074 performed `1.0.0-rc.4 → 1.0.0-rc.5` after making the verifier require exact `0640` evidence files, adding a deterministic source assertion, and documenting the matching `0750` directory mode. A new local annotated tag will identify the reviewed repair commit; all prior candidate tags remain immutable and publication remains owner-controlled.
