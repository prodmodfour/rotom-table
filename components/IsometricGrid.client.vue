...
const getWeatherBounds = (): WeatherBounds => {
  ...
  const maxMapSide = Math.max(props.dimensions.x, props.dimensions.z)
  const margin = Math.max(1.4, Math.min(5, maxMapSide * 0.22))
  const topY = Math.max(
    highestY + 3.2,
    groundY + Math.max(4.5, props.dimensions.y + 1.5),
  )
  const minX = -margin
  const maxX = props.dimensions.x + margin
  const minZ = -margin
  const maxZ = props.dimensions.z + margin
  ...
  return {
    ...
    minZ,
    maxZ,
    centerX: props.dimensions.x / 2,
    centerZ: props.dimensions.z / 2,
    groundY,
    topY,
    width: maxX - minX,
    depth: maxZ - minZ,
...
const makeSunnyWeatherVisual = (seed: string, index: number, total: number): WeatherVisual => {
  ...
  const rayCount = Math.round(THREE.MathUtils.clamp(props.dimensions.x * 0.45 + 3, 5, 10))
  ...
  for (let i = 0; i < rayCount; i += 1) {
    ...
    const rayWidth = randomRange(rand, 0.55, 1.25) * Math.max(1, props.dimensions.x / 9)
...
const makeRainyWeatherVisual = (seed: string, index: number, total: number): WeatherVisual => {
  ...
  const area = props.dimensions.x * props.dimensions.z
...
const makeSandstormWeatherVisual = (seed: string, index: number, total: number): WeatherVisual => {
  ...
  const moteCount = Math.round(THREE.MathUtils.clamp(props.dimensions.x * props.dimensions.z * 2.15, 90, 260) * (total > 1 ? 0.75 : 1))
...
const makeWeatherVisual = (kind: MapWeatherKind, index: number, total: number): WeatherVisual => {
  const seed = `${kind}:${props.dimensions.x}x${props.dimensions.y}x${props.dimensions.z}:${index}`
  switch (kind) {
    case 'sunny':
...
const syncFieldEffectMeshes = () => {
  ...
  effects.weather?.forEach((effect, index) => {
    ...
    fieldEffectContainer.add(visual.group)
...
const animate = () => {
  ...
  for (const updateFieldEffect of fieldEffectAnimators) {
    updateFieldEffect(delta, clock.elapsedTime)
  }
